...
- Mission interview `POST /api/missions/interview/create-mission` now auto-generates contract assertions at milestone, slice, and feature levels; route/e2e tests should expect extra `addContractAssertion` calls while `linkFeatureToAssertion` remains feature-only.
