...
## FNXC_LOG comments:
   ...
   - **Take the timestamp from `date -u`, not your local clock.** `check-fnxc-future-dates` compares each stamp against the calendar date of the machine RUNNING it (`new Date()` local getters — see the `#2941` note in the script). In CI that machine is on UTC, so **UTC is the calendar that decides whether `main` goes red**, and a stamp must never be ahead of it. This cost the fleet four separate breakages in one day (`scheduler.ts`, a scheduler PG test, and `task-update.ts` twice, by different authors), plus three more in one ninety-minute window; every one was a real time on the wrong day. The gate also rejects an impossible clock time, so an hour above 23 or a minute above 59 fails for a different reason.
   - The two directions are NOT symmetric, which is why "just use your local clock" keeps failing for some people and not others. If your clock is **ahead of UTC**, your local date can be tomorrow in UTC terms: writing it passes `pnpm lint` locally and turns `main` red for everybody else. If your clock is **behind UTC**, your local date is merely in the past, which the gate always accepts — but a correct `date -u` stamp then reads as FUTURE to your own local run after UTC midnight, so `pnpm check:fnxc-future-dates` goes red on your machine while CI is green. That local red is an artifact of your timezone, not a broken `main`: confirm with `date -u` against `date` before opening a fix, and check whether CI's Lint job is actually failing. Three PRs were opened against one such stamp in a single evening.
