...
- Workspace late acquisition: `task:workspace-scope-extended-post-review` is emitted once per repository acquired while its task sits in a review column with nothing landed (tier-1 admission), after the checkout and the scope extension and before the forced Code Review re-entry, through `emitBoundedRunAudit`. Metadata is ids/counts/fixed outcomes only (`taskId`, `repo`, `column`, `repositoryCount`) and never the executor's reason prose. Acquisition remains refused once landing has begun — any `landedSha`, a `merging*`/`workspace-review-required` status, or a merge pending/active for the task — and for a task that is not review-evidenced or whose workflow has no reachable Code Review node.
