...
- RUFU-182: the chat-lane pre-overflow compaction gate emits `chat:pre-overflow-compaction` (via `emitBoundedRunAudit`, target `chat:<sessionId>` / `chat:room:<roomId>`) once per gate invocation that reached the escalation ladder; a below-threshold send emits nothing. Metadata is ids/counts/fixed enums only (`tier`, `tiersAttempted`, `reason` from the overflow reason enum, `outcome` `compacted`/`proceeded-without-reduction`/`refused`/`measurement-unknown`, `beforeTokens`/`afterTokens`/`threshold`, `retrySkippedReason`); the refusal sentence lives on the chat message and never in run-audit, and `measurement-unknown` wins over `refused` whenever pi's own post-compaction measurement was unusable, so operators can distinguish "the engine refused" from "we could not observe what the engine did". RUFU-183 adds tier `fallback` to the ladder (`tier` enum `normal`/`aggressive`/`fallback`): when the LLM tiers yield nothing, ONE zero-LLM deterministic truncation runs per send and the row gains count-only `droppedMessageCount`/`droppedTokens`/`floorTokens` keys; `outcome` keeps its four values (a converged tier-3 rescue reports `compacted`), refusals keep the RUFU-182 reason codes — tier 3 is a tier, never a ninth reason — and the rescued chat reply carries the plain-language drop notice on the message row, never in run-audit.
...
- FN-9187 historical compatibility: persisted `task:auto-archive-failure-budget-exhausted` rows remain readable after task archiving was retired. No live engine path emits this event.
...
- FN-7720: `TaskStore.bypassFailedPreMergeReviewStep` emits `task:bypass-review` when a privileged operator bypasses a blocking pre-merge review gate of an `in-review` task. Target selection prefers the latest `failed` row, then a required gate with no result at all, then (FN-295) a required gate whose existing result is simply not an approval — including a `skipped` row archived by another gate's remediation, whose `remediationArchivedAt`/`remediationArchivedFromStatus` stamps the waiver erases, because an audited human decision supersedes an automatic archive and the stamp is otherwise an unconditional merge veto. Metadata includes metadata includes `workflowStepId`, `workflowStepName`, `bypassedFromStatus`, `bypassedFromVerdict`, and the mandatory `reason`. The bypass rewrites the step's `status` to `"skipped"` with `bypassedBy`/`bypassedAt`/`bypassReason`/`bypassedFromStatus` fields; it never fabricates a reviewer `verdict` and clears only the failed-pre-merge-step `getTaskMergeBlocker` reason. Reachable via `fn_task_bypass_review` (CLI/pi-extension operator tool surface only — not executor/reviewer/triage) and `POST /tasks/:id/bypass-review`.
...
<!-- FNXC:WorkflowIdentityAndModels 2026-09-14-19:11: Built-in revisions keep stable public ids; migration 0079 converges temporary revision ids, archives conflicts, resets obsolete workflow model lanes, and preserves task history. Model resolution is task → selected workflow → project role → global role → project/global Default, with Planner, Executor, Reviewer, Merger in consistent UI order and project models stored only as project settings. -->
- FN-393: built-in revisions retain their original public workflow ID. `builtin:coding-ideas-v2` is migration input, not a runtime successor alias: migration 0079 converges it to `builtin:coding-ideas`, archives conflicting legacy workflow-owned records, resets obsolete workflow model-lane values, and preserves task history. Project/global Models present **Default**, **Planner**, **Executor**, **Reviewer**, then **Merger**; Default exists only at project/global scope. Each role owns its fallback, thinking, and credential selection. Runtime precedence is task → selected workflow → project role → global role → project Default → global Default. Project models are stored as project settings, never as values on the default workflow; workflow-specific settings remain owned by the workflow editor.
...
Keep this AGENTS inventory in sync with App lazy imports (including `WorkflowNodeEditor`, whose only declaration is the App-level Workflows view), AppModals lazy modal imports (`SettingsModal`, `SetupWizardModal`, `PatchnodeView`), plugin settings lazy imports (`PluginManager`, `PiExtensionsManager`), AgentsView lazy imports (`AgentDetailView`), and `packages/dashboard/app/__tests__/lazy-loaded-views-docs.test.ts`.
...
Note: the embedded main-content views Import Tasks (`_ImportTasksView`), Automations (`_AutomationsView`), and Settings (`_SettingsView`) in App.tsx are `_`-prefixed lazy splits that reuse already-documented chunks. Workflows is NOT one of them: FN-407 removed the workflow editor's modal presentation, so its App-level declaration is the single curated `WorkflowNodeEditor` entry above. Task session terminals, the Task Detail embedded terminal, onboarding-internal modals, duplicate `AgentDetailView` imports, and right-dock overflow re-imports of already-counted views are also intentionally excluded. These exclusions stay out of the curated list and count; `lazy-loaded-views-docs.test.ts` asserts them explicitly, so do not add them as bullets.
...
- FN-9234/FN-9264: `task:review-input-recaptured` and `task:merge-stale-content-review-rerouted` use `emitBoundedRunAudit`; metadata is ids/counts/fixed outcomes only (`taskId`, workflow step id, approval verdict or reroute reason/source, `parkShape`, `parkCleared`, `mergeRetriesReset`, and resolved-in-review finding count). Self-healing recovers only the bounded-retry rejection, raw merge-door blocker, and retry-exhausted stale-content parks after re-seeding review. Fingerprints, diffs, paths, findings, and reviewer prose are never recorded.
...
- FN-9304: `task:reconcile-absent-branch-landed` and `task:reconcile-absent-branch-unproven` use the FN-9175 bounded engine seam. Metadata is ids/fixed outcomes only; hostile audit sinks cannot alter absent-branch reconciliation, and the unproven marker is set only after a landed audit write.
