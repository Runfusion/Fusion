...
- FN-9304: `task:reconcile-absent-branch-landed` and `task:reconcile-absent-branch-unproven` use the FN-9175 bounded engine seam. Metadata is ids/fixed outcomes only; hostile audit sinks cannot alter absent-branch reconciliation, and the unproven marker is set only after a landed audit write. `fn task reconcile <id>` and the automatic self-healing absent-branch sweep share the single `SelfHealingManager.reconcileLandedReviewTask` fence.
