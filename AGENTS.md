...
- RUFU-144: self-healing emits `chat:stale-in-flight-generation-cleared` when its startup + maintenance-batch-1 sweep `reconcile-stale-in-flight-chat-generations` clears `project.chat_sessions.in_flight_generation` rows whose `startedAt` (fallback: session `updated_at`) is older than the 30-minute floor, and deduped `chat:stale-in-flight-generation-no-action` when candidates exist but none are stale. Metadata is ids/counts/outcomes-only (`count`, `sessionIds` capped at 20, `outcome`); rows with unparseable timestamps are never cleared, and payload text is never recorded.
