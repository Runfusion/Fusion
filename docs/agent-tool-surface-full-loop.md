...
- `packages/engine/src/scheduler.ts` computes two independent project dimensions plus the shared host **`semaphore`**. Settings default to `maxConcurrent` 2 and `maxWorktrees` 4.
- `maxConcurrent` gates every top-level AI-active task across planning, execution, and review/merge. Planning runs read-only on the dependency-installed project root and consumes this provider-load budget without acquiring a checkout.
- `maxWorktrees` gates only worktree-consuming execution admission: WIP reservations count before checkout persistence, and live review/replan cards with retained singular or workspace checkouts keep their slot. It does not throttle planning. When disabled, this gate is structurally absent.
- The workflow hold/release sweep reserves agent, worktree, and optional semaphore capacity **before** moving a task to WIP, then transfers the reservation to the executor. A worktree-blocked execute candidate does not starve a checkout-free planning candidate ordered behind it.
- The host semaphore remains the process-global pool. Nested helpers remain parent-internal and intentionally soft-breach the displayed top-level agent cap to avoid parent/child deadlocks.
...
- **EXAM-010 write-ahead + watchdog + reclaim (2026-08-31):** every executor-invoked `fn_run_verification` call now persists a `task_verification_requests` record BEFORE dispatch (write-ahead), so a killed run always leaves an inspectable record and `fn_task_verification_status` never reports a spurious "No verification request exists" for a task that issued calls. Each call is bounded end-to-end by the `verificationWatchdogTimeoutMs` project setting (default 600000 ms; covers slot-queue wait plus subprocess) — on expiry the queued acquisition is aborted, the child is SIGTERM→SIGKILL escalated, the tool settles with an explicit `timedOut` failure, and a terminal `failed` record is persisted with a rejection reason naming the ceiling. An executor-initiated call that finds an unclaimed chat request claims it in place (preserving the original `requestId`); a `running` record is never stolen — instead, stale `running` rows from a dead executor are reclaimed by a `requestId` + `startedAt` CAS transition to terminal `failed`, unblocking follow-up requests without an operator restart.
