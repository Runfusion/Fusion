...
| --- | --- | --- |
| FR-02: optional mission admission | Ordinary autonomous intake may remain unlinked. When a task supplies mission/feature lineage, deterministically prove that the chain is approved before persisting mission metadata or entering mission-owned symbol/reconciliation paths. | No-task heartbeat prompts in `agent-heartbeat-prompts.ts` encourage one useful action and permit `fn_task_create` after duplicate checks. `renderHeartbeatNoTaskSystemPrompt({ plannerHeartbeatPatrolEnabled: false })` suppresses proactive patrol creation independently of optional mission linkage. |
...
### Phase D — Optional mission admission and safe parallel delivery
...
- **Acceptance:** ordinary autonomous tasks may remain unlinked; supplied mission lineage must be approved before mission-owned execution coordination, independent tasks use available `maxConcurrent`, `maxWorktrees`, semaphore, and verification capacity while conflicting declared symbols serialize through durable locks, diagnostics identify the binding gate/lock, and linked completion reconciles roadmap state.
