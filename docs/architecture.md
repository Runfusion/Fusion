...
Generic terminal-failure, ghost-review, stale-incomplete-review, in-progress-limbo, and zero-progress no-task-done sweeps no longer move or terminalize cards. Only a revision moves work backward. The remaining WIP liveness healer is the stuck-session detector: it disposes a silent session and re-dispatches the same task at the same column, workflow node, current step, worktree, and branch. Repeated silence remains automatically recoverable with no retry budget or human-escalation terminal. User pauses and declared external Blocked freezes remain under explicit human control.

## Planning dependency lifecycle lock

Dependency changes and planning finalization share one outer lifecycle lock keyed by the canonical project ID and task ID. In PostgreSQL mode this is a dedicated, single-connection session advisory lock: it is acquired before the normal task lock and released before the mutation/finalization Promise settles. Filesystem, Git, and init preparation must not occupy the task mutex while a planning-lock holder waits; executor planning-lock transport failures use bounded retry rather than terminal parking. Startup spec-drift replay is limited to live tasks to avoid multiplying these waiters. The operational runtime pool is never borrowed for this purpose.
...
- **Executor**: `TaskExecutor` (`executor.ts`) implements tasks in worktrees
  ...
  - **No root task worktree (FN-034):** when `.fusion/workspace.json` declares repositories, planning, execution, graph nodes, code/plan review, pause/resume, restart, and worktree-session recovery acquire or reuse only the declared repositories' worktrees. The workspace root and `<workspace>/.fusion/worktrees/<task>` are never task checkout or reviewer cwd. Historical singular `worktree`, `branch`, execution-base, and root-session routing metadata is cleared through an advisory-locked, project-scoped mutation that preserves every `workspaceWorktrees` entry and completed-step progress; recovery never deletes or recreates a root checkout.
...
- **Merger**: `runAiMerge()` (`merge/merger-ai.ts`) is the sole production path that merges approved work; the retained `aiMergeTask()` body is soft-deprecated and inert.
  - **Execution/merge exclusion and current approval (FN-180):** a task with a live execution session, execution lock, or active-task signal is deferred at every merge admission door independently of review state; execution likewise does not claim a task while its merge is in flight. An enabled pre-merge gate requires a current positive approval: a singular task binds a diff-domain review to the current diff fingerprint, while a workspace task binds each modified repository fingerprint to confirmed `repositoryScope.reviewEvidence`. A lane that fast-forwards its own checkout while approving re-captures only its own identity; other stale singular lanes are re-entered through an idle-guarded graph review, while workspace re-entry continues through its shared-evidence route. Non-fast-forward or unreadable evidence, non-approval outcomes, plan-domain approvals, active continuations, and no-progress routes fail closed. Gate resolution has three cases: a store without workflow-selection readers keeps legacy result-only semantics; a workflow-aware store with no task selection applies the default workflow's default-on groups; and a failed selection read refuses the merge. Unavailable content evidence blocks rather than being treated as approval. Plan-review fingerprints bind plan text and are never compared to a code diff. A merge is revoked when it leaves its review lane or a current gate becomes blocking, and landing rechecks the gate before advancing an integration ref. Once a merge is confirmed, finalization reconciles stale checklist state instead of parking already-landed work as failed.
  - **Post-landing task-worktree cleanup (FN-251):** for a single-repository task with `mergeDetails.mergeConfirmed === true`, `CompletionLandedCleanup` proof-gates cleanup before a task enters its complete column. `finalizeProvenAutoMergeTask` is the shared enforcement seam for direct AI merge, workflow merge, graph recovery, fast-path, and its self-healing callers; the six direct self-healing complete-lane finalizers use the same helper. `reconcileCompletedTask` remains a post-move convergence backstop for worktrees leaked by older finalization paths, including a branch-to-worktree lookup when the task pointer is already null.

    | Cleanup outcome | Worktree state | Complete-column move |
    | --- | --- | --- |
    | `removed` | Deleted and the task receives exactly `{ worktree: null }` | Continues after cleanup resolves |
    | `nothing-to-remove` | No pointer or no path exists; convergence clears a stale pointer when a prior post-removal write failed | Continues |
    ...
    - FN-9233/FN-9276 classify defensive cleanup as `clean`, `regenerable-ignored`, `ignored-only`, or `deliverable`. A checkout containing only ignored directories on the conservative regenerable allowlist (dependency and build names such as `node_modules`, `dist`, `target`, and `__pycache__`) is removable by every defensive reason. A top-level `.fusion` directory is likewise regenerable only when its immediate children are successfully probed and every child is allowlisted Fusion scratch (`cache`); nested `.fusion`, unexpected children, unreadable scratch directories, and quoted paths fail closed as `ignored-only`. Any other non-allowlisted ignored entry remains `ignored-only`, so only `CompletionLandedCleanup` with durable `postLandingProof` may remove it. Defensive removal refuses the project root checkout before this classification can authorize deletion. Modified tracked files, untracked non-ignored files, unreadable status probes, and live sessions always remain fail-closed. `worktree:removal-discarded-regenerable-content` records an allowlisted discard with IDs and entry count only; `worktree:post-landing-ignored-content-discarded` records proof-gated ignored-only removal; and `worktree:removal-preserved` now also records ignored-only preservation.
    - Direct AI finalization resolves cleanup before deleting a Fusion-managed task branch because Git refuses to delete a branch still checked out by its worktree. Operator-supplied branches remain operator-owned. Cleanup is non-fatal after a durable landing; branch deletion and task finalization still proceed after a preserved outcome or a recorded pending pointer clear, which convergence retries against the absent path.
...
- `Scheduler` (`scheduler.ts`) — dependency-aware task scheduling that dispatches eligible todo tasks in the single shared queue order: an effective Boost first, then FIFO (`createdAt` ascending) with the task-id tie-break. FN-509 removed priority tiers AND the FN-4969 dependency-unblock fanout weighting: both were ways for a newer card to overtake an older admissible one, which is exactly what the chronological queue exists to prevent. Boost is the only forward move, it is operator-only, and it never bypasses capacity, overlap, dependency, approval, or pause gates.
  - **Independent agent/worktree admission (FN-8822/FN-9185/FN-282):** scheduler execute, triage specify, project-engine merge, and direct workflow-continuation handoffs share one serialized project coordinator with two orthogonal gates. `maxConcurrent` counts every canonically live top-level AI task, including planning, and protects provider load. Optional `maxWorktrees` counts live WIP reservations plus live tasks retaining singular or workspace execution checkouts, and protects host CPU/RAM/disk. Planning runs read-only on the project root and consumes no worktree slot; merge and continuation handoffs resume an existing task and do not claim a new slot. The core resolver returns both values without taking their minimum, while transient reservations close each gate's persistence-transfer window. A full gate persists its own used/limit and holder IDs; worktree limiting off means no worktree gate object exists.
  - `blockedBy` invariant (FN-3924/FN-4091): the field is only durable when it references a current unresolved explicit dependency (or, for dependency-free tasks, an active overlap blocker). Completion gating now validates `blockedBy` through live task resolution: missing blockers and blockers already in a workflow Complete column are treated as stale, while only still-active blockers continue to prevent `fn_task_done`. If no current blocker remains, scheduler/event reconciliation clears `blockedBy` to `null` and re-evaluates from live task state.
  - `overlapBlockedBy` invariant (FN-283): the field is durable only while the named task currently holds a blocking file-scope lease over the candidate's scope. Engine repair re-reads the dependent and blocker immediately before clearing, then compare-and-sets the exact inspected blocker ID; a changed ID, revived blocker, or soft-deleted dependent is a no-op. This fence is required because scheduler and executor queue producers use `transitionQueuedEpisode`, whose advisory-locked transaction intentionally overwrites the field unconditionally. `blockOuterDispatchWhenFileScopeLeaseHeld` remains the final fresh pre-execution backstop.
...
#### Overlap release synchronization (FN-332)

`overlapBlockedBy` remains only the current contention/display truth. Every observed edge is also inserted into `project.task_overlap_waits` in the publisher's existing transaction, before the marker can be replaced or cleared. Publishers keep the lock order task advisory lock → workflow serialization → rows; they never perform Git, network, or review work while holding SQL resources. Reset cancels prior generations under that same order, while Retry and ordinary forward movement preserve them.

A post-commit wake makes the graph continuation eligible but does not authorize ordinary work. The executor must claim the pending generation, derive actual landed-file overlap, prove the delivered target is included in the checkout, and persist either `resume` or `briefing`. This strict freshness requirement applies only to recorded wait episodes; the best-effort base refresh for unrelated acquisitions remains unchanged. Receipts and the task log survive restart, and a stale owner cannot publish after Reset, deletion, checkout replacement, or a newer revision. Structural or incomplete overlap evidence produces a factual briefing only; no synthetic review or repair model runs.

A delivered predecessor commit can change SHA after landing, because the integration branch was rebased (FN-428: `main` rebased onto `origin/main`, every delivered commit keeping its patch and its `Fusion-Task-*` trailers while the predecessors' recorded `mergeDetails.commitSha` kept the pre-rebase SHA). An absent SHA is still never a no-op. When the remembered SHA is missing, synchronization first preserves a dirty stale checkout in place, then performs its ordinary base refresh, and only then — on a clean checkout — attempts a proven reconciliation: the original object must be readable, its `git patch-id --stable` must equal the candidate's, the candidate must carry the predecessor's `Fusion-Task-Id`/`Fusion-Task-Lineage` trailer (a subject anchor alone is refused), the candidate must be an ancestor of both the execution checkout and the declared target, and exactly one candidate may qualify inside a bounded window. Anything else — modified patch, foreign or ambiguous identity, wrong target, genuinely unlanded delivery, unreadable evidence — keeps a bounded refusal in the same phase, and a diagnostic names the stale evidence for the operator. A proven reconciliation is persisted in the receipt alongside the original SHA under the existing revision/owner fence, so Retry, an engine restart, and worktree deletion do not discard the progress; a memorized reconciliation is reused only after its SHA is re-proven in the current checkout. Neither the recovery planner nor self-healing gains backward-move authority from this: the planner's retry handler reports a contained-recovery refusal as a refusal instead of a successful retry, and the `tip-already-merged` reclaim withholds entirely (no worktree destruction, no cleared `error`/`status`) while any overlap episode is still working, because that reclaim cannot repair delivery evidence.

Before any first planning/hold-to-WIP admission, the shared hold-release authority evaluates the plan's atomic `## Plan Premises` against the current main checkout. Sweep, explicit promotion, external events, dashboard movement, and CLI movement all use this authority before capacity reservation or worktree allocation and recheck from the locked live row. False or invalid facts retain the planning lane with `needs-replan`; unavailable reads remain fail-closed and retryable. Review-to-WIP remediation and recovery are continuations of already-started work, not first admissions.

#### File-scope overlap lease lifetime (FN-254)

With `groupOverlappingFiles` enabled, an overlap claim runs from dispatch until work lands. A lease is `active` for WIP (even before checkout acquisition) and for review with a singular or per-repository workspace checkout, `dormant` for any other non-terminal lane with a retained checkout, or `none` for terminal/deleted work and non-WIP rows without a checkout. Checkout-free planning therefore owns no lease and overlapping planning cards never serialize one another. A hold-lane card that retained an execution checkout across replan still owns unmerged work and keeps its dormant lease—checkout evidence, not a column exception, decides. The scheduler checks terminal → WIP → review-with-checkout → other-with-checkout in that order: WIP deliberately is not checkout-gated because dispatch precedes persistence, while clearing every checkout is the release condition after landing. Active leases serialize immediately; dormant leases use the shared queue order (Boost → arrival → task ID) so two retained holders cannot deadlock. A Boost never moves a waiting card ahead of an ACTIVE holder. Archiving, deleting, or clearing a dead holder's checkout is the explicit release path.

Completion fan-out re-reads the dependent, current blocker, workflow roles, settings, and normalized scopes before compare-and-setting a stale `overlapBlockedBy`. If executor admission stops a claimed workflow continuation before graph entry, dispatch settlement owner-fences that exact `running` row into a lease-free `held` wait carrying `file-scope:<blocker-id>`; if completion already cleared the overlap, settlement instead returns the claim directly to `runnable`. After a task-level clear commits, completion fan-out publishes the exact dependent/blocker release identities, runtime composition compare-and-sets only matching waits to `runnable`, kicks the bounded continuation drain, and requests one coalesced scheduler pass. A request received during an active pass is retained and runs immediately after that pass finishes, without allowing concurrent passes. Successor owners, terminal rows, changed blockers, pauses, dependencies, capacity, and runtime stop remain guarded by the normal claim and dispatch path. The scheduler poll, `clearStaleBlockedBy()`, and stranded-continuation reclaim remain convergence backstops rather than the normal release trigger. Global or engine pause still suppresses reconciliation, and stopping the scheduler cancels an unstarted follow-up.
...
#### Continuation-drain pump guard (FN-9303)

The runtime continuation drain is a single-flight pump. Once it claims its re-entry guard, every exit path—including pause and unreadable-settings paths—must pass through its releasing `finally`; the watchdog is a last-resort escape hatch, not ordinary recovery. It measures time since the owning pass last made progress rather than total elapsed time, so an event-loop-starved but advancing pass is not reported as wedged. A watchdog force-open supersedes ownership but does not cancel the previous pass, therefore the guard, progress timestamp, and phase are generation-fenced: only the current generation may update or clear them.
...
- Self-healing independently reconciles `overlapBlockedBy` across hold, WIP, and unpaused review roles regardless of `blockedBy`, `status`, or unresolved dependencies. It clears the field when the named task no longer holds a blocking file-scope lease or grouping is disabled, without changing dependency, status, pause, or column state; scheduler overlap stamping remains focused on stable active-overlap attribution.
- `reconcileDependencyBlockingLeases()` (FN-6292/FN-217) detects dependency/lease circular waits and records queue metadata in place. It never moves the holder backward; only explicit revision routing may change lifecycle rank.
- `reconcileInReviewUnmetDependencies()` (FN-6793/FN-6797/FN-217) records `status: "queued"` and the first live unmet dependency in place. It does not return review work to WIP or hold; pause, user pause, `autoMerge:false`, and live execution/checkout guards remain no-action conditions.
...
### Task worktree dependency readiness

Native task worktrees are unconditionally task-ID-pinned (`<worktreesDir>/<lowercased-task-id>`), with the unset default at `<projectRoot>/.fusion/worktrees`; Worktrunk remains responsible for its own layout. While no override is configured, existing `<projectRoot>/.worktrees` paths remain containment-valid and sweepable without migration. Fusion no longer selects random or title-derived directory names and no longer retains completed worktrees in a recycle pool. Workspace tasks use the same rule at their task directory, with one child worktree per repository declared by `.fusion/workspace.json`.

`worktree-dependency-install.ts` runs a bounded root-level bootstrap after fresh or reusable task acquisition. It shares the Node command chooser and frozen-lockfile retry rules with the AI-merge clean-room dependency sync, but otherwise leaves the clean-room path unchanged. The task bootstrap records per-ecosystem outcomes in the worktree's private Git directory and distinguishes `not-needed` from `unrecognized` package-manager evidence. Known matrix rows run only when their binary is available on `PATH`; no-evidence repositories spawn nothing. Unknown named manifests and generic lock/declaration-shaped root entries become planner work rather than silently passing.

The planner's `fn_install_worktree_dependencies` tool executes its command through the engine command/sandbox seam and records only an observed zero exit as installed; a reasoned `none` records an explicit no-install decision for unrecognised evidence. Plan Review remains the blocking authority: it retries deterministic rows once and returns an ordinary `Dependencies are not installed.` REVISE while any member remains unresolved or unrecognized. A repeated deterministic command failure with an identical pre-spawn worktree-state token instead resolves to `config-blocked` and freezes the task for operator configuration rather than spending Plan Review budget. Indeterminate state fails open; the evidence-gated dependency path does not sniff diagnostic prose.

### Concurrency, recovery, and resiliency
- **Lifecycle containment (FN-207):** For explicitly engine- or scheduler-sourced moves, columns are classified from their own effective trait flags using highest-rank-wins precedence. A structural deny-list forbids automatic intake targets, backward jumps over more than one rank, review exits other than to WIP, and backward terminal exits. Every remaining backward move must name `ENGINE_BACKWARD_MOVE_REASONS`; capacity refusal defers in place rather than selecting another column. Optionless and user-sourced moves, plus trait-less legacy shapes, remain fail-open for operator compatibility. Automatic transitions write a `Lifecycle move:` task-log line and activity metadata with direction, source, and reason.
...
- **Workspace terminal worktree reclamation (FN-9056):** `reconcileOrphanedWorkspaceWorktrees()` removes addressable per-repo worktrees for complete-lane rows and only conservatively-idle failed or soft-deleted rows. It vetoes raw and canonical active paths, task/executor/merge liveness, operator pauses, and scheduled recovery. A forensic claim index rejects duplicate, foreign, outside-root, and unowned repo paths before invoking git. Successful removal (or an already-gone path) always runs `git worktree prune`; canonical `fusion/<id>` branches are deleted only with landed/operator-delete/zero-ahead proof. One entry-scoped `MAX_STARVATION_DROPS` budget covers remove, prune, evidence, and branch operations; completed or exhausted entries settle so periodic maintenance cannot repeat git work indefinitely.
  - Periodic-maintenance pause contract (RUFU-076): the maintenance timer is only armed for an UNPAUSED project and is cleared on a `globalPause`/`enginePaused` pause transition, re-arming on unpause, so a paused project runs no periodic self-healing git churn. Under pause, `runMaintenance` batch-1 runs only its pure-DB/FS housekeeping steps (task-lifecycle retention, GitHub check-state retention, symbol-lock reconcile, WAL checkpoint, operational/agent-log prune); the git-churn steps (`prune-worktrees`, `cleanup-orphans`, `cleanup-stale-temp-merge-worktrees`, `cleanup-orphaned-branches`, `enforce-worktree-cap`) are skipped while paused and, on an active project, demoted to at-most-hourly via a coarse-cadence gate instead of every `maintenanceIntervalMs`. `recoverDoneTaskMergeMetadata` is bounded to a per-cycle candidate cap so it is no longer O(done_tasks) × git per maintenance cycle.
  - Batch 1 maintenance includes `reconcile-orphaned-task-dirs` (FN-6783), a paused-safe housekeeping step that calls `TaskStore.reconcileOrphanedTaskDirs()` so valid live `.fusion/tasks/{ID}/task.json` records missing from PostgreSQL become visible without waiting for process restart. The guard skips any ID already present in active, soft-deleted, historical, or tombstoned storage and emits `task:reconcile-orphaned-task-dir` only for recovered rows.
  ...
  - Batch 1 maintenance also includes `reconcile-phantom-committed-reservations` (FN-7069), which calls `TaskStore.reconcilePhantomCommittedReservations()` for committed task-ID reservations that have no live, soft-deleted, or historical task row and no `.fusion/tasks/{ID}/task.json`. The sweep prunes orphaned `activityLog` rows and `agents`/cascaded `agentRuns`, preserves `runAuditEvents`, and keeps the reservation `committed` per FN-5105 so the ID is permanently reserved rather than resurrected or handed out again.
  ...
  - Batch 1 retains an `fts-maintenance` compatibility step, but PostgreSQL tsvector/GIN indexes are synchronized on write, so the step is a no-op. Legacy cold-archive indexes remain migration/forensic storage and have no live maintenance producer.
  - AI merge clean-room worktrees are created under the configured worktrees directory's hidden container, `<worktreesDir>/.ai-merge/`, as `fusion-ai-merge-fn-<id>-<random>` detached worktrees. Pre-merge recovery, pre-merge prune, stranded-commit recovery, and the stale sweep scan that current root, legacy `.fusion/ai-merge/`, historic `<root>/.worktrees/.ai-merge/` when `worktreesDir` is not configured, and `tmpdir()`; an explicit configured root remains exclusive of the historic root. See [orphan merge-body durable-write fences](solutions/reliability/merge-orphan-body-durable-write-fences.md) for the bounded abort/settle-latch investigation and its pinned write-frontier ratchet. Every repo-local clean-room root is added to the repo's local git exclude when possible, so an in-flight clean room cannot dirty the integration checkout. Before constructing a clean room, `runAiMerge` skips directly to the existing finalizer when the task row proves that the same branch already landed on the resolved integration branch. The proof requires `mergeConfirmed: true`, a locally present `commitSha` reachable from the integration ref, an absent or matching `mergeTargetBranch`, and a recorded `landedBranchTipSha` equal to the live task-branch tip. Before finalizing, it removes the task worktree and atomically deletes the branch ref only when Git still sees that pinned tip; a concurrent branch advance fails the expected-tip check, preserves the branch, and falls through to the full merge. The finalizer completes interrupted task bookkeeping without changing the first landing timestamp. Missing, unreadable, unreachable, or mismatched evidence always falls through to the full merge. The shared dependency-readiness helper runs immediately after fresh or pooled task-worktree acquisition, including each workspace sub-repository, and after `git worktree add` in an AI-merge clean room before its merge/review loop. A configured `worktreeInitCommand` is authoritative and prevents inference; otherwise Fusion infers `pnpm`/`npm`/`yarn`/`bun` installs from lockfiles and can skip only when the `node_modules/.fusion-install-marker` hash still matches. Acquisition failures are non-fatal and make the first verification less likely to run, while clean-room failures and aborts hard-stop the AI merge before merge agents or verification run. Both lanes record the same command, source, decision, and duration vocabulary. Inline cleanup runs from `runAiMerge`'s clean-room `finally` for successful lands, empty/no-op finalization, concurrent-advance retries, and thrown/aborted merges. Cleanup canonicalizes the path, attempts `git worktree remove --force`, then uses a bounded errno-based filesystem retry (`EBUSY`, permission, descriptor, and non-empty failures); on Windows permission failures it clears read-only attributes before retrying. It always retains its existing `git worktree prune` follow-up: prune reclaims a registered-but-missing path, while a residually present path deliberately retains its registration so the next reaper can target it. Present-but-unregistered leftovers are removed through the same filesystem retry. Cleanup emits `merge:ai-worktree-cleanup` audit events for git-remove, fs-rm, and prune phases; benign already-absent/de-registered paths are idempotent success, while residual failures record `success: false`, `attempts`, `residual: true`, and `registrationRetained: true`. The native worktree backend shares that retry fallback only after its established recoverable Git-error classification; a stale `is not a working tree` registration still rethrows without filesystem fallback or prune.
  - **AI review blocks are terminal by type:** `AiMergeBlockedError` is parked as `status:"failed"` before direct-merge recovery examines error text. Reviewer findings can naturally use words such as “conflict”; they must never enter git-conflict retry, bounce, or cooldown admission. The clean-room reviewer carries a current, normalized contract of at most eight unresolved squash-fidelity findings. Its four protocol markers (`REVIEW_VERDICT:`, `SEVERITY:`, `RESOLVED_PRIOR_FINDINGS:`, and `PRIOR_FINDING_DISPOSITIONS:`), plus disposition body lines, are never reasons and cannot enter the durable finding corpus; contiguous reviewer prose is one finding. An approval releases a prior `still-present` finding it does not re-confirm for that candidate, so repeated approval lands or blocks only on a named reviewer objection. The latest bounded rejection log is the crash-recovery source and contracts older than seven days expire deterministically. Blocking review is limited to branch preservation, squash collateral, and conflict soundness; whole-tree semantic concerns outside the squash footprint are advisory/follow-up material because the merger is not authorized to edit them.
  - Worktrees-dir sweeps that list direct children of `<worktreesDir>` (orphan cleanup/reap, self-healing unregistered-orphan reap, and cap enforcement) must exclude the `.ai-merge` container by name; those one-level sweeps never inspect or recycle clean rooms beneath it. Batch 1 scans every resolver-supported clean-room root for stale AI merge worktrees; candidates are bounded to names starting with `fusion-ai-merge-`. `runAiMerge` registers each live clean-room worktree in `activeSessionRegistry` with kind `ai-merge` as soon as the directory exists and keeps both raw and canonical paths registered for the duration of the merge, so the dedicated periodic sweep and pre-merge prune defer when either path is active (including concurrent same-task merge attempts). The default age gate is 2 hours; task-aware cleanup uses a 10-minute grace period for workflow-Complete tasks and for genuinely missing/deleted task rows, and every removal path is clamped by the same 10-minute minimum-age floor so a freshly created worktree is never reaped. Transient `getTask` lookup failures (for example PostgreSQL availability/transaction errors) are not treated as deletion evidence; they log a warning, emit `lookup-error` only if eventually removed, and retain the conservative 2-hour gate. The sweep canonicalizes paths before checking `activeSessionRegistry`, attempts `git worktree remove --force <path>` before bounded errno-based filesystem retry/removal (including Windows read-only clearing), runs its existing `git worktree prune` after cleanup attempts, and emits `worktree:tempdir-sweep` run-audit telemetry for removal attempts and failures including `attempts`, `residual`, and `registrationRetained` for an undeletable path. Fresh directories, active-session paths, and individual removal failures are skipped/logged without aborting the maintenance cycle.
  ...
  - **Operator cancellation is terminal for its in-flight graph run (FN-249).** Reset, a user move out of an active lane, and other `userCanceledTaskIds` entries stop traversal at the current node: the run enters no later node or merge seam, writes no terminal workflow-step result, discards its own matching pending lease, does not enter bounded merge retry, and does not park an operator-action failure. A required gate that was not recorded remains fail-closed as missing; engine-initiated pause/abort recovery is unchanged. The durable result and lease writer recomputes its field-bounded patch from the row read inside a PostgreSQL transaction that holds the same per-task advisory lock as `resetTaskPublicationImpl`, and applies an exact `startedAt` attempt check there. This is necessary because abort is fire-and-forget and Reset publishes after slow Git cleanup: neither `withPlanningLifecycleLock` (which Reset never takes) nor `withTaskLock`/`updateTaskAtomic` (an in-process promise chain) orders against that transaction. A late write waits for Reset, observes the fresh row, and refuses instead of restoring workflow evidence or `approvedPlanFingerprint`.
  - FN-7863 adds a progress-anchored guard at the execute-node self-requeue funnel (`failedNode === "execute"` and either the live row is `todo` or the in-process self-requeue marker proves a stale `in-progress` read). The guard persists `executeRequeueLoopCount` plus `executeRequeueLoopSignature`, warns at `EXECUTE_REQUEUE_LOOP_VISIBLE_THRESHOLD = 3`, and terminalizes at `MAX_EXECUTE_REQUEUE_LOOP_CYCLES = 6` with `status:"failed"` and an `EXECUTION_DISPATCH_LOOP_EXHAUSTED:` error. FN-7941 hardens the signature against residual human-reported `execute_loop_stall` cases (#2043/#2045/#2046/#2047): the signature now stores terminal-step high-water progress (`done`/`skipped` count plus total planned steps) instead of raw `currentStep` + every step status, so `pending`/`in-progress` drift or `done`↔`in-progress` oscillation cannot reset the counter forever. Only monotonic terminal-step progress resets the streak; decreases/oscillation below the high-water keep counting. The guard preserves worktree/branch/step progress, skips paused/user-paused/workflow-Complete terminalization, still resets on manual retry, successful completion, forward moves, and unpause, and complements rather than replaces the scheduler's wall-clock `dispatchStormCount` guard (fast flapping stays scheduler-owned; slow no-progress flapping is caught here).
...
#### Stuck-session recovery contract
Executor-phase stuck sessions have no kill budget or terminal exhaustion path. `StuckTaskDetector` disposes the silent session, releases old runtime ownership, and re-dispatches the same task after unwind while preserving column, workflow node, current step, worktree, branch, and progress. Repeated silence repeats this recovery; it never writes a failed status, an approval hold, or a decompose/rescope instruction. User-paused tasks remain untouched.

Each implementation attempt owns an opaque execution lease rather than the task ID alone. Persistent unwind mutations and task-keyed cleanup validate that lease inside a per-task FIFO critical section which remains held across every awaited store operation. Forced recovery first reserves its FIFO position, then synchronously signals abort so an already-entered owner can unwind; task-keyed abort claims, asynchronous settlement, child cleanup, user-control/deletion rechecks, and lease invalidation all finish inside that reservation before dispatching a successor. If an old mutation or destructive step-session cleanup entered first, recovery waits for it to settle; if invalidation reserved first, the old callback and Git cleanup are never invoked. A stale release is a no-op and cannot clear a successor's maps, worktree ownership, run context, semaphore bookkeeping, or execution claim.

The execution critical section is non-reentrant. Its callback may call asynchronous `TaskStore` APIs, but a store transaction, advisory lock, or callback already holding store ownership must never acquire the execution section; the only lock order is execution section → store API. Timeout and cleanup recovery remain in the task's current lifecycle role, so this path emits no WIP→Hold move and reports successful forced resume only after invalidation is observable in process state.

Planning-phase stuck handling remains a separate planning-session policy and still uses `stuckKillCount` / `settings.maxStuckKills` before execution starts. While under budget, a stuck triage requeue resumes from a non-empty on-disk `PROMPT.md` draft in revision mode and logs resume feedback; if `PROMPT.md` is absent, it falls back to a non-empty `plan` task document.
...
If loop recovery times out during compact-and-resume and the executor does not unwind within the bounded force-requeue grace window, `TaskExecutor.markStuckAborted()` now hard-cancels the hung task before clearing execution guards: spawned child agents are terminated, `awaitAbortInFlightTaskWork()` reaps API/step/workflow/configured-command/subagent/CLI surfaces, completed/in-progress steps are reconciled against committed branch state before any checkout deletion, the task worktree is removed with `RemovalReason.ExecutorStuckKilled`, stale in-memory worktree/loop/paused/stuck state is cleared, and then the task is moved back to `todo` with the configured `preserveProgressOnStuckRequeue` semantics. With preserve-progress enabled, committed step progress is retained; when the branch has no unique commits, affected steps are reset to `pending` before the worktree/branch are cleared so a retry cannot skip deleted uncommitted-only work. The path preserves the concurrent-recovery guard: if the latest task column is no longer `in-progress`, it only clears the execution guard and does not reap/remove resources that a self-healing recovery now owns. Task logs distinguish loop detection, compaction timeout, force-kill cleanup start, force-requeue, and cleanup completion/failure.
  ...
  - `reconcileTaskWorktreeMetadata()` (FN-4962/FN-213) reconciles stale `task.worktree`/`task.branch` rows against authoritative `git worktree list --porcelain` branch mappings during startup recovery, periodic maintenance, and completion fan-out. It runs immediately after self-owned branch reclaim and before `reclaim-stale-active-branches`: if `git worktree move` succeeded but the reclaim persist failed, the registry rebinds the row to the live relocated checkout with a worktree-only patch and emits `task:auto-recover-worktree-metadata-rebound`. Stale rows with no live branch mapping are nulled (`worktree=null`, `branch=null`, `baseCommitSha` unchanged) and emit `task:auto-recover-worktree-metadata-cleared`; active WIP/review rows with no mapping remain protected from pointer clearing.
...
Guardrails: this routine does **not** retry merges, does **not** apply to mixed/non-orphan staging, and does **not** run when no landed-work proof exists (FN-4280 class protection).
  ...
  - `reclaimSelfOwnedBranchConflicts()` includes paused `branch-conflict-unrecoverable` tasks (not just todo/in-progress), clearing paused/error state in one successful reclaim update and applying only the contained backward move for the live lane. Only a genuine `BranchConflictError` verdict may enter `branch-conflict-unrecoverable` recovery; validation, store, Git, and filesystem failures defer to the next sweep without changing lifecycle fields. A checkout already relocated before a failed persist is best-effort re-pointed and, if that write also fails, converges through the immediately following registry-backed metadata reconcile instead of being discarded.
...
- Terminal workflow-graph diagnostics copy the last visited node's own normalized exception into both the task log and `task.error`, alongside the node id and routing value. Lookup is restricted to that node's direct or materialized optional-group/foreach context keys so stale errors from earlier nodes cannot be attributed to the failure; copied text is capped at 1,000 characters, while absent or blank exceptions retain the generic failure message.
...
- **Board column ordering (board view only, FN-509)**: every column uses one non-configurable order — there is no per-column sort menu. A **manual-intake** lane (`intake` with `autoTriage: false`, such as Coding (Ideas)'s "Ideas") shows newest first. A **Complete** lane shows most-recent ARRIVAL first (`columnMovedAt`, then `updatedAt`, then `createdAt`) and is served in bounded pages with an exact total; it is deliberately NOT creation-ordered. Every other lane shows genuinely ACTIVE cards first (ordered among themselves by arrival, never by Boost), then the whole waiting queue in the shared Boost → arrival → task-id order, which is exactly the order admission will try them in.
...
- FN-509 removed the `task_refine` ordering preference in triage polling (FN-4647 pattern). Starvation under a large planning backlog is now structurally impossible for an old card: the queue is arrival-ordered, so the oldest refinement is already ahead of everything created after it. An operator who needs a specific card sooner uses Boost.
- **Starved refinement self-healing sweep (Lane B):** FN-509 removed this sweep's remediation. Its only action was a bounded one-step priority nudge, and priority no longer exists; an automatic rank write would be precisely the hidden priority FN-509 removes. `SelfHealingManager.recoverStarvedRefinementTriageTasks()` is still registered in the startup and maintenance sweeps because it also evicts stale triage processing markers, which is an independent recovery. `task:auto-recover-starved-refinement` is historical: persisted rows stay readable, but no live engine path emits it.
- **Stale planning liveness invariant:** stale-processing eviction never removes a task with a live, non-aborted triage session (`activeSessions.has(id) && !stuckAborted.has(id)`). It therefore remains in `getProcessingTaskIds()` and cannot be finalized or cleared for replanning by planning recovery sweeps. Hung promises without a session and stuck-aborted/disposed sessions remain reclaimable after the stale threshold.
- Approval resolution comes only from workflow/project policy: workflow `requirePlanApproval=true` under workflow mode or project `planApprovalMode="require-all"` stops the task at `status: "awaiting-approval"`; project `planApprovalMode="auto-approve-all"` bypasses the manual gate.
...
- **Database / `task:reconcile-dependency-blocking-lease`** — emitted by `reconcileDependencyBlockingLeases()` (FN-6292/FN-217) when self-healing records a dependency/lease wait in place. Metadata includes the dependency ID, blocked-by marker, and unmet dependency list.
- **Database / `task:reconcile-in-review-unmet-dependencies`** — emitted by `reconcileInReviewUnmetDependencies()` (FN-6793/FN-6797/FN-217) when self-healing records unmet dependencies on an `in-review` task without moving it backward. Metadata includes `unmetDeps`, `blockedBy`, and prior review status; the `-no-action` companion covers pause/user-pause, `autoMerge:false`, and live execution/checkout proof.
...
- **Database / recovery no-action family (FN-5335/FN-217)** — retained recovery checks emit annotation-only events when their evidence fails instead of mutating lifecycle state. Active mutation types include `task:reclaim-pr-conflict-no-action`, `task:reclaim-self-owned-branch-conflict-no-action`, `task:auto-rebound-scope-decay-no-action`, `task:finalize-no-op-review-no-action`, `task:stuck-merge-deadlock-no-action`, `task:missing-worktree-review-no-action`, `task:partial-progress-no-task-done-no-action`, and `task:reconcile-dependency-blocking-lease-no-action`. Ghost-review, stale-incomplete-review, limbo, generic terminal-failure, and zero-progress no-task-done events were removed with their sweeps.
...
For scheduler concurrency diagnostics, the queued reason names each active limiter and its own usage (for example `gate=maxConcurrent ...` or `gate=maxWorktrees ...`). It includes per-gate `{ used, limit, slack }`, holder IDs, and computed availability for a worktree-consuming dispatch; the worktree line is omitted entirely when that dimension is disabled. The authoritative read is the live project settings blob; only an unavailable project store may fall back to a registry snapshot, which then falls back to shipped defaults through the same resolver. `maxConcurrent` is the per-project cap on enriched live top-level planning, execution, and review/merge agents; `maxWorktrees` is enforced at the serialized scheduler admission boundary over WIP reservations and retained execution checkouts; the host semaphore is the separate process-global pool. Each newly free project slot admits review/merge first, ready execution second, and planning last; a worktree-blocked execute candidate is skipped so a checkout-free planner behind it can use agent capacity. These queued-reason logs are transition-only: a newly emitted line indicates the limiter signature changed or the condition cleared and later reappeared, not that a poll loop simply observed the same blocked state again.
...
- PostgreSQL cold-storage tables and legacy `archive.db` files remain migration and forensic inputs only.
- Store open pages through historical snapshots and reintegrates each non-deleted task into its selected workflow's Complete column, falling back to `done` when workflow metadata is unavailable.
- The historical `archived` literal may appear only as a persistence sentinel during reintegration or on internal soft-delete snapshots. It is not a workflow trait, board column, transition target, route, tool, or CLI operation.
...
- Built-in columns: `triage` (Planning), `todo`, `in-progress`, `in-review`, and `done`; custom workflows may rename role-bearing columns.
...
- **Merge phase**: `runAiMerge()` (`merge/merger-ai.ts`) is the production merge path; the soft-deprecated legacy `aiMergeTask()` body retains historical strategy and post-merge workflow steps.
...
**Execution-entry invariant: unplanned/intake-resident cards never enter a processing column (FN-7648).** The hold/release sweep (`runHoldReleaseSweep` in `packages/engine/src/execution/hold-release.ts`, driven from `Scheduler.runHoldReleaseSweepPass`) releases a `hold`-trait card into the nearest downstream `countsTowardWip` column. A card must never be released while it is still unplanned — `status === "planning"`, a pending pre-release Plan Review, a durable replan signal, a duplicate redirect, or a bootstrap-seed PROMPT.md. The automatic sweep first classifies an unplanned or approval-held card as a non-candidate and reports `awaiting-planning:<reason>` or `awaiting-approval`; it does not attempt a release and therefore writes no durable refusal. `issueRelease` remains the defence-in-depth choke point for the sweep, `promoteHeldTask`, and `releaseHeldTaskByEvent`: an explicit unplanned request records one durable refusal per episode, and the scheduler's `reserveSlot` callback independently applies the same guard before reserving a worktree/semaphore slot. FN-245 removes every operator waiver, so unplanned and approval-held cards are refused on every release surface, including explicit promote requests, until planning and approval genuinely complete. The shared predicate is trait-based rather than keyed to a literal column id, so renamed intake and hold columns receive the same protection. Held cards keep `status` and `blockedBy` untouched and never leak a reservation.

**Event-driven capacity admission (FN-242).** Poll timers remain backstops, not the primary dispatch mechanism. A `task:moved` event that leaves the resolved `countsTowardWip` membership wakes scheduling immediately, and completion of every admitted planning promise wakes the debounced planning lane. Each producer also nudges its sibling lane because planning and implementation share the project admission budget. These wakes only request another admission pass; pause, dependency, workflow, approval, capacity, and `ProjectAdmissionCoordinator.admitNext` gates remain authoritative.

**Stranded pre-release Plan Review continuation (FN-8592).** Startup and periodic self-healing detect a hold-trait card with a real non-seed PROMPT.md, a pre-release `plan-review` node in its own column, no passed plan-review result, and no active continuation of any kind. Soft-deleted and historical-sentinel rows are explicitly excluded. Candidate guards preserve pauses, live sessions, fresh moves, global/engine pause, and `autoMerge:false`; non-candidates are silent to avoid flooding audit. The repair inserts a plan-review continuation only when idle and never force-promotes or retires existing work. PostgreSQL serializes this decision with `pg_advisory_xact_lock(hashtext(project_id), hashtext(task_id))`: every ACTIVE work-item writer and terminal `plan-review`-passed writer takes this first transaction lock, so a competing writer either commits before the repair (which observes it) or waits for the repair commit. The conditional store operation checks all active kinds and the passed result under that lock, inserts only when idle, and never uses `replaceActiveTaskWorkflowContinuation`; a static ratchet guards the protected writer set without a migration.
...
**Exclusive planning ownership and Plan Review output (FN-299).** Planning is a first-class live owner even though it has no worktree session path and holds no executor lock. `isTaskPlanningOrExecutionLive` is the shared admission predicate for the three continuation-recovery sweeps and the hold-release diagnostic; it names active paths, executor lock, executor activity, triage's planning-task set, and the process-wide fail-closed planning probe. A live planning signal yields `live-planning`, so self-healing cannot reclaim that continuation. Triage writes the `running` `plan` continuation with a future ten-minute lease and a unique owner per attempt, renews it every one-third lease interval under owner/state compare-and-set, and stops the unreferenced timer before terminalizing under the same owner/state fence. A stale callback therefore cannot seize or complete a successor that is also `running`. As an independent dispatch fence, the continuation drain maps the same ownership to `planner-live` and defers the observed state for 15 seconds without dispatching, cancelling, or terminalizing it. Because capacity admission awaits mutable state, the final launch takes the planning lifecycle lock, rechecks planner liveness, rereads the exact durable task and continuation snapshot, and atomically changes that row to `running` under a distinct dispatch owner before launching the graph. Triage installs ownership under the same lock and refuses to replace that durable dispatch claim, so the winner remains visible after lock release instead of mutual exclusion ordering two eventual launches. The existing stale-processing eviction removes wedged in-memory ownership, after which an expired durable lease remains recoverable through `dead-lease`.

A `plan-review` verdict has exactly three integrated routes: `success` reaches the execution successor, `failure` reaches `plan-replan`, and `outcome:close-no-op` reaches terminal `plan-review-no-op`. A required response with no verdict is failed non-plan evidence and holds before route selection. A scheduled replan stops traversal; a declined replan persists `remediation-not-scheduled`; neither path can also reach execution. The APPROVE chain is: (1) `evaluateUnplannedForExecution` accepts a non-superseded satisfying result, (2) the scheduler and in-process runtime observe `task:updated` under `!task.status && !task.paused && !task.userPaused`, then use one of three disjuncts — removal from the approval-held set, `approvedPlanFingerprint`, or `isPlanReviewSatisfied` — and (3) schedule immediately or clear a planning continuation's `retryAfter` under compare-and-set before kicking the drain. The REVISE chain is: (1) `requestPreMergeOptionalStepFix` records the decision and writes `needs-replan`, (2) `isTaskAwaitingPlanning` re-admits that status even with a complete prompt, and (3) real `specifyTask` reads the workflow result's `notes ?? output`, not the deliberately excluded revision-log preview, and combines it with the rejected draft and cumulative decision ledger. Because queue admission requires a null task status, a current `needs-replan` wins over any earlier approval and the two output chains cannot arm together.

### Step inversion: steps as workflow-modelable nodes (`experimentalFeatures.workflowGraphExecutor`)

The columns/traits track moved *board* policy (transitions, capacity, hold, merge orchestration) onto the substrate/policy line. The **step-inversion** track extends the same inversion to *task steps* and to the *task shape itself*, riding the existing `workflowGraphExecutor` flag (orthogonal to `workflowColumns`). With the flag off — and for the default coding workflow always — step policy stays exactly as it is today (the monolithic `execute` seam, PROMPT.md `### Step N:` parsing with contiguous 0-based indices, graph-owned review-node verdicts). The default workflow is the byte-identical parity oracle; inversion is opt-in via custom workflows and a built-in stepwise coding workflow.
...
When a tracked task transitions into its workflow's Complete column, Fusion closes the linked GitHub issue with `state_reason: completed`. When it leaves Complete for an active workflow column, Fusion reopens the issue with `state_reason: reopened`. When a tracked task is permanently deleted, Fusion closes the linked GitHub issue with `state_reason: not_planned` (or deletes it when explicitly requested). Delete-path outcomes emit a `github-issue:action` store event payload (`{ taskId, action, owner, repo, number, outcome, error? }`) so success/failure remains observable even after the task row is gone and task activity logs are unwritable. Tasks without `githubTracking.enabled` or without a linked issue are unaffected, and GitHub failures are logged to task activity without blocking the move.

The GitHub tracking state listener now attaches to every registered project store (including projects registered after startup), and each store gets a one-time asynchronous startup reconciliation sweep. That sweep scans bounded done tasks with tracking enabled and closes any linked GitHub issue still open, so missed/momentary failures are caught up without blocking server boot. Its separate deleted-task pass never writes task activity: selected rows are read-only, and unchanged degraded diagnostics are emitted only once per store signature so idle sweeps remain quiet. Source-imported GitHub issues can also be auto-closed when `githubCloseSourceIssueOnDone === true`: `GitHubSourceIssueCloseService` listens for `task:moved` transitions into `done` and closes open `task.sourceIssue` links, while `GitHubTrackingReconciler.reconcileSourceIssues` performs a parallel startup sweep over done tasks with GitHub source metadata to close any source issues still open.

### Worktree model
- Each active task runs in an isolated, task-ID-named worktree under `.worktrees/<lowercased-task-id>`.
- Executor creates branches like `fusion/{task-id}` (`executor.ts`).
- The task ID is the sole native worktree naming contract. A retry, restart, or resume reuses or safely recreates that task's deterministic checkout; Fusion does not maintain a cross-task recycle pool or alternate random/title naming modes.
- Workspace tasks acquire a task-ID-named checkout for every configured repository when the task begins. The task-owned workspace directory, not a manual repository choice, is the only implementation and review context.

#### WorktreeBackend abstraction
- Backend contract: `WorktreeBackend` (`packages/engine/src/worktree-backend.ts`).
...
- Self-healing also runs `reclaimSelfOwnedBranchConflicts()` across eligible idle and paused-conflict tasks; successful reclaim keeps stranded commits intact. Only a genuine `BranchConflictError` verdict escalates through `branch-conflict-unrecoverable`; provenance validation, database, Git, and filesystem errors leave lifecycle fields untouched and retry on the next sweep. If the checkout was physically relocated before persistence failed, a worktree-only re-point is attempted and the immediately following registry-backed metadata reconcile provides the convergence backstop.
...
- `runAiMerge()` in `merge/merger-ai.ts` performs production merge flow; soft-deprecated legacy `aiMergeTask()` remains retained in `merger.ts`.
...
- The same no-commits guard is the **branch-missing escape hatch** (RUFU-014) in `runAiMerge`: when a `noCommitsExpected === true` task was executed (`baseCommitSha` set) but its git branch ref is absent and not already merged, merger routes through `evaluateNoCommitsNoOpFinalize` instead of throwing `"work appears lost"`. All-done no-commits tasks finalize as a no-op (git run-audit `merge:ai-no-branch` with `kind: "no-commits-expected"`, `finalizeTask(noOpResult(...))` → `done`); incomplete/skipped tasks demote to `todo` with `preserveProgress: true` (lane `"no-commits-branch-missing"`). Commit-expected executed missing-branch tasks still throw `"work appears lost"` unchanged.
...
- In soft-deprecated legacy `aiMergeTask` cleanup, `restoreUnrelatedRootDirChanges()` attempts restore; then `dropAutostashHandle()` runs on every terminal path and drops primary + race-rescue stashes when restoration succeeded or content is no longer live.
...
- **In-review stall deadlock**: identical stalls (same code + reason) within one unchanged episode repeated past `inReviewStallDeadlockThreshold` (default 10) auto-pause with `pausedReason: "in-review-stall-deadlock"` and `status: "failed"`. A newer valid `startedAt` or `completedAt` on the active top-level failed pre-merge result proves correction progress and starts a new episode, even when the blocker text is unchanged. Observation, progress check, log append, and disposition are recalculated and committed under the same project-scoped task advisory transaction lock as workflow-result writers, so a stale sweep cannot overwrite a concurrent correction. User-initiated retry paths (dashboard retry, `fn_task_retry`, and CLI `task retry`) clear that automatic deadlock pause so the retry can execute, but they never override explicit/manual pauses or unrelated automatic pause reasons.
...
- **Task title/ID drift (FN-4898)**: active title writes and historical reintegration normalize foreign embedded `FN-NNN` tokens via `packages/core/src/task-title-id-drift.ts`. Empty placeholder groups (`()`, `[]`, `{}`) left behind by token stripping are also removed in both `normalizeTitleForTaskId` and `sanitizeTitle` (FN-4978). Lineage is preserved in `sourceParentTaskId` / description markers, not title embeds. FN-5077 extends drift normalization to reject dangling-connector fragments (`"Close as duplicate of"`) so token-stripped residuals never persist as task titles.
...
- **Orphaned execution sweep is observation-only (FN-5337/FN-217)**: `recoverOrphanedExecutions` only annotates stale in-progress candidates with `task:orphan-detected-no-action` and `[orphan-detected] ... no action (operator-decides)` logs. It must never move `in-progress`/`in-review` backward or mutate lease/worktree metadata. WIP liveness recovery belongs to the in-place stuck-session detector.
...
- **No-progress churn recovery (FN-5168/FN-217)**: `StuckTaskDetector` tracks ignored `fn_task_update` rebuffs via `recordIgnoredStepUpdate(taskId)` and may classify repeated low-novelty activity as `no-progress-churn` after compact-and-resume. That classification now uses the same in-place session disposal and re-dispatch as inactivity; it has no terminal status, audit event, retry budget, or human hold.
...
- **Cross-process soft-delete boundary (FN-8683)**: process-local `TaskStore` events are bridged by the PostgreSQL transactional outbox. `TaskDeletedOutboxConsumer` redelivers committed events with `meta.observed === true` and an `outboxEventId`; consumers must remain at-least-once safe and must not repeat writer-owned delete audit or mailbox side effects.
- **Soft-delete audit + column reconcile (FN-5175)**: `TaskStore.deleteTask` records a `runAuditEvents` row (`mutationType: "task:deleted"`, `domain: "database"`) inside the same transaction that sets `deletedAt`, and sets `"column" = 'archived'` on the row. Callers without a heartbeat run context (`fn task delete`, pi extension, dashboard delete route) pass an `auditContext` with `agentId: "system"` and a synthetic `runId`. The watcher cross-instance emit path does NOT re-record the audit event. The row stays in `tasks` rather than cold storage; no task archive write path exists.
...
- **Deleted-task worktree convergence (FN-309)**: `SelfHealingManager` subscribes to `task:deleted` before the engine starts `TaskDeletedOutboxConsumer`, schedules Git cleanup outside the delete/outbox transaction, and replays the same reconciler at startup and during maintenance. It reads only persisted `task.worktree` and `task.workspaceWorktrees` targets, deduplicates canonical aliases, and removes a modern workspace coordinator only after every child converges and the directory is empty. Explicit deletion may force-discard dirty content from a Git-registered, uniquely task-owned Fusion checkout, but repository roots, configured-root escapes, foreign or ambiguous claims, active executor/planner/workflow/merge sessions, and unreadable ownership evidence fail closed. Failed but non-deleted workspace tasks retain the ordinary one-day floor and landing/branch-preservation rules.
...
**Unified squash gates (FN-9050).** `landOneRepo` validates every approved clean-room squash before advancing an integration ref, for both single-repository lands and each workspace sub-repository land. Workspace checks use that repository's File Scope subset; a violation leaves the integration ref unchanged. (The former diff-volume shrinkage guard and its `merge:diff-volume-blocked` audit event were removed by operator decision on 2026-08-16 — it blocked approved squashes.)
...
An accepted `PROMPT.md` is preserved as an immutable, project-scoped spec-lock. The canonical current-plan evidence parser covers `Mission`, `File Scope`, `Steps`, `Completion/Acceptance Criteria`, `Do NOT/Non-goals`, `Dependencies`, and lineage headings. It scans planner-authored structure only: hygiene declares and escapes marker literals in the verbatim Original Description body, then the shared bounded resolver removes that region for both spec-lock and approval-fingerprint normalization. This keeps operator prose—including quoted task-file markers—from creating structural duplicates while readers remain deterministic functions of the prompt. `Mission` is a whitespace-normalized structural hash, never semantic interpretation: missing, empty, or duplicate Mission sections make comparison `unavailable` with a fixed reason. Equivalent accepted spellings of one section—`Completion Criteria`/`Acceptance Criteria`, `Do NOT`/`Non-goals`, `Mission Lineage`/`Parent-child Lineage`—combine into a single canonical body under that section's list semantics, so a plan that writes both spellings stays lockable instead of being refused as a duplicated section. Only a level-2 heading outside a balanced code fence declares a section: backtick or tilde fence lines with up to three leading spaces open a fence that closes at the next line of at least as many of the same fence character, and a heading inside that region is content of the section enclosing it, so an example block can neither duplicate a section nor truncate an enclosing body. A fence left open at end of file falls back to the previous, fence-blind declaration set, so the reader can never invent an `unavailable` reason the earlier scan would not have produced. Repeating the exact same heading outside fences is still refused as `section-duplicate` (`mission-duplicate` for `Mission`), because that boundary is genuinely ambiguous. `SPEC_LOCK_PARSER_VERSION` 2 identifies this scan, so retained evidence stays distinguishable from parser version 1.

Each authoritative prompt write reaches `PROMPT.md` before its task-row approval invalidation, then serializes evidence capture and reconciliation under the planning lifecycle lock. Current-plan evidence structurally binds the live task dependency set plus mission, slice, and parent-task lineage IDs as well as the prompt sections, so a dependency or lineage mutation appends comparable evidence even when `PROMPT.md` bytes are unchanged. Manual approval and a successful workflow-graph Plan Review create/reuse the lock before publishing acceptance evidence consumed by scheduling. If a Plan Review cannot lock unavailable evidence, its result is immediately persisted as an actionable failed, merge-blocking gate rather than silently remaining pending. Planning records deterministic unavailable-parser evidence by source hash and does not spend repeated retries on byte-identical prompt text; a changed prompt remains eligible for bounded recovery. If evidence persistence is interrupted after the file/row write, the inactive task remains safely unreleased; startup or event reconciliation re-reads `PROMPT.md`, appends the missing comparable revision, and reports drift. Re-approval reuses identical content or appends a lock with a prior-version diff; a clean later lock after an earlier retained divergence reports `diverged-relocked-approved` rather than losing that history. Drift reports compare the latest lock with current evidence and execution file paths, producing only machine-observable `scope-creep`, `silent-expansion`, or `plan-deviation` findings. Added File Scope boundaries, dependencies, lineage, durable steps, and acceptance criteria are silent expansion; reordered steps and changed Mission hashes are plan deviation. Reports are retained independently of live task rows, so soft deletion and historical reintegration retain operator history.

Parent delete with `removeLineageReferences:true` uses the same contract. It enters the invalidation boundary based on that flag even when its pre-read set is empty: the empty fast path skips prompt reads, probing, and locks but still revalidates inside the transaction, so a raced-in child aborts and retries once under its planning lock. Historical sentinel rows remain excluded from live lineage candidates.

Each attempt orders candidates → prompt map → structural transport probe → child locks → transaction → outcome → reconciliation. The transaction clears each affected child's parent binding and approved fingerprint, then resolves evidence from the pre-read prompt before committing. Evidence conflicts are classified by the matching `source_hash` row at any version (never only the latest row), and a reused row is accepted only after its canonical lineage is verified not to name the removed parent; an unresolvable or untruthful append raises `LineageEvidenceAppendError` and rolls the deletion back. The only evidence-less commit is a child without a readable `PROMPT.md`; append-only history remains, so its report fences pre-existing, possibly stale evidence but cannot claim approved alignment after the fingerprint clear. A session-scoped multi-key advisory lock covers every affected child, acquired in sorted order and nested under the parent task lock. Its callback publishes post-commit drift reconciliation before releasing those locks; lock acquisition failure occurs before the delete transaction, so no child evidence is partially published. Candidate revalidation is one retry only, then raises `LineageInvalidationCandidateRaceError`; no-clear gate rejection or lost delete claim publishes nothing. Structural pooler-only deployments may degrade serialization only after a pre-flight probe, but lock contention never degrades into an unlocked clear. Both channels reconcile through lock-free `reconcileSpecDriftWhilePlanningLocked`; report publication is the sole best-effort part. The transaction's resolved evidence version can differ from the report's `evaluateSpecDrift`-fenced latest evidence version; cleared approval, rather than evidence history removal, makes the report truthful.
...
### AI merge review reconciliation

AI merge review findings are durable task-owned reconciliation state, not task-log prose. The state binds the source head, integration tip, candidate commit/tree, exact findings, confirmation count, and corrective-pass budget. A changed source or integration tip clears all of those values together; interruption of the same identity preserves them. A blocking finding can consume a corrective pass only when its exact ID/text is supplied to the merger.

An approval remains an approval. A clean approval is reviewed a second time against the identical candidate without re-running the merger or spending budget; two consecutive clean approvals land it. Unconfirmed prior findings are displayed with the approval rather than being rewritten as a rejection. Explicit `still-present` findings remain blocking, while advisory concerns remain landable. Operator dismissal requires a nonblank reason, is audit logged, and clears the candidate/state/budget together so the next merge starts clean.
...
### Independent task and worktree concurrency

Task admission resolves two independent dimensions from the live project-scoped settings blob (`TaskStore.getSettings()` or `getSettingsFast()`). `maxConcurrent` bounds all AI-active top-level tasks, including planning. Optional `maxWorktrees` bounds tasks holding or entering execution checkouts; disabled worktree capacity is structurally absent. The shared core resolver returns `{ maxConcurrent, worktreeLimit }` without deriving an effective minimum. Registry snapshots are fallback-only and resolve through the same defaults. Queued reasons identify the exhausted gate directly, while reporting APIs expose `maxConcurrent`, `maxWorktrees`, and `worktreeLimitEnabled`.
...
A Code Review diff probe distinguishes populated, provably empty, and unavailable input; unavailable evidence remains fail-closed. A task with durable `noCommitsExpected: true` and a provably empty singular diff receives a deterministic passed Code Review without a model dispatch. Other empty-input rejections first reach the remediation producer: a Code Review or Verification gate that can name fix work returns the card to WIP with that work, because no `REVISE` may block a card merely because usable fix steps were absent. The terminal close is the fallback for a gate with no producer, or one whose producer declined, and for those it still runs before all revision-budget guards, including the recovery helper, graph-failure router, and self-healing candidate filter. The filter carve-out is limited to the canonical empty-input rejection and is self-limiting because the resulting failed status excludes the next sweep.

The terminal write compare-and-sets the exact failed Code Review attempt under the task lock. It declines when the attempt was approved, bypassed, superseded, archived, replaced, or removed, and when the task was paused, deleted, held, terminal, or already classified. A plain re-read is insufficient because both the live graph node and periodic recovery sweep race those writers. The park is written in the current lane; after the graph unwinds, `handleGraphFailure` honors it before every resume or remediation classifier and settles it forward from the Code Review remediation WIP lane into the workflow review lane. The move preserves progress, worktree, and status, identifies direct review entry explicitly, fails soft on transition refusal, and re-asserts the park afterward so a terminal card never permanently consumes a `maxConcurrent` slot.

An AI empty-merge finalization refusal now persists a hard blocking failed status with its existing precise error. Since `merge-failure-rebound` has no backward-move authority and merge admission does not consult `task.error`, an error-only write left the card eligible for the same merge attempt forever. The failed status closes that door while leaving the card in the capacity-free review lane; operator Retry is the existing recovery path after its evidence is corrected.

### Review-gated remediation safety

The review-gated coding workflow preserves implementation steps across review rejection. Its replay authority is appended remediation provenance, not step-name matching. A Code Review `REVISE` produces that remediation before any review-to-WIP move: malformed or out-of-scope reviewer findings receive one deterministic executor Fix step rather than leaving the card blocked in review. Self-healing claims keyable review episodes before budget consumption and narration, fencing the claim by step id, input signature, and owner so concurrent or stale runners cannot duplicate or erase a newer remediation attempt. Zero-diff finalization recognizes legacy verification names, structurally marked remediation steps, and required gate results so a missing or failed Verification cannot be finalized as a no-op.

## External task blocks

`classifyExternalObstacle` is the sole authority that may persist `status: "blocked"` with structured `externalBlock` metadata. Only conservative host-resource, network, model-provider, and credential classifications freeze a card. The freeze retains the column, steps, current step, worktree, branch, concurrency/worktree capacity, and its file-scope lease as an instance of the general lifetime contract. The durable `external-block` pause fences dispatch and every lifecycle-mutating recovery sweep; it is neither `userPaused` nor failure.
