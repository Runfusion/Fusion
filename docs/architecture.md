...
- **Executor**: `TaskExecutor` (`executor.ts`) implements tasks in worktrees
  ...
  - **Deterministic worktree recovery:** task-ID acquisition holds one path reservation across classification, quarantine reconciliation, and recreation. An absent native target paired with Git's recognized missing or unregistered-worktree response is an idempotent prune-and-success path for forceful and defensive callers, so stale administration clears before the deterministic checkout is recreated. Present or orphaned directories, dirty-worktree protections, live-session fencing, containment refusals, and unrelated Git errors remain protected. Self-healing scans exclude internal `.ai-merge` and recovery containers.
  ...
  <!-- FNXC:WorktreeBaseRefresh 2026-09-19-20:13: Checkout-integrity waits must retain their diagnosis through stepwise graph execution and never compensate a pre-existing Git operation. -->
  - **Unsafe checkout preparation:** An existing rebase, merge, cherry-pick, revert or sequencer operation blocks base refresh before any reset, rebase or compensation. Fusion leaves that operation intact for its owner. A typed refresh refusal survives implementation cleanup and stepwise execution into bounded in-place graph recovery, rather than becoming a generic `step-failed`. Delayed retries recheck task/engine pause, deletion, lane and active-session state before dispatch; budget exhaustion keeps the task in place with the preparation reason logged. Do not delete Git operation metadata merely to force a retry.
...
The bounded auto-merge retry must never repeat a merge-boundary check with unchanged missing proof. A newly observed evidence gap emits the recoverable `merge-boundary-evidence-recovery` value and returns only proven unfinished implementation through named remediation; it cannot request merge until durable progress makes the proof complete. `merge-boundary-unproven` remains a fail-closed terminal token only when recovery cannot prove a safe owner, while self-healing may revisit historic parked rows after re-verifying unfinished work, lifecycle ownership, liveness, and operator policy. `implementation-incomplete` is likewise a preserved structured failed reason, allowing its dedicated resumable/fail-closed graph route to run rather than repeating a merge request. Terminal merge values must survive `classifyMergePrimitiveResult` on both the collapsed synthetic `merge` seam and direct `merge-attempt` runner; do not encode a new terminal value solely as `data.status:"failed"` with an unrecognized reason, because that classifier collapses it to non-terminal `merge-failed`.
...
Each accepted external block carries a bounded four-part operator report: verified condition, stop reason, unimplemented work, and precise unblock condition. The task-wedge notification service recognizes only this live classified freeze, claims one durable episode, and delivers one system-to-user dashboard mailbox message through `sendMessageOnce`. Restarts, duplicate updates, and retries reuse the pending marker and episode CAS; Retry resolves the episode, and a later eligible block follows the established cooldown. Report fields are redacted, control-character/length bounded, and reject prompt, tool-log, and stack-shaped content. Mailbox delivery is timeout-bounded best effort: unavailable, throwing, rejecting, or hanging delivery cannot mutate lifecycle, recovery ownership, leases, review state, or merge-boundary proof.
...
The workflow merge boundary is a safety door and a recovery classifier. Missing node or foreach proof blocks the merge requester before it is called. When the task still has durable unfinished implementation and its resolved lifecycle lanes prove a review-to-WIP remediation target, the graph emits `merge-boundary-evidence-recovery`, logs the named repair, preserves progress/worktree state, and resumes the normal implementation owner. Direct admission and retry-boundary admission use that same path. Legacy unproven rows or states without a proven unfinished owner remain fail-closed; recovery never synthesizes proof or overrides user pauses, external blocks, terminal lanes, or operator auto-merge policy.
