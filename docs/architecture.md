...
- **Merger**: `runAiMerge()` (`merge/merger-ai.ts`) is the sole production path that merges approved work; the retained `aiMergeTask()` body is soft-deprecated and inert.
  ...
  - **Post-landing task-worktree cleanup (FN-251):** for a single-repository task with `mergeDetails.mergeConfirmed === true`, `CompletionLandedCleanup` proof-gates cleanup before a task enters its complete column. `finalizeProvenAutoMergeTask` is the shared enforcement seam for direct AI merge, workflow merge, graph recovery, fast-path, and its self-healing callers; the six direct self-healing complete-lane finalizers use the same helper. `reconcileCompletedTask` remains a post-move convergence backstop for worktrees leaked by older finalization paths, including a branch-to-worktree lookup when the task pointer is already null.
    ...
    - FN-9233 classifies defensive cleanup as `clean`, `regenerable-ignored`, `ignored-only`, or `deliverable`. A checkout containing only ignored directories on the conservative regenerable allowlist (dependency and build names such as `node_modules`, `dist`, `target`, and `__pycache__`) is removable by every defensive reason. Quoted porcelain paths and any non-allowlisted ignored entry fail closed as `ignored-only`: only `CompletionLandedCleanup` with durable `postLandingProof` may remove them. Modified tracked files, untracked non-ignored files, unreadable status probes, and live sessions always remain fail-closed. `worktree:removal-discarded-regenerable-content` records an allowlisted discard with IDs and entry count only; `worktree:post-landing-ignored-content-discarded` records proof-gated ignored-only removal; and `worktree:removal-preserved` now also records ignored-only preservation.
...
Completion fan-out re-reads the dependent, current blocker, workflow roles, settings, and normalized scopes before compare-and-setting a stale `overlapBlockedBy`. After at least one release commits, it requests one coalesced scheduler pass; a request received during an active pass is retained and runs immediately after that pass finishes, without allowing concurrent passes. The scheduler's periodic poll and `clearStaleBlockedBy()` remain convergence backstops rather than the normal release trigger. Global or engine pause still suppresses reconciliation, and stopping the scheduler cancels an unstarted follow-up.
