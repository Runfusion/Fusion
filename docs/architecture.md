...
A post-commit wake makes the graph continuation eligible but does not authorize ordinary work. The executor must claim the pending generation, derive actual landed-file overlap, prove the delivered target is included in the checkout, and persist one of `resume`, `briefing`, or `revalidate`. This strict freshness requirement applies only to recorded wait episodes; the best-effort base refresh for unrelated acquisitions remains unchanged. Receipts and the task log survive restart, and a stale owner cannot publish after Reset, deletion, checkout replacement, or a newer revision.
