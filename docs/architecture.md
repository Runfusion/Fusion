...
The planner's `fn_install_worktree_dependencies` tool executes its command through the engine command/sandbox seam and records only an observed zero exit as installed; a reasoned `none` records an explicit no-install decision for unrecognised evidence. Plan Review is the sole blocking authority: it retries deterministic rows once, returns an ordinary `Dependencies are not installed.` REVISE while any member remains unresolved or unrecognized, and relies on the existing Plan Review budget/cap to escalate to `awaiting-approval` instead of approving a non-ready worktree.
