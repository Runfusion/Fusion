...
- **Board column ordering (board view only)**: `todo` cards mirror scheduler pickup order (priority descending, then `createdAt` ascending/FIFO within each priority tier, then task ID ascending). `triage` and `in-progress` use priority descending then task ID ascending, with missing/invalid priority normalized to `normal`. `done` is completion-recency ordered (`columnMovedAt`, then `updatedAt`, then `createdAt`, newest first) and served in bounded pages with an exact total. In `in-review`, merge-active tasks (`status === "merging"`, `"merging-pr"`, or `"merging-fix"`) are pinned above non-merging tasks, with priority-then-ID ordering within each group.
...
- **Scheduler overlap priority/age guard (FN-5325/FN-254)**: with `groupOverlappingFiles=true`, `findHigherPriorityQueuedOverlap` is the dormant-lease tiebreaker. It defers a lower-priority (or younger same-priority) candidate behind an overlapping retained-worktree holder, preserving priority→age→task-id order without preempting an active lease. Active holders still serialize immediately; the per-pairing audit event was removed in FN-6174 due to zero consumers and table bloat.
