...
### Tablet and desktop navigation footer and right dock
...
The official tablet and desktop design uses one full-width navigation footer instead of ExecutorStatusBar. Its far-left running/max capacity counter opens the existing Max concurrent tasks and Max worktrees controls; direct destinations stay centered, **More** follows them, and **Terminal** sits immediately left of **Settings** at the far right. **More** opens centered above its trigger as a compact single vertical column, with one destination per row and vertical scrolling when the list grows beyond the viewport. Hovering or focusing the trigger opens it; a continuous hover corridor and short close grace let the pointer travel slowly between trigger and menu, while leaving the combined region still closes it and Escape provides explicit keyboard dismissal. History stays available from completed workflow columns instead of appearing in that footer. The right dock body is labelled by its selected tab without repeating a generic visible header above views that already provide their own title. **Files** remains a browser list: selecting any text, image, audio, video, PDF, or binary file opens the shared file editor/preview window, and no editor or save controls replace the list inside the dock. A window opened for that specific file shows only its editor, preview, loading state, or error; it does not repeat the file list, sidebar resize separator, or narrow-layout **Back to file list** action. Opening Files without selecting a file still opens the complete browser with its list and responsive list-to-editor navigation. On a phone, Files is sized by the panel that hosts it rather than by the screen: the file list is the only area that scrolls, it scrolls all the way to its last entry, and the surface never scrolls a second time behind it. Opening a file from that list always shows **Back to file list**, which returns to the list without closing Files. A window opened directly for one file has no list to return to and therefore shows no such return. On every tablet and desktop dock, **Chat** is an inline conversation list: selecting it keeps the list in the dock body, and the dock is never replaced by an expanded Chat window. The list marks each conversation that already has its own window as **Open**. Clicking a conversation, or creating one, opens its dedicated conversation window; asking for the same conversation again raises that existing window instead of duplicating it, and an action that hands text to Chat seeds only the conversation it opened. The Alpha desktop right dock additionally adds **Notes**. A dedicated chat window keeps only its fixed conversation title, transcript, in-conversation search and composer; it has no conversation list, title selector, Back action, or New Chat action, even at narrow and mobile widths. Notes remains a searchable list in the dock; selecting or creating a note opens or raises one dedicated window per note. Each note window owns its editor, dirty draft, save/conflict/delete state and close confirmation independently, without a list or Back action, while the standard Notes page keeps its existing list/detail split.

<!-- FNXC:OfficialDashboardDesignDocs 2026-09-13-02:40: Tablet and desktop share only the wide Alpha footer, including Terminal immediately left of Settings. Tablet retains its sidebar, compact Header, standard right dock, page routing, and absence of Alpha desktop windows, while mobile retains its own navigation pill. -->
<<<<<<< HEAD
Dashboard, Board, List, Planning, Missions, Agents, and Mailbox remain footer pages or actions as applicable. **New Task** is absent from both the desktop footer and Header; tablet and mobile retain the rightmost compact Header action that opens the existing modal, while desktop creation remains available from its other dedicated surfaces and keyboard shortcut. Column headers do not duplicate it. On Board and List, the desktop Header keeps the workflow selector and one Search icon on one non-wrapping row; long workflow names truncate before the Search icon moves. Selecting Search replaces the icon in place with an inline task field whose matching suggestions open in a dropdown below it, without an overlay or backdrop. Choosing a suggestion opens Task Detail without filtering Board or List; the close button, Escape, and selection clear the transient query, close the field, and restore the Search icon. In Quick Entry, the rightmost primary action is an icon-only Save button: a single click or tap saves the task exactly once, while a continuous 500 ms hold fills the button and starts the task. Releasing before the threshold performs the ordinary save; leaving or cancelling the gesture creates nothing, and the associated synthetic click is ignored. On mobile, every visible icon-only action in the primary row uses the same token-sized square; text option controls retain their content-sized width. Graph and enabled plugins, Skills, Memory, Whiteboard, Goals, Automations, Import Tasks, Workflows, Insights, Research, Ideation, Evals, and Settings remain overflow page/actions with their existing gates. Dev Server, Secrets, Pull Requests, Usage, Activity, Git, Files, Scripts, Chat, and task-detail surfaces remain owned by their existing Header/right-dock/modal hosts. Terminal additionally uses the shared tablet/desktop footer action and continues to use More on mobile, with every launcher opening the same modal. Tablet retains its sidebar, compact Header, standard right dock, ordinary page routing, and no Alpha desktop windows or guards; its dock does not add Notes, and its **Chat** entry shows the same inline conversation list used on desktop. No-project navigation retains its existing presentation. Historical alphaUpdates values never select another layout.
=======
Dashboard, Board, Planning, Missions, Agents, Chat, and Mailbox remain footer pages or actions as applicable. <!-- FNXC:AlphaDesktopNavigation 2026-09-15-07:00: Chat is a footer page again on operator request: the Alpha desktop dock's Chat tab stays a list-only launcher for per-session windows, while the footer entry opens the full Chat page in the main content area. --> **New Task** is absent from both the desktop footer and Header; tablet and mobile retain the rightmost compact Header action that opens the existing modal, while desktop creation remains available from its other dedicated surfaces and keyboard shortcut. Column headers do not duplicate it. On Board and List, the desktop Header keeps the workflow selector and one Search icon on one non-wrapping row; long workflow names truncate before the Search icon moves. Selecting Search replaces the icon in place with an inline task field whose matching suggestions open in a dropdown below it, without an overlay or backdrop. Choosing a suggestion opens Task Detail without filtering Board or List; the close button, Escape, and selection clear the transient query, close the field, and restore the Search icon. In Quick Entry, the rightmost primary action is an icon-only Save button: a single click or tap saves the task exactly once, while a continuous 500 ms hold fills the button and starts the task. Releasing before the threshold performs the ordinary save; leaving or cancelling the gesture creates nothing, and the associated synthetic click is ignored. On mobile, every visible icon-only action in the primary row uses the same token-sized square; text option controls retain their content-sized width. Graph and enabled plugins, Skills, Memory, Whiteboard, Goals, Automations, Import Tasks, Workflows, Insights, Research, Ideation, Evals, and Settings remain overflow page/actions with their existing gates. Dev Server, Secrets, Pull Requests, Usage, Activity, Git, Files, Scripts, Quick Chat, and task-detail surfaces remain owned by their existing Header/right-dock/modal hosts. Terminal additionally uses the shared tablet/desktop footer action and continues to use More on mobile, with every launcher opening the same modal. Tablet retains its sidebar, compact Header, standard right dock, ordinary page routing, and no Alpha desktop windows or guards; its dock does not add Notes, and Chat keeps the existing in-dock list/detail flow and expansion action. No-project navigation retains its existing presentation. Historical alphaUpdates values never select another layout.
...
**A window opens at about two thirds of the work-area height.** The standard opening height is capped at roughly 62% of the height between the header and the active footer, so a window looks like a window rather than a full screen and the board stays visible behind it. A window whose own minimum is taller than that cap still opens at its minimum, and a work area shorter than the window always has the last word. Resizing by hand, docking to a half or to the top (which still fills the whole work area), and mobile full-screen sheets are unaffected: dock a window to the top whenever you want it to fill the area as before.
...
2. **Description** — the operator's original description, read-only, inside a height-bounded region that scrolls internally so a long description cannot push the rest of the view off screen. It is focusable, so it can be reached and scrolled from the keyboard.
...
On the mobile Kanban board, free-scroll while your finger is down. At release, Fusion resolves the existing directional or nearest-column target and moves continuously to that exact reachable column, so the board feels fluid instead of abruptly locking or visibly hard-jumping. Direction is locked at finger-up from the net swipe (not rubber-band ticks), and the result is held briefly by a bounded fence. A direction-zero or off-center release uses the same controlled nearest-column settle, so the board never rests between columns. Reduced-motion preferences, negligible-distance corrections, or unavailable animation capability use an immediate safe landing instead.
...
**Settings → Appearance → Open tasks as popups** changes ordinary board task-card clicks and List row/card opens across desktop, tablet, and mobile viewports. When enabled, those clicks use the existing task popup/FloatingWindow surface on the board/task-detail layer instead of the full-panel task detail or List split-detail/docked detail, keeping the board or List view visible in the background. Overlapping Chat and task popups interleave by the most-recent pointer or focus interaction; other utility windows retain their higher global stacking. On desktop and tablet, every task popup opens at the standard task size, centred, independently of the other windows; on mobile, task popups stay full-screen sheets. Board task-card `changes`/`retries`/`workflow` chips open the popup with the requested tab. List context-menu/refine actions, task-detail links, plugin/graph opens, and explicit pop-out actions keep their existing paths.
...
2. Select **Activity Log**, **Git Manager**, **Files**, or another available inline tool in the dock toolbar.
...
<!-- FNXC:TaskDetailHeaderActionsDocs 2026-09-11-17:35: Task Detail relocates the existing flat Actions list from the footer to an icon-only header overflow while preserving every descriptor, applicability gate, handler, initial focus, keyboard path, and close behavior. -->
...
Chat provides project-scoped conversations with agents. There is one Chat per project: on tablet and desktop it is the window opened from the right dock's **Chat** entry, the Header Chat action, or the sidebar Chat entry, and on phones it is the Chat drawer. Every host starts at the conversation list; select a conversation to open its thread. When the software keyboard is open, the composer remains visible on phones in portrait and landscape, tablets/iPad, and narrow Chat windows; wide desktop Chat is unaffected.
...
The shared Chat header owns **New Chat** in both the list and selected detail across the Chat window, the mobile Chat drawer, and detached conversation windows. Search and tag filters, archived/restore, and each row's rename, pin, archive, and delete actions remain list-only. Detail intentionally contains the saved conversation title and secondary model metadata when available. The default list contains only active sessions; use **Archived conversations** to view archived sessions, restore one to the active list, or explicitly delete it. Archive is the default removal action, while delete remains a separate destructive action.
...
<!-- FNXC:ChatContextGuardEscalation 2026-09-04-12:56: RUFU-182 replaced the honest-reason-code refusal contract and the single aggressive escalation on the chat lane. Operators must be able to tell a self-healed stale-usage refusal from an absolute engine refusal (which tool/memory reduction cannot unlock) from a transient failure that was retried once. -->
<!-- FNXC:ChatContextGuardTier3 2026-09-04-23:21: RUFU-183 ships the partial-history tier this copy pointed forward to. Operator requirement: an `Already compacted` deadlock must never leave a chat permanently unsendable, the deterministic recovery must be provably under threshold with zero model calls, and the operator must SEE what history was dropped — without the tier ever becoming a ninth refusal reason. -->
Chat sessions are protected from the model context wall by a deterministic pre-overflow compaction gate. Before every model-loop LLM call (regular chat, QuickChat, and room responder replies) Fusion measures the loaded history — provider usage when present, otherwise a deterministic chars/4 estimate — and compacts the session when it reaches the compaction threshold. The threshold defaults to **80% of the model's context window** (e.g. 102,400 for a 128K model); the **Token Cap** project setting is an upper bound on that threshold — a smaller value compacts earlier, and values above the model's hard limit are clamped. A compaction attempt that fails **without touching the session** (a transient engine error) escalates exactly once with an aggressive consolidation directive; every other LLM-pass failure is terminal at the first pass, because the engine refuses a second full compaction once one has already been applied — no directive can unlock it. An accepted compaction must actually reduce the context: a summary that does not reduce is reported honestly instead of being papered over, and a send that still fits proceeds flagged as unvalidated rather than silently trusted. When the gate finally refuses, the call is **not sent** and a visible chat error (code `CHAT_CONTEXT_OVERFLOW`) surfaces instead of a silently degraded 1-token reply — and the message names the honest reason code (`reason=already-compacted`, `reason=empty-summary`, `reason=non-reducing-summary`, `reason=post-compaction-over-limit`, `reason=compaction-error`, …) with the engine's own words, never a borrowed diagnosis. Two outcomes matter most: an **`Already compacted`** refusal no longer deadlocks the chat — shrinking the agent's tools or memory still cannot unlock a second LLM pass, but the gate's deterministic partial-history tier (RUFU-183) then takes over: with **zero model calls** it truncates older history at a turn boundary, keeps the newest turns verbatim, replaces what was dropped with a marked digest, and proceeds only after proving the rebuilt context sits under the threshold; the rescued reply carries an inline notice stating how many older messages and tokens were dropped, with an explicit unproven variant when the re-measurement was unavailable. A larger-window model or a fresh chat remain the remedies only when that tier itself cannot converge — e.g. the newest turn alone exceeds the budget — which still ends in the honest `CHAT_CONTEXT_OVERFLOW` refusal; a refusal where a **fresh measurement** of the current prompt, tools, and history fits below the threshold means the recorded usage was stale, and Fusion proceeds with the send automatically (self-healing, no action needed). Every gate compaction attempt records one bounded `chat:pre-overflow-compaction` run-audit row (reason/tier/outcome/tokens only — never message text; a deterministic rescue records `tier=fallback` with count-only drop keys). The first call after resuming a session is gated too, so a session that loaded a large history is compacted before its first message. Root cause (pi's threshold auto-compaction is blind to providers that omit usage from their streams, e.g. dsai1), design, and the pi second-compaction constraint the tier-3 route works around: `docs/solutions/logic-errors/chat-pre-overflow-compaction-blindness.md` and `docs/solutions/reliability/pi-second-compaction-deadlock-deterministic-tier3.md`.
...
<!-- FNXC:ChatContextGuardEscalation 2026-09-04-12:56: RUFU-182 second (CLI-chat-adjacent) copy of the pre-overflow gate paragraph updated in lockstep with the first copy above — the two must stay byte-identical. -->
<!-- FNXC:ChatContextGuardTier3 2026-09-04-23:21: RUFU-183 second-copy update in lockstep with the first copy above — operator requirements: `Already compacted` must self-recover via zero-LLM deterministic truncation (provable, under threshold) with an operator-visible drop notice, never a ninth refusal reason. -->
Chat sessions are protected from the model context wall by a deterministic pre-overflow compaction gate. Before every model-loop LLM call (regular chat, QuickChat, and room responder replies) Fusion measures the loaded history — provider usage when present, otherwise a deterministic chars/4 estimate — and compacts the session when it reaches the compaction threshold. The threshold defaults to **80% of the model's context window** (e.g. 102,400 for a 128K model); the **Token Cap** project setting is an upper bound on that threshold — a smaller value compacts earlier, and values above the model's hard limit are clamped. A compaction attempt that fails **without touching the session** (a transient engine error) escalates exactly once with an aggressive consolidation directive; every other LLM-pass failure is terminal at the first pass, because the engine refuses a second full compaction once one has already been applied — no directive can unlock it. An accepted compaction must actually reduce the context: a summary that does not reduce is reported honestly instead of being papered over, and a send that still fits proceeds flagged as unvalidated rather than silently trusted. When the gate finally refuses, the call is **not sent** and a visible chat error (code `CHAT_CONTEXT_OVERFLOW`) surfaces instead of a silently degraded 1-token reply — and the message names the honest reason code (`reason=already-compacted`, `reason=empty-summary`, `reason=non-reducing-summary`, `reason=post-compaction-over-limit`, `reason=compaction-error`, …) with the engine's own words, never a borrowed diagnosis. Two outcomes matter most: an **`Already compacted`** refusal no longer deadlocks the chat — shrinking the agent's tools or memory still cannot unlock a second LLM pass, but the gate's deterministic partial-history tier (RUFU-183) then takes over: with **zero model calls** it truncates older history at a turn boundary, keeps the newest turns verbatim, replaces what was dropped with a marked digest, and proceeds only after proving the rebuilt context sits under the threshold; the rescued reply carries an inline notice stating how many older messages and tokens were dropped, with an explicit unproven variant when the re-measurement was unavailable. A larger-window model or a fresh chat remain the remedies only when that tier itself cannot converge — e.g. the newest turn alone exceeds the budget — which still ends in the honest `CHAT_CONTEXT_OVERFLOW` refusal; a refusal where a **fresh measurement** of the current prompt, tools, and history fits below the threshold means the recorded usage was stale, and Fusion proceeds with the send automatically (self-healing, no action needed). Every gate compaction attempt records one bounded `chat:pre-overflow-compaction` run-audit row (reason/tier/outcome/tokens only — never message text; a deterministic rescue records `tier=fallback` with count-only drop keys). The first call after resuming a session is gated too, so a session that loaded a large history is compacted before its first message. Root cause (pi's threshold auto-compaction is blind to providers that omit usage from their streams, e.g. dsai1), design, and the pi second-compaction constraint the tier-3 route works around: `docs/solutions/logic-errors/chat-pre-overflow-compaction-blindness.md` and `docs/solutions/reliability/pi-second-compaction-deadlock-deterministic-tier3.md`.
