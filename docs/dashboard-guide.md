...
<!-- FNXC:AlphaFooterDocs 2026-09-10-22:00: Alpha removes the executor footer only on phones, where the navigation pill owns the bottom edge; tablet and desktop operators must retain footer controls such as Terminal. -->
Alpha does not render the executor status footer on mobile phones. On mobile, the pill clears the safe area, standalone gap, and visual-viewport compensation, while the Board height chain reserves its measured border box and placement exactly once; empty, skeleton, and populated columns end directly above the pill without overlap or an extra padding band. The last link, button, field, or card can therefore scroll and receive focus fully above it in portrait or landscape. Open drawers do not subtract the pill height or system offset from their outer geometry; they absorb safe-area, standalone, and visual-viewport clearance inside their own scrollable surface. The software keyboard may still suppress the pill without changing that drawer geometry. Tablet and desktop retain the project sidebar, right dock, existing floating/modal presentation, and executor footer with controls such as Terminal. Standard mode keeps its existing footer, full-screen mobile sheets, and mobile navigation at every breakpoint. On desktop Board and List, Alpha Updates keeps search inline in the header; tablet and mobile search retain their existing interaction.
