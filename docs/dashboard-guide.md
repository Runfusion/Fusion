...
<!-- FNXC:BoardNavigationDocs 2026-09-18-02:12: FN-522 separates the two return rules an operator can confuse. Horizontal lane context and the Board's own geometry are preserved, while the FN-330 vertical arrival reset is unchanged. -->
Returning to **Board** puts the board back where you left it: same columns, same selected workflow, same horizontal position, and its full usable height with no empty band or leftover header space inherited from Planning or Missions. The board is not pushed down for a moment while the Header reclaims its workflow selector, and no page reload is needed to straighten the layout. This is a horizontal and geometric guarantee only — the FN-330 rule below is unchanged, so a genuine Board arrival still starts every column at its first task, while a Board left active behind a phone drawer is not treated as a new arrival.
...
<!-- FNXC:MobileNavGesture 2026-09-18-00:54: FN-520 documents that the mobile navigation list itself now closes with the same pull-down contract as every other drawer, from either opening route, while a scrolled list keeps its native interior scrolling. -->
The navigation list itself closes like a drawer: when it is already scrolled to the top, pull it downwards and it closes, instead of bouncing against an empty overscroll. This works the same whichever way the list was opened — with the menu button, or with the upward swipe described below. When the list is already scrolled, the downward drag keeps scrolling it normally, and single taps on destinations are unchanged.
...
**Ephemeral action menus.** A context menu opened from a row, a card, or a file node must always paint *above* the panel or window that hosts its own target, so it takes the two steps above the reserved control: `calc(var(--fusion-max-z) + 5)` for its click catcher and `calc(var(--fusion-max-z) + 6)` for its panel. A menu that claimed a layer from the window counter could never exceed the published ceiling and therefore disappeared behind a transient panel declared at `+ 3` — this is why the Notes "…" menu used to render behind the Notes panel itself. These menus are allowed above the window-visibility control because they are strictly ephemeral: they close on an outside pointer, on Escape, and on any scroll. `styles.css` remains the source of truth for the whole scale.

**Gesture elevation.** While a floating window is engaged in a pointer gesture — a header drag or a resize, with the pointer captured — it is the last surface the operator grabbed, so it is painted one step above every transient surface listed here (`ceiling + 7`). This claim is made in JavaScript by `engagedGestureZ()` in `floatingWindowStack.ts`, never in a CSS rule, is never published into `--fusion-max-z` (the transient band would otherwise chase it on every gesture), and lasts exactly as long as the gesture: it is released on `pointerup`, on `pointercancel`, and on unmount. It reuses no reserved step. It complements, rather than replaces, the dismissal rule: a popover owned by `useOutsidePointerDismiss` now closes on the very press that starts the gesture, even when that press stops its own propagation before reaching `document`.

**Long press never selects text.** On every surface where holding a finger opens a context menu — list rows qualified by the shared row gesture, list cards, and file-browser nodes — the browser's native text selection and the iOS copy callout are suppressed, so the operator sees the menu and nothing else. The suppression is a base rule, identical on desktop, tablet, and phone, and editable controls nested inside a row keep native selection.
