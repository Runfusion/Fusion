...
>>>>>>> af32cdce81 (feat(dashboard): restore Chat as an Alpha footer page destination)
...
| **Chat** | A **Chat** button in the bottom bar opens the conversation list as an anchored panel; selecting or creating a conversation closes the list and opens or raises that conversation's own window. Under the **Left sidebar** navigation placement, **Chat** stays a full main page instead. On mobile the list uses the standard responsive modal presentation. |
| **Board / List** | A header button toggles Board and List; from any other destination it opens List. |
| **Dev Server** and **plugin views** | Ordinary primary-navigation destinations under their existing flags. |

Only one navigation panel — Activity, Notes, or the Chat list — is open at a time. Escape or a click outside closes the open panel and returns focus to its trigger; closing a list never closes the conversation or note windows it opened. The AI usage popover stays independent.
