...
**Definition** starts with the operator’s original description rendered read-only, followed by **Read plan** and the unified execution progress. The title remains available in **Edit task**, and **Summarize** remains next to Description when the task is editable and has description text; neither action reintroduces the title into the shared header. Progress uses one summary bar plus a labeled list that identifies every implementation and workflow step, including pending, running, completed, skipped, advisory, and failed states. **Read plan** opens the complete task `PROMPT.md` inside Task Detail, temporarily replacing the tab strip and Definition summary; **Back to definition** restores the prior surface. The plan view retains Open PROMPT.md, Edit, AI revision, approval/rejection, loading, and empty-plan behavior, and automatically closes when the host switches to another task.
