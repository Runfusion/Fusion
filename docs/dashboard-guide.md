...
Desktop Alpha replaces the left sidebar and ExecutorStatusBar with one full-width navigation footer. Its far-left running/max capacity counter opens the existing Max concurrent tasks and Max worktrees controls; direct destinations stay centered, **More** follows them, and **Settings** is isolated at the far right. History stays available from completed workflow columns instead of appearing in that footer. The Alpha desktop right dock adds **Chat** and **Notes**: Chat shows only the conversation list, and selecting or creating a conversation opens or reactivates its dedicated chat window; it never renders a transcript or generic expanded dock modal. Notes uses a compact list-to-detail flow directly in the dock, preserves its draft and conflict state through dock tab changes, and has no pop-out or expansion action.
