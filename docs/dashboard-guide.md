...
<!-- FNXC:AlphaMobileDrawerDocs 2026-09-10-04:41: Alpha mobile makes Board the permanent background and presents every other project surface in one bounded, mobile-first drawer while preserving standard mobile and larger Alpha layouts. -->
The Alpha icon-only pill provides Dashboard, Planning, Chat, and Mailbox as a floating overlay; Board is deliberately absent from both the pill and hamburger because closing the current drawer returns to the Kanban. Destinations, plugins, Settings, Workflows, Automation, imports, files, Git, Activity, Scripts, Terminal and Task Detail share a bottom drawer with a visible handle, accessible title and close control, an independently scrollable body, and a bounded top edge that leaves part of the Kanban visible. Task Detail uses that same drawer whether it was opened from Board, List, a plugin, the former pop-out action, or another mobile task surface, retaining its requested tab and nested-detail history without opening a second copy. Escape, backdrop, browser/Android Back, and the close control consume one navigation entry and restore the Kanban.

Alpha does not render the executor status footer on phone, tablet, or desktop. On mobile, the pill clears the safe area, standalone gap, and visual-viewport compensation, while the shared content scroller reserves its measured rendered height and placement; the last link, button, field, or card can therefore scroll and receive focus fully above it in portrait or landscape. A utility drawer or the software keyboard hides the pill and removes its measured reserve while retaining system-safe clearance. Tablet and desktop retain the project sidebar, right dock, and existing floating/modal presentation without footer spacing, while standard mode keeps its existing footer, full-screen mobile sheets, and mobile navigation at every breakpoint. On desktop Board and List, Alpha Updates keeps search inline in the header; tablet and mobile search retain their existing interaction.
