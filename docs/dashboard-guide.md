...
The Alpha icon-only pill provides Dashboard, Planning, Chat, and Mailbox as a floating overlay; Board is deliberately absent from both the pill and hamburger because closing the current drawer returns to the Kanban. Destinations, plugins, Settings, Workflows, Automation, imports, files, Git, Activity, Scripts, Terminal and Task Detail share a bottom drawer with a visible handle, accessible title and close control, an independently scrollable body, and a bounded top edge that leaves part of the Kanban visible. The drawer surface starts at the bottom edge and paints above the pill: the pill remains only its trigger in the background, not a reserved strip below it. Task Detail uses that same drawer whether it was opened from Board, List, a plugin, the former pop-out action, or another mobile task surface, retaining its requested tab and nested-detail history without opening a second copy. Escape, backdrop, browser/Android Back, and the close control consume one navigation entry and restore the Kanban.
