...
Movable dashboard pop-outs remember their last desktop location and size, while centered resizable dialogs remember their size. On mobile, project utilities use a bounded drawer that leaves the desktop geometry record untouched; reopening a pop-out on desktop restores its prior floating geometry. Simultaneous secondary chat windows are visibly offset from the window behind them; if a remembered window is close to the viewport size, its displayed window shrinks only enough to preserve that offset. The remembered desktop geometry remains the un-cascaded, un-shrunk base. Mobile and short-viewport full-screen sheets remain exempt from cascading.
