...
### Desktop Alpha footer and right dock

Desktop Alpha replaces the left sidebar and ExecutorStatusBar with one full-width navigation footer. Its far-left running/max capacity counter opens the existing Max concurrent tasks and Max worktrees controls; direct destinations stay centered, **More** follows them, and **Settings** is isolated at the far right. **More** opens above its trigger as a compact single vertical column, with one destination per row and vertical scrolling when the list grows beyond the viewport. Hovering or focusing the trigger opens it, leaving the combined trigger-menu region closes it, and Escape provides explicit keyboard dismissal. History stays available from completed workflow columns instead of appearing in that footer. The right dock body is labelled by its selected tab without repeating a generic visible header above views that already provide their own title. **Files** remains a browser list: selecting any text, image, audio, video, PDF, or binary file opens the shared file editor/preview window, and no editor or save controls replace the list inside the dock. The Alpha desktop right dock adds **Chat** and **Notes**: Chat shows only the conversation list, labels each secondary window as **Open** or **Minimized**, and selecting or creating a conversation opens, restores, or raises that same dedicated window without duplication. A dedicated chat window keeps only its fixed conversation title, transcript, in-conversation search and composer; it has no conversation list, title selector, Back action, or New Chat action, even at narrow and mobile widths. Notes remains a searchable list in the dock; selecting or creating a note opens or raises one dedicated window per note. Each note window owns its editor, dirty draft, save/conflict/delete state and close confirmation independently, without a list or Back action, while the standard Notes page keeps its existing list/detail split.

<!-- FNXC:AlphaDesktopHeaderDocs 2026-09-12-21:52: Alpha desktop omits the global New Task Header action and keeps the shrinkable workflow selector immediately beside Search. On Board/List, the magnifier expands in place into the shared task combobox and dropdown without a modal; tablet/mobile retain their compact Header actions and existing search presentation. -->
Dashboard, Board, List, Planning, Missions, Agents, and Mailbox remain footer pages or actions as applicable. **New Task** is absent from both the Alpha desktop footer and Header; tablet and mobile retain the rightmost compact Header action that opens the existing modal, while desktop creation remains available from its other dedicated surfaces and keyboard shortcut. Alpha column headers do not duplicate it. On Board and List, the Alpha desktop Header keeps the workflow selector and one Search icon on one non-wrapping row; long workflow names truncate before the Search icon moves. Selecting Search replaces the icon in place with an inline task field whose matching suggestions open in a dropdown below it, without an overlay or backdrop. Choosing a suggestion opens Task Detail without filtering Board or List; the close button, Escape, and selection clear the transient query, close the field, and restore the Search icon. In an Alpha Quick Entry, the rightmost primary action is an icon-only Save button: release a brief press to save normally, or hold continuously for 1,200 ms until the progress fill changes it to Start. Leaving or cancelling the gesture creates nothing. Non-Alpha Quick Entry retains its text Save button and separate Start action. Graph and enabled plugins, Skills, Memory, Whiteboard, Goals, Automations, Import Tasks, Workflows, Insights, Research, Ideation, Evals, and Settings remain overflow page/actions with their existing gates. Dev Server, Secrets, Pull Requests, Usage, Activity, Git, Files, Scripts, Terminal, Quick Chat, and task-detail surfaces remain owned by their existing Header/right-dock/modal hosts. Alpha tablet and standard desktop docks do not add Notes; their Chat keeps the existing in-dock list/detail flow and expansion action. No-project navigation retains its existing presentation.
...
Movable dashboard pop-outs remember their last desktop location and size, while centered resizable dialogs remember their size. When a standard-mobile pop-out becomes a full-screen sheet at mobile widths or its configured short-height breakpoint, it leaves that desktop record untouched; Alpha mobile instead presents project utilities in a bounded drawer and likewise leaves desktop geometry untouched; reopening it on desktop restores the prior floating geometry. Simultaneous secondary chat windows are visibly offset from the window behind them; if a remembered window is close to the viewport size, its displayed window shrinks only enough to preserve that offset. The remembered desktop geometry remains the un-cascaded, un-shrunk base. Mobile and short-viewport full-screen sheets remain exempt from cascading.
...
Task Detail uses the same four-zone shell in its modal, Board panel, List split view, right dock, and pop-out: header, tab strip, active content, then an optional footer owned by that tab. The task title remains in the shared header on every destination, alongside its task and lifecycle identity. **Chat** and **Activity → Live** keep the transcript as the scrollable content and pin their composer in the footer; read-only destinations such as Activity Feed and Raw do not reserve an empty footer.
...
**Definition** starts with the operator’s original description rendered read-only, followed by **Read plan** and the unified execution progress. Progress uses one summary bar plus a labeled list that identifies every implementation and workflow step, including pending, running, completed, skipped, advisory, and failed states. **Read plan** opens the complete task `PROMPT.md` inside Task Detail, temporarily replacing the tab strip and Definition summary; **Back to definition** restores the prior surface. The plan view retains Open PROMPT.md, Edit, AI revision, approval/rejection, loading, and empty-plan behavior, and automatically closes when the host switches to another task.
...
On standard mobile board-card detail, **Back to board** restores the prior horizontal Board position while every workflow column starts at its top. In the Alpha mobile drawer, the canonical **Close** action performs that restoration and **Back to board** is intentionally absent.
...
On mobile viewports, the Right Dock never renders. Standard mode keeps the existing compact Header actions and reserved bottom `MobileNavBar`; its **More** action continues to open the bottom drawer with backdrop, grab handle, swipe-down dismissal, and Back/Escape handling. With global **Alpha Updates** enabled, the wordmark is reduced to its logo and the hamburger becomes the rightmost control in the floating pill. It opens the same compact, scrollable navigation popover, reusing the destination registry, badges, feature gates, plugin views, and Scripts submenu without introducing another drawer or gesture. Selecting any destination opens it in the shared drawer over the still-mounted Kanban.

<!-- FNXC:AlphaMobileDrawerDocs 2026-09-10-16:56: Alpha mobile makes Board the permanent background and presents every other project surface in one bounded drawer anchored to the viewport bottom above its trigger pill, while preserving standard mobile and larger Alpha layouts.
...
With **Alpha Updates** enabled, the complete Task Detail surface—not only its chat content—uses Fusion’s neutral homemade Alpha presentation. Every host follows the same fixed order: **Header → Tabs → Content → optional Footer**. The compact header shows the task number and status, followed by icon-only **Duplicate**, **Retry**, **Delete**, **Pause/Unpause**, and **Reset** actions when each is eligible. Its icon-only **Actions** overflow retains attachments, GitHub tracking, oversight, priority, Fast mode, refinement, bypass, Revise, and Revert operations. Every icon has a localized accessible name and tooltip.
...
The modal from `AppModals`, mobile drawer, Board main-panel detail, List split detail, Right Dock detail, and task pop-out all render this same retained surface. Activity and Actions menus and the Refine dialog inherit its light or dark neutral palette even though overlays are portaled. Switching hosts or receiving live task updates does not reset the selected tab, edit draft, focus, close/navigation behavior, or lifecycle guards. When **Alpha Updates** is disabled, Task Detail keeps the same structure and interaction ownership with its established stable presentation.
...
Alpha does not render the executor status footer on mobile phones. On mobile, the pill clears the safe area, standalone gap, and visual-viewport compensation, while the Board height chain reserves its measured border box and placement exactly once; empty, skeleton, populated, duplicate, and pagination-error columns fill that safe height with the same spacing token above and below, without overlap or another safe-area padding band. The last link, button, field, or card can therefore scroll and receive focus fully above it in portrait or landscape. Open drawers do not subtract the pill height or system offset from their outer geometry; they absorb safe-area, standalone, and visual-viewport clearance inside their own scrollable surface. The software keyboard may still suppress the pill without changing that drawer geometry. Tablet and desktop retain the project sidebar, right dock, existing floating/modal presentation, and executor footer with controls such as Terminal. Standard mode keeps its existing footer, full-screen mobile sheets, and mobile navigation at every breakpoint. On desktop Board and List, Alpha Updates keeps search inline in the header; tablet and mobile search retain their existing interaction.
...
### Homemade Alpha boundary

Fusion’s homemade Alpha system is enabled only when `experimentalFeatures.alphaUpdates` is true and only inside explicit Board, Chat, and Task Detail boundaries. This includes the kept-alive Board and main Chat, Quick Chat, popped-out and docked Chat, task and planning Chat, and every shared Task Detail host; List, navigation, Settings, and plugin-owned content remain on the stable presentation.

Within those boundaries, Alpha has exactly two canonical neutral palettes: light and dark. The resolved document `data-theme` selects the palette, including when the global preference is `system`; `data-color-theme`, custom shadcn colors, and other Fusion theme accents, radii, or shadows never alter an Alpha surface. Body-portaled menus, popovers, and dialogs receive the same palette as their originating surface. Semantic error and warning states remain visible through the fixed light/dark Alpha tokens rather than a Fusion color theme.

Use the shared `alpha-ui` primitives instead of introducing a component dependency. They render native HTML controls with browser form behavior and Fusion-owned keyboard, focus, overlay, and dismissal handling. Alpha Board and Chat use quiet secondary actions, restrained borders/elevation, and a clear input-first hierarchy; Quick Entry keeps Save prominent but compact and places advanced options behind its disclosure. Shared adaptive primitives keep their historical unmarked rendering outside an active Alpha boundary, including Quick Entry in List and controls reused by Mailbox.
