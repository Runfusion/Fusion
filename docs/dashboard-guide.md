...
Movable dashboard pop-outs remember their last desktop location and size, while centered resizable dialogs remember their size. Their dynamic movement and resize limits use the shell's current usable rectangle: windows may touch the available edges without artificial padding, but cannot cover the visible header, footer, or open right sidebar. The limits update when those shell regions resize, open, close, or disappear, and an existing window is clamped back into the new usable area without discarding its remembered desktop geometry.

On mobile, Chat uses a navigation drawer and other project utilities use their responsive drawer or sheet presentation; these leave desktop geometry records untouched. Reopening a pop-out on desktop restores its prior floating geometry. Simultaneous secondary chat windows are visibly offset from the window behind them; if a remembered window is close to the usable shell rectangle, its displayed window shrinks only enough to preserve that offset. The remembered desktop geometry remains the un-cascaded, un-shrunk base. Mobile and short-viewport sheets remain exempt from cascading.
...
**Definition** is read top to bottom in three parts:

1. **Progress** — heading, `N/M completed` counter and summary bar are always visible. The per-step list is **collapsed by default** behind a **Show steps** / **Hide steps** control (a real button with `aria-expanded`/`aria-controls`, so keyboard and screen-reader users get the same affordance). Expanding it reveals the labeled list identifying every implementation and workflow step, including pending, running, completed, skipped, advisory, and failed states. Your choice survives live refreshes of the same task and resets to collapsed only when a different task is opened. A task with no steps shows no control at all.
...
FNXC:TaskPopupGeometry 2026-09-13-02:05: Desktop/tablet task popups share a persisted geometry key across task IDs so operators can size and place the popup once, then open other tasks without repeating that setup. Mobile routes Task Detail into the official bounded drawer without changing saved desktop geometry.
FNXC:TaskPopupLayer 2026-09-14-10:42: Task popups and desktop Chat windows share the board/task-detail interaction stack, so the most recently clicked or focused overlapping surface appears on top. Terminal, Files, New Task, workflow editor, and other utility windows retain their separate higher utility stacking.
...
**Settings → Appearance → Open tasks as popups** changes ordinary board task-card clicks and List row/card opens across desktop, tablet, and mobile viewports. When enabled, those clicks use the existing task popup/FloatingWindow surface on the board/task-detail layer instead of the full-panel task detail or List split-detail/docked detail, keeping the board or List view visible in the background. Overlapping Chat and task popups interleave by the most-recent pointer or focus interaction; other utility windows retain their higher global stacking. On desktop and tablet, task popups restore the last saved popup size and position between tasks; on mobile, task popups stay full-screen sheets. Board task-card `changes`/`retries`/`workflow` chips open the popup with the requested tab. List context-menu/refine actions, task-detail links, plugin/graph opens, and explicit pop-out actions keep their existing paths.
...
<!-- FNXC:DashboardNavigationDocs 2026-09-14-12:57: The right dock keeps Chat as a launcher entry that opens the single Chat window; keep this user-facing roster aligned with STATIC_OVERFLOW_VIEW_ENTRIES. -->
...
The workflow editor opens as a full-screen modal editor for inspecting built-ins and authoring custom workflows, while mobile uses the shared bounded drawer.
