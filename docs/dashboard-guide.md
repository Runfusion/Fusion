...
- Task context menus do not offer a destination picker. Use **Reset** or **Respecify** to replan from the original description, or use the other retained Actions controls. Manual-intake cards may still offer **Start** to admit a new idea into its workflow.
...
Board-local and is not persisted as a project setting. The physical Archived lane is request-backed
because its selected order must be applied before each bounded server page is sliced.
-->
...
- Column ordering semantics: every visible non-archived lane has two choices in its existing actions menu. **Arrival in this column** shows the newest `columnMovedAt` first, falling back to `updatedAt` and then `createdAt` only for legacy rows; **Task ID** shows the highest numeric task suffix first with deterministic textual ties. An explicit choice intentionally replaces the lane's normal priority, merge, or hold-FIFO display ordering, without mutating tasks or refreshing `columnMovedAt`.
- The choices are independent and Board-local: changing Planning, Todo, In Progress, a review lane, a complete lane, or a renamed/custom workflow lane leaves every other lane's choice unchanged and does not create a persisted project preference. Complete lanes retain **Archive All Done** in the same menu when available.
- The physical Archived lane uses the same actions-menu choices, but its selection is sent to the bounded PostgreSQL archive endpoint. The server orders the full project-scoped archive before each 100-row `LIMIT/OFFSET` page, so **Show more** continues the selected global order without fetching or client-sorting the entire archive. Archived arrival is most recently archived first.
...
- **Models** — one row per model. Each row has a **Model ID** (required), an optional **Display name** (falls back to the model ID when blank), and two optional numeric fields: **Context window** and **Max output tokens**. Blank window fields leave the model on the registry defaults `128,000` / `16,384`; set them to the model's true limits so compaction thresholds are computed from the real window. Add or remove rows with the **Add model row** and per-row **Remove model row** actions. Each row also has a **Thinking format** select (**Default**, **Qwen (chat template)**, **Qwen (enable_thinking)**, **Z.AI / GLM (thinking)**, **DeepSeek (enable_thinking)**, **String thinking**, **OpenRouter**, **Together AI**, **Ant Ling**) and a **No thinking params** checkbox (RUFU-143): the select pins the pi-ai thinking-parameter dialect for OpenAI-compatible gateways (for example Qwen3 via LiteLLM, which rejects the `reasoning_effort` parameter), and checking **No thinking params** opts that model out of reasoning/thinking parameter sending entirely — the select is disabled while the checkbox is checked, and the checkbox wins over any selected format in the saved configuration. Both are per-model and optional; a row left at **Default** and unchecked persists nothing extra, so existing provider configurations round-trip unchanged. The raw `chat-template` format (which requires an accompanying `chatTemplateKwargs`) and the `openai` format (identical to the default) are deliberately not exposed in the UI and are accepted only from manual `models.json` edits — see [`customProviders` in the Settings Reference](./settings-reference.md#customproviders).
...
8. Add one **Models** row per model: enter the **Model ID** (and optionally a **Display name**, **Context window**, and **Max output tokens**), pin a **Thinking format** or check **No thinking params** when the gateway's thinking-parameter dialect or opt-out differs from the default, or select **Detect Models** to query the provider's `/models` endpoint and append detected models as rows (probed window limits included when the endpoint reports them).
...
3. Update **Provider name**, **API type**, **Base URL**, **API key**, or the **Models** rows (model IDs, display names, context windows, max output tokens, thinking formats, no-thinking opt-outs) as needed.
...
Custom-provider models also carry optional per-model thinking metadata (RUFU-143): the **Thinking format** select value persists in the provider's `models` entries as `thinkingFormat`, and the **No thinking params** checkbox persists as `reasoning: false`. Both register per model into pi's custom-provider model registration, so `thinkingFormat` selects pi-ai's thinking-parameter dialect for the model (for example `qwen-chat-template` for Qwen3 gateways that reject `reasoning_effort`) and `reasoning: false` stops pi-ai from sending reasoning/thinking parameters for that model. A model with neither flag keeps the default registration — the wire payload is unchanged — so unflagged models behave exactly as before for the chat reasoning-effort lanes.
...
<!-- FNXC:CLIChatMemoryRecall 2026-08-20-05:58: RUFU-128 extends RUFU-120's per-turn memory recall to CLI-agent-backed chat sessions (claude/pi PTY chat). The operator requirement encoded here: recall is on by default under the same per-turn recall settings, is delivered through each CLI's own native channel (never as typed terminal text), and degrades silently when it has nothing to say. -->
CLI-agent-backed chat sessions (claude and pi) participate in per-turn memory recall under the same settings as model-loop chat (Settings → Memory, **Per-turn memory recall**, on by default — `memoryPerTurnRecallEnabled` / `memoryPerTurnRecallTopK`). Each chat session is provisioned per spawn — and re-provisioned on resume — with a per-session generated artifact: a Claude Code `UserPromptSubmit` hook script delivered via `--settings` (claude) or a pi `before_agent_start` extension delivered via `--extension` (pi). On every turn the hook/extension posts the operator prompt to the loopback `/api/cli-agent/memory-recall` endpoint authenticated by that session's own token, and injects the returned recall cue through the CLI's native message channel. No text is ever typed into the terminal: the cue does not appear in the TUI transcript or in user input, and when recall is disabled, the prompt is not topical, or the endpoint is unreachable or errors, the turn simply proceeds without a cue (a silent empty-cue 202, never a chat error).


Chat sessions are protected from the model context wall by a deterministic pre-overflow compaction gate, which runs when the project setting `chatPreOverflowCompactionEnabled` is enabled (Settings → Memory → "Pre-overflow compaction guard", default: enabled, runtime-toggleable without a restart). Before every model-loop LLM call (regular chat, QuickChat, and room responder replies) Fusion measures the loaded history — provider usage when present, otherwise a deterministic chars/4 estimate — and compacts the session when it reaches the compaction threshold. The threshold defaults to **80% of the model's context window** (e.g. 102,400 for a 128K model); the **Token Cap** project setting is an upper bound on that threshold — a smaller value compacts earlier, and values above the model's hard limit are clamped. If compaction fails or the context still sits at/above the hard limit after compaction, the call is **not sent** and a visible chat error (code `CHAT_CONTEXT_OVERFLOW`) surfaces instead of a silently degraded 1-token reply. The first call after resuming a session is gated too, so a session that loaded a large history is compacted before its first message. The companion `chatContextBudgetEnabled` setting (Settings → Memory → "Chat context budget (64K window fit)", default: enabled) bounds the chat static context so agent chat fits 64K-window models; it is also runtime-toggleable without a restart. Root cause (pi's threshold auto-compaction is blind to providers that omit usage from their streams, e.g. dsai1) and design: `docs/solutions/logic-errors/chat-pre-overflow-compaction-blindness.md`.
...
<!-- FNXC:ChatInFlightRecovery 2026-08-20-21:24: A dashboard restart used to leave chat in_flight_generation flags stuck at "generating" forever, so every re-attach reopened a zombie streaming/thinking box. The engine now clears in-flight flags older than 30 minutes at startup and every maintenance cycle, and turns that exhaust their output budget on thinking (stopReason "length" with no visible content) show an explicit notice instead of a silent empty bubble. -->
- After a dashboard or engine restart, Chat self-heals stuck "generating" states: the engine clears persisted in-flight generation snapshots older than 30 minutes at startup and on every maintenance cycle, so a restarted server never reopens a zombie streaming/thinking box for a generation that died with the old process.
- When a model spends its entire output budget on thinking and a turn ends with no visible answer, the assistant message shows an inline notice ("The model used its entire output budget on thinking — raise maxTokens for this model.") in place of an empty bubble; the thinking stays expandable below the notice. Turns with a real answer, or legitimate empty answers (stop without truncation), are unaffected.
