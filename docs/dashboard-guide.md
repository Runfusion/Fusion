...
<!-- FNXC:AlphaUpdatesDocs 2026-09-10-03:16: Alpha removes the executor status footer at every viewport size while preserving tablet/desktop project chrome; mobile keeps the pill visually floating but reserves its measured footprint so final controls remain reachable above system insets. -->
The Alpha icon-only pill provides Dashboard, Board, Planning, Chat, and Mailbox as a floating overlay on every supported mobile project view. Alpha does not render the executor status footer on phone, tablet, or desktop. On mobile, the pill clears the safe area, standalone gap, and visual-viewport compensation, while the shared content scroller reserves its measured rendered height and placement; the last link, button, field, or card can therefore scroll and receive focus fully above it in portrait or landscape. All Projects, full-screen modals, and the software keyboard hide the pill and remove that reserve. Tablet and desktop retain the project sidebar and right dock without footer spacing, while standard mode keeps its existing footer and mobile navigation at every breakpoint. On desktop Board and List, Alpha Updates keeps search inline in the header; tablet and mobile search retain their existing interaction.
