...
**Snapping to the edges.** Drag a window by its header against the left edge and it occupies the left half of the work area, full height; the right edge does the same on the right. Drag it against the top and it fills the whole work area (not the browser full screen). While you approach an edge a preview shows the target area, and the placement is applied when you release; in a corner the top wins. Halves are always exactly half of the CURRENT work area: opening or closing the sidebar immediately re-splits the docked windows, and the shell is never closed for you. A docked window shows no resize handles.

**Getting out of a docked position.** Drag a docked window downwards and it returns to the size and shape it had before you docked it, following the cursor; its handles come back. Going from left to right and then to the top never replaces that remembered shape. Nothing is exclusive: several windows can share the same half, several can fill the work area, and a newly opened window always appears in front of them at its usual size.
