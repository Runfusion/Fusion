...
<!-- FNXC:TaskDetailHeroUIDocs 2026-09-11-02:41: Alpha applies one neutral HeroUI presentation to the complete Task Detail surface in every host, while the disabled flag preserves the stable presentation and all existing interaction ownership. -->
### HeroUI Alpha Task Detail

With **Alpha Updates** enabled, the complete Task Detail surface—not only its chat content—uses the neutral HeroUI presentation. Its compact header prioritizes task identity and status, the horizontally scrollable destination strip keeps every conditional tab reachable, and the central content is composed from quiet cards with one persistent action footer. On phones the content forms one column, respects the safe area, and keeps its own central scroll; tablet and desktop progressively use the available width without changing information or action order.

The modal from `AppModals`, mobile drawer, Board main-panel detail, List split detail, Right Dock detail, and task pop-out all render the same retained surface. Activity and Actions menus and the Refine dialog inherit its light or dark neutral palette even though overlays are portaled. Switching hosts or receiving live task updates does not reset the selected tab, edit draft, focus, close/navigation behavior, or lifecycle guards. When **Alpha Updates** is disabled, Task Detail keeps its established stable presentation and native control structure.

<!-- FNXC:AlphaFooterDocs 2026-09-11-15:01: Alpha removes the executor footer only on phones, where one measured pill reserve lets every Board state finish within an extra-small token of the overlay; tablet and desktop retain footer controls such as Terminal. -->
Alpha does not render the executor status footer on mobile phones. On mobile, the pill clears the safe area, standalone gap, and visual-viewport compensation, while the Board height chain reserves its measured border box and placement exactly once; empty, skeleton, and populated columns finish one extra-small spacing token above the pill without overlap or another safe-area padding band. The last link, button, field, or card can therefore scroll and receive focus fully above it in portrait or landscape. Open drawers do not subtract the pill height or system offset from their outer geometry; they absorb safe-area, standalone, and visual-viewport clearance inside their own scrollable surface. The software keyboard may still suppress the pill without changing that drawer geometry. Tablet and desktop retain the project sidebar, right dock, existing floating/modal presentation, and executor footer with controls such as Terminal. Standard mode keeps its existing footer, full-screen mobile sheets, and mobile navigation at every breakpoint. On desktop Board and List, Alpha Updates keeps search inline in the header; tablet and mobile search retain their existing interaction.
...
Within those boundaries, HeroUI has exactly two canonical neutral palettes: light and dark. The resolved document `data-theme` selects the palette, including when the global preference is `system`; `data-color-theme`, custom shadcn colors, and other Fusion theme accents, radii, or shadows never alter an Alpha surface. Body-portaled menus, selects, popovers, and dialogs receive the same palette as their originating surface. Semantic error and warning states remain visible through fixed light/dark HeroUI colors rather than a Fusion color theme.

Use the native HeroUI component whenever it provides the required control or surface. Fusion-owned CSS is limited to compact layout and density, token mapping, kanban drag behavior, transcript virtualization/streaming, and other behavior without a HeroUI equivalent. Alpha Board and Chat use quiet secondary actions, restrained borders/elevation, and a clear input-first hierarchy; Quick Entry keeps Save prominent but compact and places advanced options behind its disclosure. Shared adaptive primitives must keep their historical native rendering outside an active Alpha surface, including Quick Entry in List and controls reused by Mailbox.
