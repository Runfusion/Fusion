...
Alpha does not render the executor status footer on phone, tablet, or desktop. On mobile, the pill clears the safe area, standalone gap, and visual-viewport compensation, while the shared content scroller reserves its measured rendered height and placement; the last link, button, field, or card can therefore scroll and receive focus fully above it in portrait or landscape. Open drawers do not subtract the pill height or system offset from their outer geometry; they absorb safe-area, standalone, and visual-viewport clearance inside their own scrollable surface. The software keyboard may still suppress the pill without changing that drawer geometry. Tablet and desktop retain the project sidebar, right dock, and existing floating/modal presentation without footer spacing, while standard mode keeps its existing footer, full-screen mobile sheets, and mobile navigation at every breakpoint. On desktop Board and List, Alpha Updates keeps search inline in the header; tablet and mobile search retain their existing interaction.
