...
FNXC:AlphaMobileDrawerDocs 2026-09-10-22:24: Every Alpha drawer has one visible header and one canonical dynamic-viewport height; view-owned actions remain in that header while central content scrolls without displacing persistent inputs. -->
The Alpha icon-only pill provides Dashboard, Planning, Chat, and Mailbox as a floating overlay; Board is deliberately absent from both the pill and hamburger because closing the current drawer returns to the Kanban. Destinations, plugins, Settings, Workflows, Automation, imports, files, Git, Activity, Scripts, Terminal and Task Detail share a bottom drawer with a visible handle, accessible title and close control, one visible header, and a canonical height independent of content length. A destination's own header keeps ownership of its title and actions; only headerless content receives a visible shell title. Central content scrolls within the fixed drawer while persistent controls such as the Chat composer remain visible and focusable above internal system and keyboard clearance. The bounded top edge leaves part of the Kanban visible. The drawer surface starts at the bottom edge and paints above the pill: the pill remains only its trigger in the background, not a reserved strip below it. Task Detail uses that same drawer whether it was opened from Board, List, a plugin, the former pop-out action, or another mobile task surface, retaining its requested tab and nested-detail history without opening a second copy. Escape, backdrop, browser/Android Back, and the close control consume one navigation entry and restore the Kanban.
...
Alpha does not render the executor status footer on mobile phones. On mobile, the pill clears the safe area, standalone gap, and visual-viewport compensation, while the shared content scroller reserves its measured rendered height and placement; the last link, button, field, or card can therefore scroll and receive focus fully above it in portrait or landscape. Open drawers do not subtract the pill height or system offset from their outer geometry; they absorb safe-area, standalone, and visual-viewport clearance inside their own scrollable surface. The software keyboard may still suppress the pill without changing that drawer geometry. Tablet and desktop retain the project sidebar, right dock, existing floating/modal presentation, and executor footer with controls such as Terminal. Standard mode keeps its existing footer, full-screen mobile sheets, and mobile navigation at every breakpoint. On desktop Board and List, Alpha Updates keeps search inline in the header; tablet and mobile search retain their existing interaction.
