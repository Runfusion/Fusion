...
<!-- FNXC:TaskDetailAlphaDocs 2026-09-11-16:14: Alpha applies one neutral Fusion-owned presentation to the complete Task Detail surface in every host, while the disabled flag preserves the stable presentation and all existing interaction ownership. -->
### Alpha Task Detail

With **Alpha Updates** enabled, the complete Task Detail surface—not only its chat content—uses Fusion’s neutral homemade Alpha presentation. Its compact header prioritizes task identity and status, the horizontally scrollable destination strip keeps every conditional tab reachable, and the central content is composed from quiet cards with one persistent action footer. On phones the content forms one column, respects the safe area, and keeps its own central scroll; tablet and desktop progressively use the available width without changing information or action order.
