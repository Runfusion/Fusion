...
- Column ordering semantics (FN-509): there is no per-column sort choice and no column actions menu. Every lane uses one shared order, resolved from the column's own traits:
  ...
  Ordering never mutates a task or refreshes `columnMovedAt`. The retired **Replan All**, **Stop All** and auto-approve shortcuts lived in that removed menu; the individual operations and their endpoints are unchanged, and the **History** button and the **Auto-merge** toggle were always independent header controls.
