...
- R3. One agent per column. Simple mode: an agent holds at most one column per board; advanced mode may staff one agent on multiple columns. Agents are strictly board-scoped — an agent belongs to exactly one board (the CEO is project-level); no cross-board staffing or transfer, and the agents menu/creation are board-scoped (a board shows only its own team).
- R4. Board owns workflow/column config; tasks land on a board, never select a workflow.
- R5. Agent-driven movement is sequential, no skipping; only Lead/Reviewer move backward; the CEO has no movement powers. Human drags are limited to Idea ↔ Todo, Done → Archive, and Done/Archive → Todo (revert, R24) — no stage-skipping shortcuts, no dragging out of in-review.
- R6. Non-coding work keeps the worktree/merge machinery intact and produces a `report.md`-style artifact committed and merged through the standard pipeline (no merge-less column variant — reliability over cleverness, especially under low-end models).
- R24. Revert flow: Done/Archive → Todo reverts the task's merged changes (revert of its merge commit) and re-enters the cycle; conflicts surface to the user rather than forcing.
...
- R14. Simple mode (default) keeps boards, global chat, per-task chat, agent roster, basic settings, notifications. Simple mode HIDES (and advanced mode shows) Missions, graph/workflow editor, traits config, per-task agent/model selection, custom task fields, branch-group/merge-queue UI, plugin dev surfaces. Plugin-contributed user-facing views can declare simple-mode compatibility and appear in simple mode; only plugin development surfaces are categorically gated.
...
- R23. Simple mode always uses worktree isolation: tasks execute in per-task worktrees, the worktree setting cannot be disabled from simple mode (the toggle is an advanced-mode surface), and the board shows only tasks and columns — no worktree or branch details on cards; that detail lives in the task detail view and advanced mode.
...
- **One feature flag for the company-model semantics: `experimentalFeatures.companyModel`.** Composed additively with the existing `workflowColumns`/`workflowGraphExecutor` flags (which it implies for board internals). Flag off → execution semantics byte-identical to today (ephemeral workers, triage processor, entry-driven merge, no role teams, no CEO); the flag does NOT gate board containment — boards replace lanes unconditionally, in every mode, with the lane rendering path deleted outright. Rollback posture for that one unflagged, irreversible decision: forward-fix only, backed by the idempotent re-runnable migration, the dry-run-against-a-production-copy gate, and `task_workflow_selection` retained read-only as the substrate from which task homing can be re-derived if the boards table proves wrong. Kill-switch parity: every execution entry point must read the flag — bindings, seeds, and routing are inert when off (mirrors the column-agent kill-switch rule in `docs/solutions/architecture-patterns/per-entity-execution-principal-override-blast-radius.md`).
...
- **The movement matrix is strict for everyone.** Agents: adjacent-forward, Lead/Reviewer-only backward. CEO: creates tasks, never moves them. Humans: Idea ↔ Todo, Done → Archive, Done/Archive → Todo (revert) — nothing else; there is no human drag out of in-review, so manual merge approval happens through the existing Manual-required/merge-request affordances, not drags. The company template gains an unstaffed built-in Idea intake column before Todo (user-created tasks land there; CEO-routed tasks land in Todo). Auto-merge fires only on a passing Reviewer verdict.
...
- **Approach:** The template is a workflow IR preset whose role columns carry locked markers; reconciliation rejects deleting/replacing role columns and inserting columns before todo — custom columns are legal between todo and in-review and after in-review (post-approval steps that run between the Reviewer's pass and done; the verdict remains the sole gate out of in-review). Transition validation distinguishes actor class: agent moves must be adjacent-forward (or backward only when the acting effective agent is the board's Lead/Reviewer); the CEO has no movement powers; human moves are limited to the matrix (Idea ↔ Todo, Done → Archive, Done/Archive → Todo revert). The template includes the unstaffed Idea intake column before Todo. All boards keep the merge machinery; non-coding boards differ only in agent instructions (produce a report.md merged through the standard pipeline, R6).
- **Test scenarios:**
...
- **Requirements:** R1 (Lead role behavior), R20 (plan-approval hold), origin F1 step "Lead structures the request".
- **Dependencies:** U3, U4.
...
- **Requirements:** R11 (merge half), R19, AE1.
- **Dependencies:** U6.
...
- **Test scenarios:**
  ...
  - Merge-mode guard: `shouldAutoMergeTask` takes merge mode as an input and returns false for PR-mode boards regardless of any residual `autoMerge` stamp; a self-healing sweep over a PR-mode in-review task with a passing verdict never enqueues the legacy merge queue while a PR entity exists.
  - AE6 PR-mode variant: owner drags in-review→done with verdict pending on a PR-mode board → the task completes with an explicit "manually completed — no PR" marker; `pr-create` does not fire without a verdict, and the legacy queue is untouched.
...
- **Approach:** A column work engine is a per-column configuration resolved at dispatch: standard engines (triage pipeline, executor session, validator) or a CE Stage (registry id → bundled skill → artifact location). The CE board template staffs: Lead/Todo → ce-plan (plan doc artifact attached to the task; PROMPT.md points at it), Executor → ce-work (consumes the plan artifact), Reviewer → ce-code-review (its findings outcome feeds the U6 verdict interface), a Compound custom column between In Review and the PR/Done region → ce-compound (captures learnings to docs/solutions before shipping), and the PR respond loop → ce-resolve-pr-feedback (when the board is PR-mode). Engines run as CE Sessions through the existing detached-turn machinery; stage completion advances the column. Offered only when the plugin is installed (probe the bundled-plugin registry); never the default board type. Be explicit about what is net-new here (verified against the current code, not wiring of existing machinery): (a) the CE stage registry (`STAGE_DEFINITIONS`) registers only strategy/ideate/brainstorm/plan/work — `code-review` and `compound` stage entries must be added; (b) `ce-resolve-pr-feedback` does not exist in the bundled plugin's skills — author and bundle it (or map the respond loop to an equivalent new stage) before the PR respond-loop engine can bind; (c) a CE session completes with a markdown artifact, not a structured verdict — U13 builds a structured-outcome adapter (stage completion → pass/fail + findings) to feed the U6 verdict interface; (d) the orchestrator builds interactive sessions only (its non-interactive backend is an unwired seam) — LFG requires threading a headless mode through session construction (suppress the interactive question opening, never persist awaiting-input). LFG mode (CE boards only): when set on the board (overridable per task), every stage runs in its headless/pipeline form — no blocking questions, autonomous defaults — the plan-approval hold is skipped, and a session that would ask a question instead proceeds with its pipeline-mode behavior; the task flows Todo → done/PR untouched. LFG backstops (committed, not bypassed): the PR sub-graph's GitHub-corroborated state, bot denylist, and pre-push secret scan stay active; the U6 rework budget applies; and a stage with no safe headless default parks the task with a needs-attention diagnostic (promoting it out of LFG for that task) rather than fabricating an answer. Engine fallback covers parked states too: a task in a plan-approval hold or awaiting-input whose CE engine is gone at release parks with a plugin-missing diagnostic instead of degrading into an engine that can't consume the artifact.
- **Test scenarios:**
  ...
  - LFG no-safe-default: an LFG task hitting a stage that cannot proceed without input parks visibly with a needs-attention diagnostic instead of fabricating an answer.
  - Two postures, one identity: one CE column agent runs an interactive task (parked awaiting-input) and an LFG task (headless, mid-flight) concurrently — heartbeats, resume/wake-up filters, and change detection are per-session, so neither posture's lifecycle disturbs the other (posture is a session attribute, not an agent attribute).
  - Plugin uninstalled while a CE task is parked (approval hold or awaiting-input) → release parks the task with a plugin-missing diagnostic; no degrade into an engine that can't consume the artifact.
...
- **Test scenarios:**
  ...
  - Owner moves a task (cross-board or backward) while a question is pending → the awaiting-input state is invalidated and the Q&A view refuses to submit into the old session (reconciled with U10's restart-on-move warning).
  - Stall-detection exclusion: a task parked in awaiting-input or the plan-approval hold beyond the stuck threshold is NOT flagged stuck and is never auto-moved — intentional waits are not faults (coordinate with U5's stuck-detection re-keying).
...
- **Test scenarios:**
  ...
  - Flag off, multi-workflow: a project with two workflows-in-use renders two team-less boards via the switcher, tasks home correctly, and legacy (non-CEO) task creation still lands tasks on the right board — no path assumes a CEO exists.
...
- **Test scenarios:**
  ...
  - Covers R23: in simple mode, task execution uses a worktree even when the stored setting says disabled; the disable toggle is absent from simple-mode settings; cards render no worktree/branch chrome while the task detail still exposes it. Note: worktree behavior is governed by several scattered settings reads (executor, project-manager, engine-manager, runtime, scheduler) — implement the force-on as one central resolver those sites consult, not per-site patches, and decide explicitly which settings the override covers.
  - Worktree force-on is never silent: switching a worktree-disabled project to simple mode (or converting via R17) surfaces a one-time notice that simple mode forces worktree isolation (with the advanced-mode opt-out named) and records an audit event.
  - Git-failure visibility: a simple-mode task blocked on a merge conflict or branch/push failure raises a distinct blocked/needs-attention card badge (reusing the stuck-task indicator) with the conflict detail and resolution affordance reachable from the task detail — suppressing branch chrome must never suppress "the task needs your git decision."
...
- **Approach:** Project creation (flag on) runs a five-step onboarding flow mirroring the `AgentOnboardingModal` view-state machine, skippable at any step and never re-shown: (1) select/create the working directory and name the project; (2) meet the CEO — optionally name it and set its personality, and define the project's Goals (“What is the objective of your company's CEO?”); (3) create the first board, gamified as “Create the first department of your company” — name + description, which the CEO uses for routing; (4) team setup — “Here are your employees — give them a clear role in how they execute their missions”: the three defaults shown, with the option to create more; (5) land on the board. The team panel shows the roster column-by-column with role badges and handles loading / seed-failed (retry CTA) / loaded states; role agents expose instructions editing only (no delete/replace per R1 — replace staffing for custom columns only). Agents are strictly board-scoped (R3): the agents menu and the create-agent flow operate on the current board's team only — no project-wide agent list on the board surfaces (a company-wide roster view is deferred), and no agent transfer between boards. Adding a custom column is a single flow: name the column, pick or create its agent (validation from U2 enforces uniqueness). Tasks parked in the plan-approval hold (R20) surface the attached plan artifact in the task detail with Approve / Send feedback actions (dashboard, TUI, CLI); board creation offers the board-type picker (standard, and Compound Engineering when the plugin is installed per U13). Board creation reuses the team seed (U2) so every new board is born staffed. A "Convert to simple mode" action in project settings runs the U1 conform mapping on demand for legacy/advanced projects (columns onto the company template, extra columns carried as custom columns, team seeded), with a preview of the column mapping before applying.
- **Test scenarios:**
...
- **Upstream branch dependency: `gsxdsm/fixprflow`** (local branch; plan `docs/plans/2026-06-05-001-feat-unified-pr-entity-review-loop-plan.md`, its U1–U6 committed). Carries the unified PR entity (`pull_requests` table, `pr-create`/`pr-respond`/`pr-merge` nodes, reconcile substrate). **Merge it as a Phase-A prerequisite, before U1** — not before U7 — because migration ordering must be settled before any new migration lands. The branch was authored at schema v109 while this branch's baseline is v112, so its `if (version < 109)` block would be silently skipped on every upgraded DB (the data copy and partial unique indexes would never run in the field while fresh DBs look fine). Remedy: re-author the **incoming** PR-entity migration as the next free version on this branch (113), then U1's boards migration lands at 114 and U6's `task_reviewer_runs` at 115 — never renumber this branch's blocks under the incoming one. Ensure `ensurePullRequestsSchemaCompatibility` is reachable on the upgrade path, keep the body re-runnable, and add one combined seed-at-v112 upgrade test exercising PR entity + boards + reviewer runs in sequence, plus the SCHEMA_VERSION-equals-highest-migration invariant after the merge. The textual merge surface is also real — both efforts touch `types.ts`, `workflow-ir-types.ts`, `executor.ts`, `workflow-node-handlers.ts`, `workflow-graph-executor.ts`, `db.ts` — budget the merge as its own step with the engine test suite run after.
...
- AE2. Given a documentation board, when the Executor produces a report.md in its worktree and the Reviewer passes it, the task merges and reaches done through the standard pipeline. (U3 + U6)
...
- AE4. Given a task in todo, when a non-Lead agent moves it to in-review, the move is rejected; a human Todo → In Review drag is also rejected — human drags are limited to Idea ↔ Todo, Done → Archive, Done/Archive → Todo. (U3)
...
- AE6. Given a task in in-review with a pending or failed verdict, no actor — agent or human — can drag it to done; a Manual-required merge completes only through the explicit merge-request affordance. (U6 + U7)
- AE10. Given a Done task, when the user sends it back to Todo, its merge commit is reverted on the integration branch (conflicts surfaced, never forced) and the task re-enters the cycle from Todo. (U3 + follow-up)
