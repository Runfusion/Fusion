...
| `worktreeNaming` | `"random" \| "task-id" \| "task-title" \| "branch"` | `"task-id"` | Naming mode for new worktree directories. Default `"task-id"` keeps FN-258's shipped task-ID directories. `"random"` (adjective-noun) and `"task-title"` (slugified title) affect only the generated name. `"branch"` names the directory after the task's working branch with its namespace dropped and slugified (`feature/PRD-1234-my-slug` → `prd-1234-my-slug`), so a JIRA-derived branch names the checkout after its ticket; an unusable slug (empty, a reserved container name in any case, or a sibling task's live name) falls back to the task id and logs why rather than failing acquisition. `"branch"` is also the mode that names a **workspace** project's task directory — every other mode leaves it on the task id — and its segment is pinned at first workspace acquisition, so a later branch rename never moves recorded paths (see [workspaces.md](workspaces.md#worktree-layout)). `"task-id"` additionally enables **task-pinned worktrees**: each task lives in exactly one derivable directory `<worktreesDir>/<lowercased-task-id>` (e.g. `.fusion/worktrees/fn-7996`) for its whole lifecycle. Acquisition derives → validates → reuses-or-recreates at that same path (never suffixing a sibling name), so a task dispatched N times (kills, requeues, engine restarts, manual Retry) only ever touches that one directory and stale/foreign `task.worktree` metadata self-corrects (emitting `worktree:pin-rederived`) without consuming worktree-session retries. Pinning is bypassed when the worktrunk backend owns layout. |
