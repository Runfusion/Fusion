...
| `useAntigravityCli` | `boolean` | `undefined` | Enables operator-installed `antigravity-cli` models after the bounded `agy` probe succeeds. Toggle it in Settings → Authentication; disabling hides only Antigravity rows and preserves other providers. |
| `antigravityCliBinaryPath` | `string` | `undefined` | Optional global, machine-local `agy` executable override used consistently by Settings, runtime, status probes, and model discovery. Blank values retain PATH detection; a non-empty path is validated before it is saved and failure returns bounded remediation. |
