...
Project-scoped ordered list of up to **four** navigation quick-access destinations, surfaced in Settings as **Navigation quick access**. The key name is retained for backward compatibility: existing persisted preferences stay valid and are simply applied to the shared bottom navigation bar instead of a mobile-only footer. The default is `command-center`, `tasks` (the Board), `planning`, `missions` — `agents` and `mailbox` are not default quick accesses and appear in **More**.

The cap is four rather than five because the bottom bar's fifth quick slot belongs to **Chat**, which is deliberately not configurable: on the shared tablet/desktop footer it is the Chat button of the right-hand group, and on the mobile shell it is the **More** menu entry plus the swipe-up gesture on the bottom bar. A preference persisted with five entries needs no migration — it is simply truncated to its first four eligible entries, and the fifth stays reachable in **More**.

Settings shows selected items in persisted order with move/remove controls and an add dropdown that disables at four entries; the rendered list is itself truncated to four, so a legacy five-entry value never shows an inert fifth row. Edits preview in the live navigation bar and auto-save after editing. Unknown ids, `more`, Terminal/scripts, shell controls, plugin views, and separators are ignored; legacy `documents`/`recommendations` normalize to `mailbox`; duplicates are collapsed; a list longer than four is truncated to its first four eligible entries; and an empty or fully ineligible list falls back to the default. Only destinations with a bottom-bar entry are eligible — `chat`, `notes`, `secrets`, `settings`, `patchnode`, `activity`, `usage`, `projects`, and `ideation` are not, and keep their existing owners. Omitted destinations remain reachable in **More**, whose trailing button is always present, and disabled feature-gated destinations render nowhere until their feature is enabled.
