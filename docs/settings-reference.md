...
| `requestyModelSync` | `boolean` | `true` | Sync Requesty models into model pickers at startup: the managed policies from `https://router.requesty.ai/v1/models/managed` first, then the full catalog from `https://router.requesty.ai/v1/models`. When a Requesty API key is configured it is sent as a bearer token, so the catalog lists only the models approved for your organization; both lists are public, so sync works without one. |
...
| `useAntigravityCli` | `boolean` | `undefined` | Enables operator-installed `antigravity-cli` models after the bounded `agy` probe succeeds. Toggle it in Settings → Authentication; disabling retains an unauthenticated Antigravity status row without probing `agy` during passive authentication refreshes. |
| `antigravityCliBinaryPath` | `string` | `undefined` | Optional global, machine-local `agy` executable override used consistently by explicit Settings readiness/enable checks, runtime, enabled status probes, and model discovery. Blank values retain PATH detection; a non-empty path is validated before it is saved and failure returns bounded remediation. |
...
| `executorAllowSiblingBranchRename` | `boolean` | `false` | Opt into sibling allocation for live checkout conflicts. Independently of this setting, Fusion narrowly reserves a bounded sibling for an engine-owned, unregistered canonical branch with foreign or mixed unmerged history after preserving that original ref; operator branches, live sessions, pauses, and exhausted recovery budgets remain protected. Other conflicts fail safely for operator resolution. See [Task Management → Branch conflict handling](./task-management.md#branch-conflict-handling). The dashboard Settings modal exposes the same toggle with warning copy because broad sibling allocation is discouraged. |
...
| `prTitlePromptInstructions` | `string` | `undefined` | Optional project guidance appended to the Create PR dialog's AI metadata system prompt for the generated PR title. Guidance supplements fresh commit and diff evidence from the task's persisted PR branch; blank or whitespace-only values are treated as unset and keep the default prompt behavior. |
| `prDescriptionPromptInstructions` | `string` | `undefined` | Optional project guidance appended to the Create PR dialog's AI metadata system prompt for generated PR body fields (`summary`, `changes`, `testing`). Guidance supplements fresh task-branch commit and diff evidence. When a repository PR template exists, Fusion completes its sections with generated content rather than copying form instructions into the PR; blank or whitespace-only values are treated as unset and keep the default prompt behavior. |
...
When the Antigravity Runtime plugin (`fusion-plugin-antigravity-runtime`) is installed and `useAntigravityCli` is enabled, `agy models` entries are surfaced additively under `antigravity-cli`. Discovery uses an effective-binary-keyed short negative-TTL single-flight cache; unavailable, unsupported, or unauthenticated discovery returns no Antigravity rows without changing any other provider. Primary Antigravity selection fails with clear remediation when its runtime is unavailable. A fallback-only Antigravity selection is dropped with a warning rather than replacing a healthy primary session, because no safe cross-runtime context transfer is proven.
