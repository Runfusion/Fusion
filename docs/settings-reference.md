...
Custom-provider models are presumed thinking-capable and expose all seven levels by default; no capability checkbox or other configuration is required. Fusion registers these levels with pi as transmissible, while pi owns API-specific `off` translation and up-then-down clamping. Consequently, a global or inherited thinking level that custom providers previously ignored is now sent to the gateway. A strict gateway can reject an unsupported effort; select **Off** for that gateway, which pi translates to its explicit non-reasoning form or omits when the API has no such form. RUFU-143 adds per-model controls to the dashboard's custom-provider row editors (Settings → Models and the legacy Model Onboarding form): a **Thinking format** select (blank = default; the UI-safe pi-ai values `qwen-chat-template`, `qwen`, `zai`, `deepseek`, `string-thinking`, `openrouter`, `together`, `ant-ling`) and a **No thinking params** checkbox. The select pins pi's `compat.thinkingFormat` for OpenAI-compatible models — for example, Qwen3 behind LiteLLM then receives `chat_template_kwargs.enable_thinking` instead of the `reasoning_effort` parameter it rejects. The checkbox (`reasoning: false`) opts the model out of reasoning/thinking parameter sending entirely; the checkbox wins over the select (the select is disabled while it is checked), and the model picker then offers only **Off** for that model, because pi derives its `supportedThinkingLevels` as `["off"]`. A row left at the default and unchecked persists nothing extra, so existing provider registrations remain byte-identical. See the [`customProviders`](#customproviders) entry for the full field reference.
...
| `customProviders` | `CustomProvider[]` | `[]` | <a id="customproviders"></a>User-defined OpenAI-compatible, OpenAI Responses API (`apiType: "openai-responses"`), Anthropic-compatible, or Google Generative AI (`apiType: "google-generative-ai"`) providers used by the custom-provider API (`/api/custom-providers`). Each entry uses `{ id, name, apiType, baseUrl, apiKey?, supportsDeveloperRole?, anthropicPromptCaching?, models? }`; `models` entries use `{ id, name, contextWindow?, maxTokens?, timeoutSeconds?, thinkingFormat?, reasoning? }`, where `contextWindow`/`maxTokens` (RUFU-123) are optional positive-number per-model window overrides — an omitted, zero, negative, or non-numeric value is replaced at registration with the registry defaults `128000`/`16384`, so legacy entries round-trip unchanged. `timeoutSeconds` is an optional per-model HTTP idle/first-byte timeout in seconds — the maximum silence allowed (request to first byte, and between streamed chunks), NOT a total-generation cap, so a model that streams continuously (even slowly) never trips it. Omitted/invalid keeps the current 300s default; `0` disables the timeout for that model. It is applied at two layers: the per-session OpenAI SDK first-byte timeout (the engine writes the resolved model's `timeoutMs` into the per-session pi SettingsManager `retry.provider.timeoutMs`, where a user `0` is stored as `2147483647` because the SDK aborts immediately on a literal 0) and the engine's process-global undici dispatcher, which carries the per-origin `bodyTimeout`/`headersTimeout` for every custom-provider origin — on a shared origin the most permissive configured value wins (disabled beats positive), and unlisted origins keep the 300s default. Re-saving a provider re-installs the dispatcher for new connections. Set it for local slow/buffered models that would otherwise die with the classic `Request timed out.` (openai `APIConnectionTimeoutError`) at the 5-minute mark. The compaction gate and pi's threshold compaction reason from the registered windows, so set them explicitly for models whose true window is far below 128K. `thinkingFormat` (RUFU-143) is an optional per-model pi-ai thinking-format discriminator persisted into the provider's `models` entries and registered as pi's `compat.thinkingFormat` for that model — the UI-safe values are `qwen-chat-template`, `qwen`, `zai`, `deepseek`, `string-thinking`, `openrouter`, `together`, and `ant-ling` (omitted/blank is the default), and `chat-template` (raw; requires an accompanying `chatTemplateKwargs`) is accepted from manual `models.json` edits only because the dashboard select cannot expose its required kwargs. The `openai` value is deliberately not exposed either because it is identical to the omitted default, and `baseten` (raw; requires `chatTemplateArgs`) stays manual-only for the same reason. `reasoning` (RUFU-143) is an optional per-model boolean: `false` is an opt-out that stops pi-ai from sending reasoning/thinking parameters for that model (the lane's reasoning-effort setting is not sent), while omitted/`true` keeps the default parameter-sending behavior. Both flags round-trip through the custom-provider API, and a model without either flag round-trips unchanged (byte-identical registration). `supportsDeveloperRole` is an OpenAI-compatible opt-in that enables `developer` role emission (default/omitted is `false`, forcing safe `system` role). `anthropicPromptCaching` (FN-7689) is an opt-in for `openai-compatible`/`openai-responses` gateways that proxy an Anthropic-format backend (for example a self-hosted router fronting Claude models); when `true`, Fusion registers that provider's `openai-completions` models with pi-ai's `compat.cacheControlFormat = "anthropic"`, so pi-ai attaches Anthropic-style `cache_control` breakpoints to the system prompt, the last conversation message, and the last tool definition, letting the gateway serve cached-prefix reads/writes instead of re-billing the full context every turn. Default/omitted is `false` — leave it off for gateways that do not understand `cache_control` (e.g. Together, Fireworks) to avoid provider 400s. It is inert (a documented no-op) for `anthropic-compatible` providers, which already auto-cache without any flag, and for `google-generative-ai` providers, which have no `cache_control` concept. Enable it from Settings → Authentication → Advanced: Custom Providers via the "Enable Anthropic-style prompt caching" checkbox shown for OpenAI-compatible/OpenAI Responses provider entries. API keys are stored raw but masked in API responses. Fusion resolves these providers from the active global settings directory (`~/.fusion`, with legacy `~/.pi/fusion` and `~/.pi/kb` migration support) so custom-provider models remain available after restart. Dashboard, serve, and daemon startup refresh each configured provider's persisted `models` list from its `/models` endpoint on a best-effort basis; failures leave the previous list intact and do not block startup. In Settings → Authentication → Advanced: Custom Providers, use **Refresh Models** on a provider row to manually refresh that provider after changing credentials, endpoints, or upstream model availability. Saved local/LAN/internal provider URLs are eligible for this stored-provider refresh path, while the add/edit **Detect Models** form keeps stricter SSRF protections for untrusted one-off input. The refresh path auto-detects per-model `contextWindow` for local Ollama, LM Studio, and vLLM backends (see the [dashboard guide](./dashboard-guide.md) for the per-backend sources); `maxTokens` is never inferred from the context window. |
...
| `codeReviewMaxRevisions` | unset | Workflow-native Code Review remediation cap. Unset/empty uses the workflow's authored default (standard built-ins: 3; Compound Engineering: 2); a non-negative integer overrides the cap; `0` disables automatic Code Review remediation; a stored `"unbounded"` value explicitly removes the bound. |
...
| `groupOverlappingFiles` | `boolean` | `true` | Serialize execution when file scopes overlap until the holder's work has landed: WIP holds from dispatch, review and other non-terminal lanes hold while they retain a worktree, and terminal/cleaned work releases the claim. |
...
| `worktreeInitCommand` | `string` | `undefined` | Authoritative bootstrap command for every task worktree. When set, it suppresses inferred rows; a zero exit is recorded as installed and also covers otherwise unrecognised dependency evidence. A failure is recorded as unresolved, does not fail acquisition, and blocks Plan Review until repaired or escalated. The merge clean-room continues to use its existing dependency-sync path. |
...
Readiness is stored outside the repository at `<private-git-dir>/fusion-dependency-install.json`. It resolves in strict precedence as `unresolved`, `unrecognized`, `satisfied`, or `not-needed`. `not-needed` means there is no manifest or evidence at all and no command is spawned. A matching successful record avoids redundant work; changed manifest or evidence content re-arms the relevant row.
...
| `autoArchiveDuplicateTasksEnabled` | `boolean` | `false` | FN-7658/FN-8401: gates whether same-agent duplicate intake on every create backend auto-archives the later/new task. Default `false` — the duplicate is flagged in place (`nearDuplicateOf`/`nearDuplicateScore` marker, yellow "Duplicate" chip with a clear-the-flag control and Archive action), and no live sibling is deleted or archived automatically. Set `true` to restore opt-in archival of the new task only. Does not affect ghost-bug preflight or tombstone-resurrection blocking. |
...
| `memoryPerTurnRecallEnabled` | `boolean` | `true` | When enabled, a per-turn proactive memory recall runs before each chat or task step prompt: it derives 2–3 content keywords from the current topic, searches the memory backend via `MemoryBackend.search`, and injects a deduped, client-side score-filtered top-K cue block (≤800 chars) under the pre-steering marker. On by default; silent skip on empty/stopword-only topics, fully-deduped sessions, or unavailable/unsupported backend search. Only used when `memoryEnabled` is true. |
| `memoryPerTurnRecallTopK` | `number` | `3` | Top-K for the per-turn memory recall cue block; clamped to 1–10 by the core module. Only used when `memoryPerTurnRecallEnabled` is true. |
| `chatPreOverflowCompactionEnabled` | `boolean` | `true` | Enables the deterministic pre-overflow compaction gate for chat: the session context is measured before every model-loop LLM call and compacted (a single compaction call through the session compaction entry point, re-measured afterward) once it reaches ~80% of the model's context window, clamped by the `tokenCap` upper bound; if the context still sits at/above the hard limit after compaction, the call is not sent and a `CHAT_CONTEXT_OVERFLOW` error surfaces. On by default; project-scoped runtime setting (Settings → Memory → "Pre-overflow compaction guard") that takes effect without a restart. When disabled, chat relies on pi's threshold auto-compaction, which never fires for zero-usage providers (e.g. dsai1) — see `docs/solutions/logic-errors/chat-pre-overflow-compaction-blindness.md`. |
| `chatContextBudgetEnabled` | `boolean` | `true` | Bounds the chat static context so agent chat fits 64K-window models: oversized project memory is injected as a bounded heading index instead of the full content, and chat sessions use the curated chat toolset instead of the full tool list. On by default; project-scoped runtime setting (Settings → Memory → "Chat context budget (64K window fit)") that takes effect without a restart. When disabled, restores pre-RUFU-135 unbounded memory injection and the full tool set. |
...
The dashboard's Settings → Models custom-provider editors expose the equivalent
per-model controls (the **Thinking format** select and **No thinking params**
checkbox; see the [`customProviders`](#customproviders) entry). They persist
only the `thinkingFormat` discriminator and the `reasoning` opt-out —
`chatTemplateKwargs`/`chatTemplateArgs` remain a `models.json`-only escape
hatch for the raw `chat-template`/`baseten` formats, which require explicit
kwargs the dashboard form cannot express.
