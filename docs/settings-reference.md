...
<!-- FNXC:LiquidGlassTheme 2026-09-09-15:50: `liquid-glass` is a persisted theme choice with adaptive light/dark functional glass and standard content materials; accessibility preferences can reduce its motion, transparency, and decorative depth without changing the stored ID. -->
...
| `defaultThinkingLevel` | `"off" \| "minimal" \| "low" \| "medium" \| "high" \| "xhigh" \| "max"` | `undefined` | Default reasoning effort for AI sessions. `xhigh` and `max` are accepted canonical values; runtime adapters forward the selected value according to the active provider/model. If a provider/runtime rejects simultaneous `thinking` and `reasoning_effort` parameters, Fusion retries without the explicit thinking override instead of failing the run. |
...
| `autoRestartAfterUpdate` | `boolean` | `undefined` (effective `false`) | After a successful dashboard install, requests the existing supervised restart. It does not initiate an install by itself; unsupervised hosts retain manual restart guidance. It is shown in **Settings → General → Automatically restart after an update**. |
| `autoUpdateAndRestart` *(hidden compatibility)* | `boolean` | `false` | Deprecated persisted fallback, not a current Settings control. Only when either new key is unset does legacy `true` supply that behavior; an explicit new `false` wins. The effective matrix is: false/false no background install; true/false install then wait for manual restart; true/true install then request restart; false/true does not initiate background installation but may restart after a successful manual dashboard install. All paths honor `updateCheckEnabled` and the selected stable/beta channel. |
...
| `jiraEnabled` | `boolean` | `undefined` (effective `false`) | Global opt-in for JIRA branch-name derivation. Project values override this fallback. Dashboard location: **Settings → Source Control · Global → JIRA Configuration**. |
| `jiraBaseUrl` | `string` | `undefined` | Global JIRA web base URL, such as `https://acme.atlassian.net`. Required when enabled; project values override it. |
| `jiraApiBaseUrl` | `string` | `undefined` (effective `<base>/rest/api/3`) | Optional global API base override. Use `<base>/rest/api/2` for JIRA Data Center or Server. |
| `jiraAuthEmail` | `string` | `undefined` | Optional global Atlassian account email. When present Fusion uses Basic email/API-token auth; otherwise it uses Bearer PAT auth. |
| `jiraAuthTokenSecretKey` | `string` | `"JIRA_API_TOKEN"` | Global secrets-store key for the JIRA API token or PAT. This is a reference, never a token value. |
...
| `persistAgentToolOutput` | `boolean` | `true` | Persists detailed tool arguments and successful `tool_result` payloads by default; failed `tool_error` detail always persists as bounded diagnostic signal. Tool timeline rows are stored either way. Existing installs with an explicitly saved `false` retain the previous low-volume behavior until an operator enables the setting. |
...
- **Values** — the per-project values for the workflow that is open. Values are
  ...
  enter `0` to disable automatic revision for that path. Every automatic workflow
  review is finally bounded by the shared absolute backstop of 8 revisions;
  `"unbounded"` means only that no lower configurable limit applies. The separate
  `planReviewReplanCap` can impose a stricter Plan Review ceiling. Leave it empty
  to use its legacy fallback, or set a lower non-negative integer (`0` parks on
  the first REVISE). Fusion always chooses the lowest applicable limit and parks
  visibly without another model dispatch when it is exhausted. `plannerOversightLevel` is the
...
| `reviewerInlineFixes` | `false` | Reviewers judge without repairing by default and return findings to executor remediation. Set to `true` to restore same-session reviewer edits for this workflow. |
| `planReviewMaxRevisions` | unset | Workflow-native Plan Review/spec revision cap. Unset/empty uses the finite absolute backstop; a lower non-negative integer caps attempts; `0` disables automatic Plan Review revision. |
| `codeReviewMaxRevisions` | unset | Workflow-native Code Review remediation cap. Unset/empty uses the workflow's authored default (standard built-ins: 3; Compound Engineering: 2); a lower non-negative integer overrides it; `0` disables automatic remediation; `"unbounded"` selects the finite absolute backstop. |
...
| `codeReviewBlockingSeverity` | `critical` | Same gate for Code Review, defaulting to `critical` (P0 only) because Code Review findings land against real code the executor can address through named remediation. Same values and the same fail-closed contract as `planReviewBlockingSeverity`. |
| `planReviewReplanCap` | unset (legacy fallback 15) | Additional ceiling on consecutive Plan Review `REVISE` → replan cycles before the task parks at `awaiting-approval` with reason `plan-review-replan-cap`. Fusion chooses the lowest of this setting, the resolved revision budget, and the absolute backstop of 8; `0` parks on the first REVISE. |
...
| `maxConcurrent` | `number` | `2` | Provider/LLM-load ceiling over every top-level AI-active task across planning, execution, and review/merge. Range **1–50** in Settings, Command Center, and Engine Control. Independent from `maxWorktrees`: checkout-free planning still consumes this agent budget. |
...
<!-- FNXC:TaskRecommendations 2026-08-19-13:05: Document the project-only required-evaluation switch, its cap-zero precedence, and quality-over-count contract beside the recommendation cap. -->
| `requireTaskRecommendations` | `boolean` | `false` | Project-only opt-in. When `true` and `maxRecommendationsPerTask` is positive, every accepted successful completion must explicitly submit a recommendation array. The executor aims toward the cap using grounded, distinct, task-ready findings, but fewer recommendations or `[]` is correct when relevance does not support more; filler, duplicates, restatements, speculation, and scope drift are never valid. Cap `0` remains authoritative and disables capture, so this toggle cannot request a payload. Reset clears the project override back to `false`. |
...
| `maxWorktrees` | `number` | `4` | Host-resource ceiling over live tasks holding or entering execution checkouts. WIP reservations count before path persistence; live review/replan cards with retained singular or workspace checkouts continue to count. Checkout-free planning, merge claims, and same-task workflow continuations do not claim a new worktree slot. Independent from `maxConcurrent`; reporting surfaces expose both values separately. |
| `worktreeLimitEnabled` | `boolean` | `true` | When false, removes the execution-worktree gate structurally. It does not change write-capable execution isolation, and planning remains checkout-free. |
...
| `aiUndoTaskWorkflowId` | `string` | `"builtin:review-heavy"` | **FN-7556 / FN-7578.** Workflow selected for AI-undo board tasks created by `POST /api/tasks/:id/revert` (`mode: "ai"` and the `auto`/workspace conflict fallbacks) — a stricter review posture since these tasks reverse already-shipped code. Blank/unset means the created task inherits the project default workflow. The route validates the configured id and falls back to inherit on a blank or unknown value, so a misconfigured id never breaks AI-undo task creation. Editable from **Settings → General** ("AI-undo task workflow" picker, next to the workflow-enablement controls); choose "Inherit project default workflow" to store the blank/inherit sentinel. See [Task Management → Reverting completed tasks](./task-management.md#reverting-completed-tasks-git-path--ai-undo-fallback). |

| `groupOverlappingFiles` | `boolean` | `true` | Serialize execution when file scopes overlap until the holder's work has landed: WIP holds from dispatch, review and other non-terminal lanes hold while they retain a worktree, and terminal/cleaned work releases the claim. After the durable release, a coalesced scheduler wake automatically re-evaluates waiting tasks; periodic polling remains a recovery backstop. |
...
| `autoMerge` | `boolean` | `true` | Auto-finalize tasks from `in-review`. **FN-514:** this setting is unchanged and still governs the AUTOMATIC lanes; it is distinct from the per-task delivery lock and from the human **Merge** command. Arming a delivery lock never disables it, removing one never enables it, and choosing **Create PR** never turns it on. A locked card waits for an explicit operator command even when this setting is `true`; an unlocked card is unaffected. The former review-column Auto-merge toggle was removed — this project setting was not, and remains editable in Settings → Merge. Tasks can override this per-task (including at create time in New Task modal via **Auto-merge** = Default/Enabled/Disabled); explicit overrides are tagged with `autoMergeProvenance: "user"`, while tasks left at **Default** keep following the live global setting and do not snapshot it when entering review. Legacy pre-FN-6245 in-review rows that were stamped `autoMerge: true` are marked `autoMergeProvenance: "legacy-stamp"` on startup and can be inspected/cleared with Settings → Merge → **Legacy auto-merge stamp cleanup**, `fn pr automerge-cleanup [--apply] [--json]`, or `reconcileLegacyAutoMergeStamps({ apply: true })` after operator review. For grouped branch flows, per-task `autoMerge` governs member→group-integration landing while group `autoMerge` governs group→default-branch promotion eligibility. |
...
| `integrationBranch` | `string` | `undefined` | Optional canonical project integration branch override. A configured `integrationBranch` (then legacy `baseBranch`) remains authoritative. When neither is configured, merge/self-healing/branch-conflict defaults resolve `origin/HEAD`, then usable local branches (`main`, `master`, `trunk`, `develop`, current HEAD, or one non-Fusion branch), then an unambiguous `refs/remotes/origin/*` candidate, and finally fallback `"main"`. Project registration applies the same ladder and ensures the selected ref is local: it adopts an existing local branch, creates one from an `origin` remote-tracking ref, or creates it at `HEAD`. Remote tracking never overrides a usable local branch. Auto-detect still never chooses an arbitrary non-`origin` remote; if no candidate exists, Fusion logs the existing one-time remote diagnostic. This resolved value is used as `projectDefaultBranch` for `resolveTaskMergeTarget(...)`; task-level overrides still come from task metadata. |
| `prerebaseAutoEnabled` | `boolean` | `true` | **Legacy and inert (master-plan U0)** — retained published surface, consumed only by soft-deprecated aiMergeTask in `packages/engine/src/merger.ts`; production runAiMerge never reads it. The disabled check is strict `=== false`, so absent means enabled; worktrunk.enabled returns worktrunk-deferred before the disabled check. `false` is the only opt-out from safety-fallback-any-divergence. |
| `prerebaseHotFiles` | `string[]` | `[`"AGENTS.md"`, `"packages/core/src/store.ts"`, `"packages/core/src/db.ts"`, `"packages/engine/src/executor.ts"`, `"packages/engine/src/scheduler.ts"`, `"packages/engine/src/merger.ts"`, `"packages/dashboard/app/styles.css"`]` | **Legacy and inert (master-plan U0)** — retained published surface, consumed only by soft-deprecated aiMergeTask in `packages/engine/src/merger.ts`; production runAiMerge never reads it. The legacy range was task.baseCommitSha to refs/heads/ integration-branch tip, resolved by git rev-parse --verify; deliberately not the rootDir HEAD. git diff --name-only paths used exact full-path equality, no globs or prefix matching; a match fired before the threshold check. |
| `prerebaseDivergenceThreshold` | `number` | `50` | **Legacy and inert (master-plan U0)** — retained published surface, consumed only by soft-deprecated aiMergeTask in `packages/engine/src/merger.ts`; production runAiMerge never reads it. Evaluated after the hot-file check, it fired when threshold > 0 and commitsBehind >= threshold. An absent field fell back to ?? 1 despite shipped project default 50. `0` is not a full opt-out: it suppresses divergence-threshold only; safety-fallback-any-divergence still fires for any behind branch. |
...
| `worktreeRebaseRemote` | `string` | `""` | Optional remote-name override for worktree rebase and remote-enabled workspace integration; blank resolves each repository's actual branch, sole, or existing `origin` remote. Repositories without remotes remain local-only. See [Worktree and pre-merge rebase settings](#worktree-and-pre-merge-rebase-settings). |
...
Remote selection first honors a non-empty `worktreeRebaseRemote`. For workspace landing, a configured name missing from a repository and multiple non-`origin` remotes are actionable environment failures; Fusion does not invent a remote. For integration work, a blank value then uses `git config branch.<integrationBranch>.remote` before repository discovery. Executor creation paths instead prefer `origin`, then use the sole discovered remote. A repository with no remote skips the remote rebase and uses the local dependency-base fallback; a repository with one non-`origin` remote can use that sole remote, while repositories with multiple non-`origin` remotes need an explicit setting or integration-branch tracking configuration.
...
| `pushAfterMerge` | `boolean` | `false` | Publish successful integration refs after a merge. It gates both direct and multi-repository workspace landings; pull-request strategy is excluded only for direct merges because PR mode publishes its task branch separately before PR creation. |
| `pushRemote` | `string` | `"origin"` | Git remote target used by direct merges when `pushAfterMerge` is enabled. Accepts `remote` (for example `origin`) or `remote branch` (for example `upstream main`). Empty/unset values fall back to `origin` plus the resolved merge integration branch. |
...
In a multi-repository workspace, `pushAfterMerge` controls each sub-repository independently. When it is off, every member lands only on its local integration ref and writes no remote refs. When it is on, Fusion resolves each member's target remote in this order: configured `worktreeRebaseRemote`, the integration branch's configured remote, the sole remote, then `origin`. `pushRemote` applies only to direct merges. Workspace tasks always land directly, so the pull-request exclusion does not suppress their configured publication.
| `worktreeInitCommand` | `string` | `undefined` | Authoritative bootstrap command for every task worktree. When set, it unconditionally suppresses inferred rows, including metadata-aware uv inference, and is the recommended resolution for a uv `configuration-required` refusal. A zero exit is recorded as installed and also covers otherwise unrecognised dependency evidence. A failure is recorded as unresolved; a repeated deterministic failure against unchanged worktree evidence becomes `config-blocked` and exposes Retry. Editing this setting, changing worktree contents, or Retry lifts the block without recreating the task. The merge clean-room continues to use its existing dependency-sync path. |
| `worktreeCopyFiles` | `string[]` | `[]` | Repository-root-relative regular files to copy into each newly assigned non-resume task worktree. Configure from Settings → Worktrees with editable rows or Browse (useful for `.env`-style files). Fusion copies these files after fresh creation and before `worktreeInitCommand`, secrets-env materialization, and task execution. Blank/duplicate entries are ignored; absolute paths, `..` traversal, missing files, directories, and unreadable/non-regular sources are skipped as non-fatal task-log/audit diagnostics without logging file contents. Resume/existing worktrees are not overwritten. |
...
Fusion evaluates dependency readiness independently for every fresh task worktree, including every configured workspace repository. The bounded root-only matrix recognizes Node lockfiles/package manifests (`pnpm`, npm, Yarn, Bun), `uv.lock`, `poetry.lock`, `Pipfile.lock`, `requirements.txt`, Cargo, Go, Composer, Bundler, .NET projects, Maven, Gradle, Mix, Dart/Flutter, and Swift. Each row has one deterministic command and runs only when its toolchain is available on `PATH`; each command has a 300-second timeout and the complete worktree pass has a 600-second budget.

The detector also reports `unrecognized` evidence rather than assuming it is dependency-free: named files include Nix, Bazel, Conan, vcpkg, Haskell, Perl, R, Julia, Zig, sbt/Clojure, CocoaPods, Conda, Crystal, Rebar, and Dub manifests; an open-ended root-level shape rule covers unexplained `*.lock`, `*.lockb`, `*-lock.json`, `*-lock.yaml`, `*.cabal`, `*.nimble`, `*.opam`, `*.podspec`, and `*.gemspec` files. Plain build files such as `Makefile`, `CMakeLists.txt`, and `meson.build` are not dependency evidence.

Readiness is stored outside the repository at `<private-git-dir>/fusion-dependency-install.json`. It resolves in strict precedence as `config-blocked`, `unresolved`, `unrecognized`, `satisfied`, or `not-needed`. `not-needed` means there is no manifest or evidence at all and no command is spawned. A matching successful record avoids redundant work; changed manifest or evidence content re-arms the relevant row.

At planning time, unresolved or unrecognised repositories add a dependency-installation instruction. The planner uses `fn_install_worktree_dependencies`: `install` runs its supplied command through Fusion's sandbox-routed command seam and records success only from exit code 0; `none` records a mandatory reason for genuinely non-installable evidence. This planning-only command tool is not available to readonly, executor, reviewer, or merger sessions.

Plan Review re-runs deterministic rows once before dispatch. If any worktree remains `unresolved` or `unrecognized`, it returns the ordinary Plan Review `REVISE` whose output begins `Dependencies are not installed.` The normal Plan Review revision budget and `planReviewReplanCap` bound that loop; exhaustion parks at `awaiting-approval` with `awaitingApprovalReason: "plan-review-replan-cap"`. A probe that cannot determine readiness is logged as `not-determined` and does not block review.

#### Worktree backend settings

| Setting | Type | Default | Description |
| --- | --- | --- | --- |
| `worktreesDir` | `string` | `undefined` | Optional container directory for task-ID worktrees. When unset, new native worktrees use `<projectRoot>/.fusion/worktrees`; an existing `<projectRoot>/.worktrees` root remains valid for containment and cleanup sweeps without migrating recorded paths. Supports absolute paths, project-relative paths, `~` expansion, and `{repo}` token substitution. For workspace projects, a configured root groups native member checkouts as `<root>/<workspace>/<repo>/<lowercased-task-id>`; safe workspace names are preserved and unsafe/nested segments gain deterministic hash suffixes. Equal workspace basenames sharing one external root fail closed rather than collide. When `worktrunk.enabled` is `true`, worktrunk-managed layout takes precedence. |
...
- `worktreesDir`: Default: `<projectRoot>/.fusion/worktrees`; Fusion names native task worktrees with the lowercased task ID. While unset, an existing `<projectRoot>/.worktrees` root remains honored and swept; recorded paths are never migrated.
...
| `pausedScopeDecayMs` | `number` | `1800000` | Minimum pause age in ms before self-healing can rebound a paused `in-progress` scope-holder back to `todo` when it is actively blocking at least one follower via `blockedBy`/`overlapBlockedBy`. A paused holder keeps its file-scope claim while it owns unfinished work; this setting governs only the existing recovery path. Uses `columnMovedAt ?? updatedAt` as the pause-age proxy. Set `0` to disable decay-based rebound. |
...
| `maxPostReviewFixes` | `number` | `10` | Default max automatic fix passes for generic review/pre-merge optional-step feedback, including self-healing recovery. Lower node/workflow budgets win; `"unbounded"` and values above 8 are clamped to the shared absolute backstop. Code Review still uses its workflow value and authored default before this fallback. |
...
| `jiraEnabled` | `boolean` | `undefined` (inherits global, then `false`) | Project opt-in for JIRA branch-name derivation. Dashboard location: **Settings → Source Control · Project → JIRA Configuration**. |
| `jiraBaseUrl` | `string` | `undefined` (inherits global) | Project JIRA web base URL. Blank clears the project override and restores global inheritance. |
| `jiraApiBaseUrl` | `string` | `undefined` (inherits global, then `<base>/rest/api/3`) | Optional project JIRA API base override; use `/rest/api/2` for Data Center or Server. |
| `jiraAuthEmail` | `string` | `undefined` (inherits global) | Project email selecting Basic email/API-token auth; blank inherits the global setting. |
| `jiraAuthTokenSecretKey` | `string` | `undefined` (inherits global, then `"JIRA_API_TOKEN"`) | Project secrets-store key reference for the JIRA token or PAT; no plaintext token is stored in settings. |
| `jiraAuthTokenSecretScope` | `"project" \| "global"` | `undefined` (inherits global, then `"project"`) | Preferred project secret scope; a project-scope miss falls back to global. |
| `jiraBranchNameTemplate` | `string` | `undefined` (inherits global, then `"feature/{key}-{summary}"`) | Project template for the editable JIRA-derived workspace branch candidate. |
...
| `agentLogFileRetentionDays` | `number` | `0` | Retention window for per-task `.fusion/tasks/{ID}/agent-log.jsonl` files after a task is soft-deleted or remains in the historical sentinel. Periodic maintenance removes JSONL entries older than this many days; active tasks are never pruned. Set `0` to disable pruning. |
...
- **Anthropic Subscription** (`anthropic-subscription`) is Claude subscription OAuth. It powers login/logout, `/api/auth/status`, usage/subscription checks through `https://api.anthropic.com/api/oauth/usage`, and the OAuth re-login banner when no authenticated Anthropic API-key/CLI fallback is present. It is **also an execution surface**: subscription OAuth resolves for the direct `anthropic` provider at runtime, so `anthropic/*` selections run on `https://api.anthropic.com/v1` with Fusion-owned Claude Code identity headers (Bearer OAuth, no API key required). Fusion pins this identity to Claude Code `2.1.251`; set `FUSION_ANTHROPIC_CLAUDE_CODE_VERSION` to a valid `major.minor.patch` value only when Anthropic raises a requirement before a Fusion release. Anthropic gates some models, including `claude-fable-5-1`, on a minimum client version, so Fusion’s pinned identity must be raised alongside any newly added gated model. When any subscription credential exists, it displaces a legacy bare `anthropic` OAuth row entirely; that legacy row is used only for installs with no subscription credential at all.
...
- **Anthropic API Key** (`anthropic` direct `/v1`) is raw API-key auth. It accepts `ANTHROPIC_API_KEY`, a `models.json` `apiKey`, or an `api_key` auth credential and uses `x-api-key`. Set global [`anthropicAuthPreference`](#anthropicauthpreference) to choose whether this raw key or subscription OAuth wins when both are configured; it cannot make a legacy bare OAuth row override a stored subscription account.
...
| `triageDuplicateResolution` | `"prompt" \| "keep" \| "delete"` | `"prompt"` | Controls `DUPLICATE: FN-NNNN` markers emitted during triage. **prompt** flags and system-pauses the task until an operator clears the flag or deletes the task; the existing decision banner links to the canonical task. **keep** dismisses the marker and replans a real task. **delete** restores legacy auto-delete behavior. |

### `mobileNavPrimaryItems`

Project-scoped ordered list of up to **five** navigation quick-access destinations, surfaced in Settings as **Navigation quick access**. The key name is retained for backward compatibility: existing persisted preferences stay valid and are simply applied to the shared bottom navigation bar instead of a mobile-only footer. The default is `command-center`, `tasks` (the Board), `planning`, `missions`, `chat` — `agents` and `mailbox` are not default quick accesses and appear in **More**.

The cap is five because that is the number of slots the bar renders: four in the centred group plus the far-right slot, which the **fifth** resolved destination occupies. `chat` is an ordinary eligible destination, so the far-right slot is configurable; the mobile pill renders exactly the same five destinations, in the same order.

Resolution completes any shorter valid selection up to five from the default order, preferring its tail (which ends with `chat`), and then appends the chosen destinations in default order. This is a render-time rule only: the persisted value is never rewritten and no migration exists, so a preference saved with four destinations keeps rendering Chat in the fifth slot. The only way to keep `chat` out of the bottom bar is to define five destinations explicitly, after which `chat` becomes a **More** entry. A resolved destination is never also listed as omitted, so no destination can be rendered twice on one host.

Settings shows selected items in persisted order with move/remove controls and an add dropdown that disables at five entries; the rendered list is itself truncated to five, so an oversized persisted value never shows an inert sixth row. Edits preview in the live navigation bar and auto-save after editing. Unknown ids, `more`, Terminal/scripts, shell controls, plugin views, and separators are ignored; legacy `documents`/`recommendations` normalize to `mailbox`; duplicates are collapsed; a list longer than five is truncated to its first five eligible entries (with no completion); and an empty or fully ineligible list falls back to the default. Only destinations with a bottom-bar entry are eligible — `notes`, `secrets`, `settings`, `patchnode`, `activity`, `usage`, `projects`, and `ideation` are not, and keep their existing owners. Omitted destinations remain reachable in **More**, whose trailing button is always present, and disabled feature-gated destinations render nowhere until their feature is enabled.

### `mobileNavMenuSwipeGesture`

Project-scoped boolean, default `false`, surfaced in Settings as **Open the mobile menu with a swipe**. When enabled, the mobile bottom bar renders no menu button at all (no empty shell and no orphaned `aria-controls`/`aria-expanded`) and the navigation menu opens by swiping the bar upwards, presented as a drawer exactly as wide as the bar and starting at its top edge. When disabled, the menu button is rendered and the gesture is disarmed. The gesture is touch-only, stays inert while the menu is already open, and has no effect on the wide desktop bar. It replaces the retired “swipe up to open Chat” shortcut.
...
Settings → Authentication can hold multiple named credential accounts for each non-CLI provider. Select **Add another account** to create a client-generated account id, optionally label it, and complete OAuth or save an API key; an abandoned pending account is not stored. An instance login stores the credential it produced directly in that account. If it authorizes an account Fusion already held before that login started, the Authentication card names that existing account and asks you to sign out in the browser and retry; storage is left unchanged. First logins and re-authorizing the same instance are never refused. The first credential becomes the provider default; later accounts do not change it. Authentication actions without a selected named account target that provider default, while actions on a named account retain its instance id through login, cancellation, logout, save, and clear. Operators can rename, remove, or make an existing account default. Labels are display-only, optional, and need not be unique. CLI-backed provider cards retain their own credential handling and do not support Fusion credential instances.
...
### AI-authored task output language

`taskOutputLanguage` is a project setting with `"english"` (default), `"input"`, and `"interface"` modes. English explicitly targets English; input follows the original task text (with detection only as a hint); interface follows the global Fusion UI `language`, falling back to English when unset. Existing `taskDefinitionInInputLanguage: true` projects resolve as input until an explicit new mode is saved; explicit modes clear that legacy flag atomically. The setting affects future planning, triage, title, execution, workflow-summary, and recommendation generation sessions only, never existing task text. Chat session auto-titles apply the same resolved output language in a background operation, so sending a chat message does not wait for title generation or settings resolution. The immediately following **Auto-summarize task titles** project toggle snapshots this selected language at task creation for every non-empty untitled description when enabled; no description-length threshold controls the policy. When disabled or generation is unavailable, titleless Board and List cards use bounded presentation-only fallback without persisting a description-derived title. Human prose is targeted while headings, JSON keys, markers, code, paths, tool names, and schemas remain canonical.

### JIRA branch naming

JIRA branch derivation is opt-in. Configure `jiraEnabled`, `jiraBaseUrl`, optional `jiraApiBaseUrl`, `jiraAuthEmail`, `jiraAuthTokenSecretKey`, `jiraAuthTokenSecretScope`, and `jiraBranchNameTemplate` in Settings → Source Control · Global or Source Control · Project. Project values override global values. Cloud defaults to `/rest/api/3`; self-managed installations may set `/rest/api/2`. An email selects Basic email/API-token authentication, otherwise Fusion uses Bearer PAT authentication. The account requires JIRA issue read permission. Fusion never stores tokens in settings and never invokes a JIRA CLI.

## Update automation

`autoUpdateEnabled` and `autoRestartAfterUpdate` are global settings and both default to `false`. Automatic installation follows the selected stable or beta channel and respects `updateCheckEnabled` plus `FUSION_UPDATES_EXTERNALLY_MANAGED` when the deployment owns releases. With install/restart off no background installation occurs; install on/restart off installs once and waits for a manual restart; both on installs then requests a supervised restart; install off/restart on only restarts after a successful dashboard installation.
...
`reviewArbitrationEnabled`, `reviewArbitrationProvider`, and `reviewArbitrationModelId` control the arbitration stage for repeated unchanged feedback before a budget is exhausted. A numeric limit, `planReviewReplanCap`, or the shared absolute backstop is a final dispatch boundary: exhaustion parks the task visibly and never schedules another remediation, reviewer, or arbitration model call.
