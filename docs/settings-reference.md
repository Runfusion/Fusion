...
| `chatPreOverflowCompactionEnabled` | `boolean` | `true` | Enables the deterministic pre-overflow compaction gate for chat: the session context is measured before every model-loop LLM call and compacted (one 50% pass, then a 10% pass if needed) once it reaches ~80% of the model's context window, clamped by the `tokenCap` upper bound; if the context still sits at/above the hard limit after compaction, the call is not sent and a `CHAT_CONTEXT_OVERFLOW` error surfaces. On by default; project-scoped runtime setting (Settings → Memory → "Pre-overflow compaction guard") that takes effect without a restart. When disabled, chat relies on pi's threshold auto-compaction, which never fires for zero-usage providers (e.g. dsai1) — see `docs/solutions/logic-errors/chat-pre-overflow-compaction-blindness.md`. |
