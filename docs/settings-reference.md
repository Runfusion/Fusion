...
| `plannerOverseerAdvisorProvider` | `""` | Workflow-native session-advisor model provider (OMP advisor parity). Must be set together with `plannerOverseerAdvisorModelId`. Empty keeps live transcript advising soft-disabled even when oversight is `autonomous` (cost-safe dual gate). |
| `plannerOverseerAdvisorModelId` | `""` | Workflow-native session-advisor model id. Must be set together with `plannerOverseerAdvisorProvider`. When both are set, the session advisor reviews executor agent-log deltas and may inject `[session-advisor]` steering comments at `steer`/`autonomous` (observe = log only). Discover project review priorities via `OVERSEER.md` / `WATCHDOG.md`. See `docs/architecture.md` → "Planner overseer session advisor". |
