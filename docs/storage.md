...
- The migrator recognizes only documented retired SQLite schema (`tasks.breakIntoSubtasks` and central `globalConcurrency`) and reports it as an intentional skip. It also completes a source as a stale duplicate only when all of its attempted rows conflict and PostgreSQL is already ahead in that same project partition. This preserves newer PostgreSQL data rather than overwriting it; an unknown column/table, a partial copy, or same-count content divergence without that source-level evidence remains a real failure.
- Retained `fusion.db`, `archive.db`, and `fusion-central.db` files are migration inputs and operator backups only. The migrator never edits, moves, quarantines, or deletes them automatically. Startup reads a source only while its matching `fusion_sqlite_migrations` key is incomplete: `project:<projectId>` gates core/archive/identity work, `central:legacy-sqlite` gates central work, and `project-plugins:<canonical project path>` independently gates plugin adoption. A completed core marker makes a retained duplicate inert but never suppresses a still-incomplete plugin bridge.
...
- Appends inspect the existing tail and insert a separator newline when its final line is unterminated. An interrupted write therefore costs at most its interrupted entry; later rows remain independently parseable and their stored `lineNo` and `agentLog:<taskId>:<lineNo>` source refs remain aligned with physical file lines. Reads retain corrupt historical lines as skipped data and report them once per read with a bounded summary.
