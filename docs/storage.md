...
File-scope contention keeps its durable synchronization obligation in `project.task_overlap_waits`, independently of the transient `tasks.overlap_blocked_by` display marker. Each row is partitioned by `(project_id, task_id, episode_id)`, snapshots the predecessor identity, and advances through observation, freshness, ready, delivery, or cancellation phases with revision/owner compare-and-set fencing. Clearing or replacing the marker therefore cannot erase an unconsumed **delivery**. Reset cancels the reset task's active generations and stamps its incoming waits in the same task-locked publication transaction, preserving any landed snapshots before clearing merge metadata. `reconcileTaskOverlapWaits` retires unlanded waits behind reset, deleted, missing, or completed predecessors; a real landed commit still requires ordinary checkout synchronization. This distinction prevents a discarded execution from becoming a permanent missing-delivery blocker. The obsolete model-verdict phases are drained to `ready` by migration 0077 without deleting delivery proof.
...
Startup and periodic self-healing run `reconcile-released-overlap-waits`, including failed work whose display marker is already absent. The same reconciliation runs before execution synchronization. It removes obsolete scheduling indicators and resumes an idle missing-delivery or superseded-owner failure **in its current work lane**, preserving steps, reviews, branches and worktrees. It does not clear other failures, fabricate delivered files, override user pauses/live continuations or auto-merge-Off policy, or remove explicit dependencies. Pre-upgrade resets with a retained successful original-description replacement log can also be recognized; incomplete branch-cleanup logs alone are not sufficient proof of a committed Reset.
...
Legacy archive snapshots may preserve deliveries that predate the live completion writer. `reconcilePatchnodeLedger` re-arms every 15 minutes to backfill current completion rows, latest-only legacy revert markers, and historical snapshots whose `preArchiveColumn` proves completion. It also runs two bounded, provenance-proved repair passes over EXISTING rows: the degenerate `title = task_id` label marker, and a body still equal to the live `tasks.summary`, which is the only available proof that it predates the plan-sourced description. It is a backlog convenience, not the live guarantee. A delivery completed and superseded before Patchnode existed left no reliable lane, occurrence, or point-in-time summary evidence and is intentionally not fabricated.

Delivery evidence itself is only ever inserted, never rewritten. The single exception is FN-444's bounded label-repair pass, reported as `labelsRepaired`. `Task.title` is optional and FN-391 removed every title backfill, so the pre-FN-444 capture builder wrote `title = task_id` for each titleless task and the History card showed the identifier twice. The repair pass selects only entries whose stored label is exactly their own task id, joins the still-live (not soft-deleted) task, and rewrites that label to the canonical projection (stored title, else the first 220 description characters exactly, else the id). It creates no entry, occurrence, lane, or date; it never rewrites a point-in-time summary, clearing `body` only when that field too was a copy of the identifier; it leaves entries whose task is gone untouched; and it is idempotent and bounded by the same page size and page cap as the other passes.
...
`project.task_lifecycle_events` is the write-only durable source for cross-process task lifecycle observation. The delete transaction conditionally claims the first `deleted_at` transition; only its winner writes the audit record and one outbox event. A concurrent loser re-reads the project-scoped deleted task in that transaction and has no writer-side audit, event, emit, or mailbox effect.

`task:entered-review` is the second event type. The store's move transaction records each crossing into the review lane — previous column, target column, entry timestamp, and the `moveSource` that names the path — so engine, scheduler, workflow-graph, completion-handoff, dashboard-drag, and CLI-bundle entries all leave the same committed evidence, because all of them funnel through one move implementation. Pre-existing entries are deliberately not backfilled: absent events mean no record was ever written, not that the card never entered review.

Events use `(project_id, seq)` and a deterministic `evt_` SHA-256 identity over project, event type, task ID, and occurrence timestamp. `project.task_lifecycle_event_seq` allocates that per-project sequence through a transactional counter upsert. Its lock is held until commit, so allocation order equals commit order, committed rows are in-order and gap-free, and rollback reverts the counter without consuming a number. Payloads are a discriminated union keyed on the event type: `task:deleted` carries task IDs, previous lane/status, deletion timestamp, resurrection/issue action, and actor ID fields; `task:entered-review` carries the task ID, previous column, target column, entry timestamp, and move source.
