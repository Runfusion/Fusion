...
If a task is found in `in-review` with failed pre-merge workflow results and no active executor, self-healing ordinarily auto-revives it by replaying the same remediation send-back flow. A terminal failed **pre-merge review** result with no verdict is distinct: it is treated as a lost review dispatch, and the engine re-seeds that exact review node through its idle continuation fence instead of requesting implementation remediation or clearing merge state. This bounded recovery runs only for singular, auto-merge-eligible, unpaused cards with no active continuation or merge owner; it retains the failed result and its findings until the fresh review records a real result. A singular task uses its task worktree; a workspace task uses the acquired sub-repository worktree selected by the failed repository review outcome. Recovery is checkout-unavailable only when neither a singular worktree nor a usable acquired sub-repository worktree can be resolved. Generic optional gates use the resolved workflow/project budget; built-in Plan Review and most Code Review groups are unbounded unless workflow settings or node config set a numeric cap. Compound Engineering's Code Review node supplies a two-pass cap.
...
A bypass is refused when the failed result contains any open finding, including a no-verdict result carrying a genuine critical finding. Those cards must take the fenced re-review recovery and remain merge-blocking until a fresh valid approval is persisted. A failed no-verdict result with no open findings may still use the existing audited operator escape hatch; automatic recovery never invokes it.
