...
- Recovery (FN-179): Recovered the `fusion/fn-152` branch and linked worktree in the nested `fusion-source` repository after root-repo cleanup deleted the original branch. The task-agnostic pre-commit hook implementation, linked-worktree regression tests, architecture note, and changelog entry from FN-152 remain intact (3 commits, 11/11 engine tests passing). The root Contxt repo cannot track `fusion-source/` changes because the directory is ignored, so recovery was performed in the nested repository itself. Follow-up task FN-184 created for the soft-deletion blocker preventing normal merge confirmation.
- a1b2c3d: Make the worktree identity pre-commit guard task-agnostic so the shared `.git/hooks/pre-commit` no longer embeds a baked task-specific branch default. The hook now derives `EXPECTED_BRANCH` from worktree-local `fusion-task-id` metadata at runtime using POSIX `tr '[:upper:]' '[:lower:]'`, eliminating overwrite churn across linked worktrees and removing the risk of sibling worktrees inheriting a stale baked default. FN-149/FN-091 uppercase metadata normalization behavior remains preserved.
