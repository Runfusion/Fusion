...
| `fn_task_done` | End the task: `outcome="completed"` (default) marks it complete; `outcome="blocked"` honestly parks it failed (`BLOCKED: <reason>`) with no completion claim, preserving steps/worktree and recording `blockedBy` as dependencies. Accepted outside-worktree blocks may supply a safe four-part operator report; Fusion bounds and redacts it before durable mailbox delivery. | `summary?` (string), `outcome?` (`completed` \| `blocked`), `blockedBy?` (string[]), `reason?` (string, required when blocked), `blockedReport?` ({ `verifiedCondition?`, `stopReason?`, `unimplementedWork?`, `unblockCondition?` }; never prompts, logs, stack traces, or credentials) |
