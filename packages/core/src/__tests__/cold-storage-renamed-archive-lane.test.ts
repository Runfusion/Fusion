...
them. `listTasksImpl` therefore has to decide whether to read that second store, and it decided with

    includeArchived && (!columnFilter || columnFilter === "archived")

So a caller naming a RENAMED archive lane — `listTasks({ column: "filed", includeArchived: true })`,
which is exactly what an archive view does — skipped cold storage and got an empty page. The rows
were unreachable through the only API that can reach them.

Note the shape: the UNFILTERED read (`!columnFilter`) was always correct, so this fails only for the
caller that names the lane. That is why it survives a board-level smoke test and why the failure
looks like "the archive is empty" rather than like a bug.

Driven through `listTasksImpl` with a fake cold-storage reader, so the assertion is about the
DECISION to consult it. Whether the archive store returns the right rows is its own contract and has
its own tests; conflating them would make this pass for the wrong reason.
*/
