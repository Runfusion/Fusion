 ...
 * The probe timeout claims to prevent orphan children. Assert it, because settling the
 * promise on time and terminating the child on time are separate behaviours, and only
 * the second one keeps the claim true.
 */
describe("probe timeout containment", () => {
  it("SIGKILLs the hung lookup child when the probe budget elapses", async () => {
    vi.useFakeTimers();
    try {
      const kills: string[] = [];
      const spawnMock = vi.fn(() => {
        ...
        child.stderr = new EventEmitter();
        child.pid = 8080;
        child.kill = vi.fn((signal: string) => { kills.push(signal); });
        // Never emits close: this is the hung probe the timeout exists for.
        return child;
      });

      vi.doMock("node:child_process", () => ({ spawn: spawnMock }));
      const { detectFnBinary } = await import("../cli/fn-binary.js");

      const pending = detectFnBinary();
      // One lookup probe per candidate binary ("fn", "fusion"); each must time out
      // before detectFnBinary reports the binary as absent.
