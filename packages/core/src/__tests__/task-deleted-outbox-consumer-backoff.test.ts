...
idle/active feedback from each poll drives the rescheduling delay; fencing/ordering/at-least-once
delivery logic is untouched (backoff only changes when poll() runs).
*/
...
describe("TaskDeletedOutboxConsumer idle backoff", () => {
  ...
  it("disarms the rescheduled timer on stop so shutdown cannot leave an orphaned poll", async () => {
    ...
    // No further polls fire after stop(); a manual poll() is a no-op while not running.
