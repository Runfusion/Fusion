...
describe("resolveWorktreeCapacityLimit", () => {
  it("returns the configured limit when worktrees are enabled", () => {
    expect(resolveWorktreeCapacityLimit({ maxWorktrees: 7, worktreesEnabled: true })).toBe(7);
  });

  it("treats an omitted worktreesEnabled as ON (back-compat for every existing project)", () => {
    // Existing rows have no `worktreesEnabled` key. They must keep gating exactly
  ...
  it("returns null — not Infinity, not a large number — when worktrees are off", () => {
    const resolved = resolveWorktreeCapacityLimit({ maxWorktrees: 4, worktreesEnabled: false });
  ...
  it("ignores maxWorktrees entirely when off, including values that would deadlock", () => {
    ...
    expect(resolveWorktreeCapacityLimit({ maxWorktrees: 0, worktreesEnabled: false })).toBeNull();
    expect(resolveWorktreeCapacityLimit({ maxWorktrees: 999, worktreesEnabled: false })).toBeNull();
    expect(resolveWorktreeCapacityLimit({ worktreesEnabled: false } as never)).toBeNull();
  });

  it("falls back to the shipped default when the limit is missing or non-finite", () => {
    expect(resolveWorktreeCapacityLimit({ worktreesEnabled: true } as never)).toBe(DEFAULT_SETTINGS.maxWorktrees);
  ...
  it("ships with worktrees enabled by default", () => {
    ...
    expect(DEFAULT_SETTINGS.worktreesEnabled).toBe(true);
  });
});
