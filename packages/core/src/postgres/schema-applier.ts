...
/* FNXC:Identity 2026-08-23-23:49: 0067 is the identity actors schema. Main already shipped 0061-0066, so identity cannot keep 0061 or upgraded DBs would skip the identity tables. */
 ...
 * The identity schema's own immutable bookkeeping identity. Renumbered 0047 -> 0059 -> 0060 -> 0061 -> 0067
 ...
 * immutable only once RELEASED; this one has not been. Main shipped 0061-0066 after the previous
 * identity remumber, so identity must be 0067.
 */
  ...
  */
  ...
  // NOT EXISTS), so re-running is safe.
    ...
    */
    ...
    error, so dropping them would brick legacy upgrades). Apply AFTER main's 0066 chat memory-focus so
    upgraded databases that already recorded 0061-0066 still receive the identity tables as 0067.
    */
