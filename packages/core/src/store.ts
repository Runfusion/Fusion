...
import { createRunAuditSnapshot, type ActivityLogSnapshot, type RunAuditSnapshot, type TaskMetadataSnapshot } from "./shared-mesh-state.js";
...
import { taskToArchiveEntryImpl, deleteTaskBackendImpl, archiveTaskBackendImpl, unarchiveTaskImpl, restoreFromArchiveImpl } from "./task-store/archive-lifecycle-2.js";
...
/** non-string, empty, or whitespace value deletes that nodeId override, which */
  ...
  async pauseTask( id: string, paused: boolean, runContext?: RunMutationContext, agentOptions?: { pausedByAgentId?: string }, ): Promise<Task> {
...
/** Record a successful verification pass for the given tree sha and commands. */
  ...
  async getTaskMetadataSnapshot(): Promise<TaskMetadataSnapshot> {
    return getTaskMetadataSnapshotImpl(this);
  }
  async applyTaskMetadataSnapshot(snapshot: TaskMetadataSnapshot): Promise<{ applied: number; skipped: number }> {
    return applyTaskMetadataSnapshotImpl(this, snapshot);
  }
  async getActivityLogSnapshot(limit = 10_000): Promise<ActivityLogSnapshot> {
    return getActivityLogSnapshotImpl(this, limit);
  }
  applyActivityLogSnapshot(snapshot: ActivityLogSnapshot): { applied: number; skipped: number } {
    return applyActivityLogSnapshotImpl(this, snapshot);
  }
  async applyActivityLogSnapshotAsync(snapshot: ActivityLogSnapshot): Promise<{ applied: number; skipped: number }> {
    return applyActivityLogSnapshotAsyncImpl(this, snapshot);
  }
  ...
  applyRunAuditSnapshot(snapshot: RunAuditSnapshot): { applied: number; skipped: number } {
    return applyRunAuditSnapshotImpl(this, snapshot);
  }
  async applyRunAuditSnapshotAsync(snapshot: RunAuditSnapshot): Promise<{ applied: number; skipped: number }> {
    return applyRunAuditSnapshotAsyncImpl(this, snapshot);
  }
