...
export async function restoreTaskFromArchive(
  ...
  */
  try {
    return await restoreTaskFromArchiveOnce(layer, entry, options, false);
  } catch (error) {
    if (!isPostgresUniqueError(error)) throw error;
    return await restoreTaskFromArchiveOnce(layer, entry, options, true);
  }
}
      ...
      */
      ...
      await insertTaskRowInTransaction(
        tx,
        {
          ...
          The unique-violation retry forces the segment null on the insert path as well — the
          snapshot may carry a name a successor already claimed, and the next acquisition remints.
          */
          ...(releaseSegment ? { workspaceWorktreeDirSegment: null } : {}),
          column: options.targetColumn ?? "archived",
