...
FN-408 adds an explicit per-card human decision AFTER Plan Review is satisfied, independent of the
project `planApprovalMode` and of Fast execution. The decision is operator-authored proof; it is
deliberately NOT derived from `approvedPlanFingerprint`, which triage and Plan Review write
automatically and which therefore proves only plan identity, never human intent.

`planFingerprint` pins WHICH plan was decided and `planningEpisodeId` pins WHICH review episode
produced it, so a rejected-then-regenerated byte-identical plan (same fingerprint, new episode)
still requires a fresh decision. Creation may only arm `enabled`; a client-supplied `decision` is
refused so no caller can forge release proof.
*/
export type HumanPlanApprovalDecisionKind = "approved" | "rejected";

export interface HumanPlanApprovalDecision {
  /** Operator request identity. Repeating it is idempotent; an opposite decision is a conflict. */
  requestId: string;
  decision: HumanPlanApprovalDecisionKind;
  /** Free text. Approval notes become implementation context; rejection notes become planner feedback. */
  message?: string;
  decidedBy: string;
  decidedAt: string;
  /** computePlanApprovalFingerprint of the decided plan. */
  planFingerprint: string;
  /** Identity of the Plan Review episode that produced the decided plan. */
  planningEpisodeId: string;
}

export interface HumanPlanApprovalState {
  /** Explicitly armed at creation. Absent state or `false` preserves the existing project policy. */
  enabled: boolean;
...
export interface TaskReleaseGateVerdict {
  ...
  /* FNXC:HumanPlanApproval 2026-09-15-06:24: FN-408's per-card decision refusal is surfaced to the browser as its own reason. */
  reason: "plan-review-pending" | "human-plan-approval-pending" | "planning-status" | "needs-replan" | "duplicate-prompt" | "seed-prompt" | "awaiting-approval" | null;
...
export interface Task {
  ...
  /**
   * FNXC:HumanPlanApproval 2026-09-15-06:24:
   * FN-408 per-card human plan validation. Absent means "no per-card requirement" and preserves the
   * project/workflow policy exactly. This field is never populated from the removed FN-234
  ...
  */
  ...
  /** FNXC:HumanPlanApproval 2026-09-15-06:24: FN-408's per-card decision hold, distinct from the revision-cap park. */
  awaitingApprovalReason?: "release-authorization" | "plan-review-replan-cap" | "merge-blocked-by-policy" | "code-review-non-convergence" | "human-plan-approval";
  /*
...
export interface TaskCreateInput {
  ...
  /**
   * FNXC:HumanPlanApproval 2026-09-15-06:24:
   * FN-408 — creation arms the per-card requirement with a BOOLEAN only. The decision object is
