...
type AgentLogAttribution = { provider: string; modelId: string };
...
type AgentLogRenderGroup =
  | {
    ...
    key: string;
    showBadge: boolean;
    attribution: AgentLogAttribution | null;
  }
  | {
    kind: "text" | "thinking";
...
function parseLogEntryRuntimeModel(entry: AgentLogEntry): AgentLogAttribution | null {
  const role = entry.agent === PLANNER_AGENT_ROLE
    ? "Planning"
    : entry.agent === "executor"
      ? "Executor"
      : entry.agent === "reviewer"
        ? "Reviewer"
        : null;
  if (!role || (entry.type !== "status" && entry.type !== "text")) return null;
  return parseRuntimeModelMarker(entry.text, role);
}

function attributionKey(value: AgentLogAttribution | null): string {
  return value ? `${value.provider}/${value.modelId}` : "";
}

function uniqueGroupAttribution(
  entries: readonly AgentLogEntry[],
  seed: AgentLogAttribution | null,
): AgentLogAttribution | null {
  const seen = new Map<string, AgentLogAttribution>();
  if (seed) seen.set(attributionKey(seed), seed);
  for (const entry of entries) {
    const marker = parseLogEntryRuntimeModel(entry);
    if (marker) seen.set(attributionKey(marker), marker);
  }
...
function buildRenderGroups(renderEntries: RenderEntry[], entryKeys: string[]): AgentLogRenderGroup[] {
  ...
  const lastMarkerByAgent = new Map<string, AgentLogAttribution>();

  for (let i = 0; i < renderEntries.length; i += 1) {
    const { entry, hiddenToolBoundaryId } = renderEntries[i];
    const incomingMarker = parseLogEntryRuntimeModel(entry);
    if (incomingMarker && entry.agent) lastMarkerByAgent.set(entry.agent, incomingMarker);
    ...
    if (entry.type === "text" || entry.type === "thinking") {
      ...
      const groupProvenance = (entry.agent ? lastMarkerByAgent.get(entry.agent) : undefined) ?? null;
      let j = i + 1;
      while (j < renderEntries.length) {
        ...
        Historic text markers and later lane switches must start a new group. One role-wide current
        model would otherwise disclose model B on output generated by model A.
        */
        const nextMarker = parseLogEntryRuntimeModel(nextEntry);
        if (nextMarker && attributionKey(nextMarker) !== attributionKey(groupProvenance)) break;
        groupedEntries.push(nextEntry);
        if (nextMarker && nextEntry.agent) lastMarkerByAgent.set(nextEntry.agent, nextMarker);
        j += 1;
      }
      ...
      groups.push({
        ...
        key: `${rowKey}->${endKey}`,
        showBadge,
        attribution: uniqueGroupAttribution(groupedEntries, groupProvenance),
      });
      i = j - 1;
      continue;
    }
    ...
    groups.push({
      ...
      key: rowKey,
      showBadge,
      attribution: incomingMarker ?? (entry.agent ? lastMarkerByAgent.get(entry.agent) ?? null : null),
    });
  }
