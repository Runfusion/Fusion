...
vi.mock("../ChatView", () => ({
  ChatView: ({ initialDirectSession, initialDirectSessionNonce, onOpenSessionInNewWindow, experimentalFeatures }: any) => <div data-testid={`chat-${initialDirectSession.id}`} data-session-nonce={initialDirectSessionNonce} data-alpha={String(experimentalFeatures?.alphaUpdates === true)} onClick={() => onOpenSessionInNewWindow(initialDirectSession)} />,
}));
...
describe("PoppedOutChatWindows", () => {
  it("renders independent selected chats and closes only the requested entry", () => {
    ...
    render(<PoppedOutChatWindows entries={[entry("a", 2, 0), entry("b", 7, 1), { ...entry("other"), projectId: "project-b" }]} projectId="project-a" addToast={vi.fn()} experimentalFeatures={{ alphaUpdates: true }} onClose={onClose} onOpenSessionInNewWindow={onOpenSessionInNewWindow} />);
    expect(screen.getByTestId("chat-a")).toHaveAttribute("data-session-nonce", "2");
    expect(screen.getByTestId("chat-a")).toHaveAttribute("data-alpha", "true");
