 ...
 * single-flight cache so `/api/models` never spawns the `agy` CLI per
 * request.
 *
 * FNXC:AntigravityCli 2026-07-08-00:00:
 * FN-7705: mirrors the landed Cursor picker cache (cursor-model-cache.ts,
 * FN-7696) end to end. With the Antigravity Runtime plugin installed and the
 * "Antigravity — via Antigravity CLI" provider toggle enabled (`useAntigravityCli === true`),
 ...
 *      `/api/models` requests spawn `antigravity` at most once per TTL window.
 * A missing/failed/unavailable `antigravity` binary (ENOENT, non-zero exit,
 ...
 * unavailable binary does not turn into a spawn-per-request storm. Antigravity has
 * its own settings toggle (`useAntigravityCli`); the toggle gate lives in the
...
/** The picker provider id used for all Antigravity-derived model rows. */
export const ANTIGRAVITY_PICKER_PROVIDER_ID = "antigravity-cli" as const;

/** Default cache TTL for Antigravity model discovery, in milliseconds. */
...
/**
 * Map Antigravity CLI discovery output into the stable `/api/models` row shape.
 *
 ...
 * `false`/`0` — the real `antigravity models` text output carries no such
...
/** Per-binaryPath cache of the most recently resolved Antigravity picker models. */
...
export interface GetAntigravityPickerModelsOptions {
  /** Override the Antigravity CLI binary path. Defaults to `"antigravity"`. */
  binaryPath?: string;
...
/**
 * Resolve the Antigravity CLI binary path: explicit override, then the bare
 * `"agy"` command (resolved via PATH by the CLI spawn layer).
 */
...
/**
 * Fetch Antigravity CLI-discovered models for the model picker, behind a
 * short-TTL, single-flight cache keyed by binary path.
 *
 * Never throws: a `discoverAntigravityCliModels` failure or an unavailable-binary
...
export async function getAntigravityPickerModels(
  ...
  const fetchPromise = (async (): Promise<AntigravityPickerModel[]> => {
    try {
      ...
      // Degrade to zero Antigravity rows on any spawn/parse failure (ENOENT,
      // non-zero exit, timeout, no API key configured) — never let a Antigravity
      // error propagate into /api/models. See FNXC:AntigravityCli comment above.
      return [];
    }
  })();
