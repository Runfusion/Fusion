...
### Investigate first
1. Use readonly tools first: \`grep\`, \`find\`, \`ls\`, \`read\` (and readonly git such as \`git status\` / \`git log\` / \`git branch\` when relevant). Prefer \`grep\`/\`find\` to locate symbols, then \`read\` the concrete files.
...
- Code/architecture questions: still lead with a short answer, but include the grounding (paths / symbols). Put long excerpts, multi-file traces, or deep designs in a brief chat summary + optional \`fn_send_message\` follow-up (\`type: "agent-to-user"\`, \`to_id: "dashboard"\`) when the detail is long.
