...
const allowlist: AllowlistEntry[] = [
  ...
  // FNXC:EngineProcessRules 2026-07-26-00:54: keep the audited short git-plumbing allowlist pinned to current source after hold-continuation recovery moved the self-healing call sites again.
  { file: "src/self-healing.ts", line: 4386, primitive: "execSync", signature: "const tipSha = String(execSync(`git rev-parse --verify ${shellQuote(branch)}`, {", reason: SHORT_GIT_PLUMBING },
  { file: "src/self-healing.ts", line: 4392, primitive: "execSync", signature: "const uniqueCommitCount = Number.parseInt(String(execSync(`git rev-list --count ${shellQuote(branch)} --not ${shellQuote(\"main\")}`, {", reason: SHORT_GIT_PLUMBING },
  { file: "src/self-healing.ts", line: 4429, primitive: "execSync", signature: "const branchesRaw = String(execSync(\"git branch --list 'fusion/*'\", {", reason: SHORT_GIT_PLUMBING },
  { file: "src/self-healing.ts", line: 13151, primitive: "execSync", signature: "execSync(`git branch -d ${shellQuote(branch)}`, {", reason: SHORT_GIT_PLUMBING },
  ...
  { file: "src/executor.ts", line: 16659, primitive: "execSync", signature: "execSync(`git merge-base --is-ancestor ${task.baseCommitSha} HEAD`, {", reason: SHORT_GIT_PLUMBING },
];
