...
describe("FN-8141 follow-up 1 — blocked park survives graph teardown", () => {
  ...
  it("keeps re-attempting deferred terminal persistence until the store recovers (no round cap)", async () => {
    // Store outage: every terminal failed-write is rejected. The initial bounded
    // round exhausts (7 updateTask attempts), then deferred fenced rounds keep
    // trying via updateTaskAtomic — past the old 31-round cap — and the row is
    // parked as soon as the store comes back.
    const { store, task, executor } = makeHarness({
      status: null,
      error: null,
    });
    ...
    store.updateTask.mockImplementation(async (_id: string, patch: any) => {
      if (patch?.status === "failed") throw new Error("store outage");
      return { ...task, ...patch };
    });
    ...
    store.updateTaskAtomic = vi.fn(async (_id: string, reducer: (current: any) => any) => {
      atomicAttempts += 1;
      if (atomicAttempts <= OUTAGE_ROUNDS) throw new Error("store outage");
    ...
    const terminalWrites = store.updateTask.mock.calls.filter(
      (call: unknown[]) => (call[1] as { status?: string } | undefined)?.status === "failed",
    );
    // One bounded round of 7 attempts; the deferred rounds fence via updateTaskAtomic.
    expect(terminalWrites).toHaveLength(7);
    expect(atomicAttempts).toBe(OUTAGE_ROUNDS + 1);
