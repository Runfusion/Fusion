...
describe("recoverPausedAbortFailures", () => {
  ...
  it("rehomes an in-progress pause-abort park back to todo", async () => {
    ...
    expect(store.moveTask).toHaveBeenCalledWith(
      "FN-7001",
      "todo",
      { preserveProgress: true, preserveWorktree: true, moveSource: "engine", lifecycleReason: "self-healing-session-recovery", recoveryRehome: true }, UNATTRIBUTED_MUTATION_CONTEXT,
    );
