...
async function parkTaskFailed(
  ...
  20-30s under planning load). Exponential backoff to ~31s total window; only a
  ...
  */
  ...
  executorLog.error(
    `${taskId}: failed to persist failed status after ${backoffMs.length} attempts spanning ~2m: ${lastError instanceof Error ? lastError.message : String(lastError)}`,
  );
  return false;
}
