...
**Cannot proceed — the honest blocked exit.** If the work genuinely cannot be finished because of a host-resource, network, model-provider, or credential failure, or because a prerequisite Fusion task is incomplete, call \`fn_task_done(outcome="blocked", reason="<concrete blocker + what would unblock it>", blockedBy=["FN-XXXX"])\`. Missing tooling, optional services, and unrunnable commands are not blocked exits: resolve them, substitute a runnable check, or complete achievable work and record the deferred verification. A classified external block freezes in place; task dependencies park durable failed WITHOUT auto-replan and requeue when those tasks complete. For an accepted external block, include \`blockedReport\` with concise verified condition, stop reason, unimplemented work, and precise unblock condition; never include prompts, tool logs, stack traces, or credentials. Blockers must be Fusion board tasks — do NOT treat open GitHub PRs touching the same files as blockers; other PRs are not claims on your file scope. Do NOT skip remaining steps to fake completion.
