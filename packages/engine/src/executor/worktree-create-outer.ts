...
export async function rebaseNewWorktreeOntoRemote(
  ...
  let defaultBranch = "";
  try {
    const { stdout } = await execAsync(`git rev-parse --abbrev-ref ${remote}/HEAD`, { cwd: rootDir });
  ...
  try {
    await execAsync(`git fetch ${quoteShellArg(remote)} ${quoteShellArg(defaultBranch)}`, { cwd: rootDir });
  } catch (err) {
    executorLog.warn(
