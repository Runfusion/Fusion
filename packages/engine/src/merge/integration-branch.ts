...
run therefore materializes the branch from its remote-tracking start point with plain
`git branch` (never --track, matching ensureIntegrationBranchLocalRef in core FN-183) before
...
async function materializeLocalBranch(rootDir: string, branch: string): Promise<boolean> {
  ...
  try {
    await execFileAsync("git", ["branch", branch, remoteRef], {
      cwd: rootDir,
      timeout: 5_000,
    });
    return true;
...
function materializeLocalBranchSync(rootDir: string, branch: string): boolean {
  ...
  try {
    execFileSync("git", ["branch", branch, remoteRef], {
      cwd: rootDir,
      timeout: 5_000,
      stdio: ["ignore", "ignore", "ignore"],
    });
