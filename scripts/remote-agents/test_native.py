...
from collector import connect, bind, scan, validated_base_url, post
...
class NativeTests(unittest.TestCase):
    ...
    def test_collector_url_restricts_private_http_and_embedded_credentials(self):
        ...
        with mock.patch.dict(os.environ, {'FUSION_REMOTE_AGENTS_HTTP_HOSTS': 'relay-host'}):
            self.assertEqual(validated_base_url('http://relay-host:4040'), 'http://relay-host:4040')
    ...
    def test_named_relay_host_requires_env_in_every_process_including_hooks(self):
        ...
        # environment too, not only the collector service's. Without it every delivery fails closed.
        with mock.patch.dict(os.environ, {}, clear=False):
            os.environ.pop('FUSION_REMOTE_AGENTS_HTTP_HOSTS', None)
            with self.assertRaises(ValueError):
                post('http://relay-host:4040', 'project', 't' * 32, 'feedback-ack', {})
        with mock.patch.dict(os.environ, {'FUSION_REMOTE_AGENTS_HTTP_HOSTS': 'relay-host'}):
            with mock.patch('collector.http.client.HTTPConnection') as connection:
                ...
                post('http://relay-host:4040', 'project', 't' * 32, 'feedback-ack', {})
            self.assertEqual(connection.call_args[0][0], 'relay-host')
    ...
    def test_hook_route_update_deduplicates_only_owned_invocations(self):
        ...
        old = shlex.join(['python3', '/owned/feedback_hook.py', '--url', 'http://127.0.0.1:14040', '--project', 'project', '--host', 'host-a', '--provider', 'codex', '--token-file', '/private/token', '--state', '/private/spool'])
        new = old.replace('http://127.0.0.1:14040', 'http://127.0.0.1:4040')
